...
#define bmdbg_info(fmt, ...) \
	pr_info("battman_dbg: %s: " fmt, __func__, ##__VA_ARGS__)

#define bmdbg_err(fmt, ...) \
	pr_err("battman_dbg: %s: " fmt, __func__, ##__VA_ARGS__)

/*****************/
/* Log functions */
/*****************/
#define TIMESTAMP_LENGTH		18
#define TIMESTAMP_AND_SEPARATOR_LENGTH	(TIMESTAMP_LENGTH + 2)
#define TIMESTAMP_DIVISOR		19200
static void somc_bmdbg_output_ulog(char *buf)
{
	int i;
	char *line_ptr, *timestamp_ptr, *message_ptr;
	u64 timestamp_tick;
	u32 timestamp_ms;
	...
	for (i = 0; buf[i] != '\0'; i++) {
		...
		buf[i] = '\0';
		if (strlen(line_ptr) >= (TIMESTAMP_AND_SEPARATOR_LENGTH)) {
			timestamp_ptr = line_ptr;
			timestamp_ptr[TIMESTAMP_LENGTH] = '\0';
			if (kstrtou64(timestamp_ptr, 16, &timestamp_tick) < 0)
				timestamp_tick = 0;
			timestamp_ms =
				(u32)(timestamp_tick / TIMESTAMP_DIVISOR);

			message_ptr = &line_ptr[TIMESTAMP_AND_SEPARATOR_LENGTH];

			pr_err("battman: [%7u.%03u] %s\n",
...
static ssize_t log_interval_show(const struct class *c,
...
static ssize_t log_interval_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
	struct somc_bmdbg_dev *bmdbg_dev = container_of(c,
					struct somc_bmdbg_dev, bmdbg_class);

	if (kstrtoint(buf, 10, &bmdbg_dev->bmlog_timer)) {
...
static ssize_t log_categories_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bmdbg_dev *bmdbg_dev = container_of(c,
					struct somc_bmdbg_dev, bmdbg_class);
...
static ssize_t log_categories_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
	struct somc_bmdbg_dev *bmdbg_dev = container_of(c,
					struct somc_bmdbg_dev, bmdbg_class);

	if (kstrtou64(buf, 0, &bmdbg_dev->bmlog_categories)) {
...
static ssize_t log_level_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bmdbg_dev *bmdbg_dev = container_of(c,
					struct somc_bmdbg_dev, bmdbg_class);
...
static ssize_t log_level_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
	struct somc_bmdbg_dev *bmdbg_dev = container_of(c,
					struct somc_bmdbg_dev, bmdbg_class);

	if (kstrtou32(buf, 0, &bmdbg_dev->bmlog_level)) {
							...
							void *data, size_t len)
{
	...
	if (len != sizeof(struct reg_dump_read_resp_msg)) {
		bmdbg_err("Invalid read response (glink packet size=%zu\n)",
									len);
		goto exit;
	}
	...
	if ((int)reg_read_resp_msg->byte_count < 0) {
		bmdbg_err("glink read failed (byte_count=%d)\n",
	...
	case MISC_BASE:
		...
		break;
	default:
		bmdbg_err("Invalid register address (address=%08x)\n",
...
static int somc_bmdbg_update_reg(struct somc_bmdbg_dev *bmdbg_dev, u16 addr)
{
	...
	if (rc) {
		bmdbg_err("pmic_glink_write failed (rc=%d)\n", rc);
		return rc;
	}
	...
	if (!rc) {
		bmdbg_err("Error, timed out sending message\n");
		return -ETIMEDOUT;
	}
...
static ssize_t regupdate_interval_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
	struct somc_bmdbg_dev *bmdbg_dev = container_of(c,
					struct somc_bmdbg_dev, bmdbg_class);

	if (kstrtoint(buf, 10, &bmdbg_dev->regupdate_timer)) {
		bmdbg_dev->regupdate_timer = BMLOG_DELAY_TIME_MS_DEFAULT;
	}
...
static ssize_t reg_chgr_chager_status_show(
	const struct class *c, const struct class_attribute *attr, char *buf)
{
	struct somc_bmdbg_dev *bmdbg_dev = container_of(c,
					struct somc_bmdbg_dev, bmdbg_class);
	size_t size;
					...
					struct somc_bmdbg_dev *bmdbg_dev)
{
	...
	if (bmdbg_dev->glink_client) {
		...
		if (rc < 0)
			bmdbg_err("Error unregistering from pmic_glink (rc=%d)\n",
									rc);
...
static int somc_bmdbg_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(bmdbg_dev->glink_client)) {
		...
		if (rc != -EPROBE_DEFER)
			bmdbg_err("Error in registering with pmic_glink (rc=%d)\n",
									rc);
		bmdbg_dev->glink_client = NULL;
		goto error;
	}
	...
	if (rc < 0) {
		bmdbg_err("Failed to create somc_bmdbg_class (rc=%d\n)", rc);
		goto error;
	}
