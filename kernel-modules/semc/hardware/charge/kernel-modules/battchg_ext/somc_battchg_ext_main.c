...
#define bcext_info(fmt, ...) \
	pr_info("battchg_ext: %s: " fmt, __func__, ##__VA_ARGS__)

#define bcext_err(fmt, ...) \
	pr_err("battchg_ext: %s: " fmt, __func__, ##__VA_ARGS__)
...
static int somc_bcext_handle_glink_notification(
	...
	union glink_oem_req_msg *req_msg, union glink_oem_resp_msg *resp_msg)
{
	...
	if (req_msg->hdr.owner != MSG_OWNER_OEM ||
		...
		bcext_err("unsupported message (owner=%u,type=%u)\n",
			                req_msg->hdr.owner, req_msg->hdr.type);
		goto out;
	}
	...
	case MSG_OPCODE_OEM_WRITE_BUFFER:
		...
		break;
	default:
		bcext_err("unsupported message (opcode=%u)\n",
							req_msg->hdr.opcode);
		goto out;
	}
						...
						u32 somc_prop_id, u32 *val)
{
	...
	if (rc < 0) {
		bcext_err("Failed to request reading buffer (rc=%d)\n", rc);
		return rc;
	}

	if (resp_msg.rbuf.prop_id != req_msg.rbuf.prop_id ||
					resp_msg.rbuf.data_size != 1) {
		bcext_err("Failed to get property (prop_id=%u)\n",
								somc_prop_id);
		return -EINVAL;
	}
						...
						u32 somc_prop_id, u32 val)
{
	...
	rc = somc_bcext_glink_request(bcext_dev, &req_msg, &resp_msg);
	if (rc < 0) {
		bcext_err("Failed to request wriring buffer (rc=%d)\n", rc);
		return rc;
	}

	if (resp_msg.wbuf.return_status != BATTMNGR_SUCCESS) {
		bcext_err("Failed to set property (prop_id=%u)\n",
								somc_prop_id);
		return -EINVAL;
	}
						...
						bool smart_charge_suspended)
{
	...
	if (!bcext_dev->smart_charge_enabled) {
		bcext_err(
			"Couldn't set smart charge voter due to unactivated\n");
		return rc;
	}
	...
	if (rc < 0) {
		bcext_err("Couldn't vote en (rc=%d)\n", rc);
		return rc;
	}
...
static int somc_apply_thermal_mitigation(struct somc_bcext_dev *bcext_dev)
{
	...
	if (rc < 0)
		bcext_err("Couldn't vote for thermal mitigation values (rc=%d)\n",
									rc);
...
static void somc_bcext_offchg_termination_work(struct work_struct *work)
{
	...
	bcext_info("input_report_key KEY_F24\n");
...
static ssize_t ets_mode_en_store(const struct class *c,
...
static ssize_t ets_mode_en_show(const struct class *c,
...
static ssize_t usb_input_suspend_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t usb_input_suspend_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t batt_aging_level_store(const struct class *c,
...
static ssize_t batt_aging_level_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t real_nom_cap_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_fw_update_store(const struct class *c,
					const struct class_attribute *attr,
					const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
...
static ssize_t wls_fw_update_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_int_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_en_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_iout_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_chip_id_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_guaranteed_pwr_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_vrect_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_vout_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_potential_pwr_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_fw_rev_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t batt_soc_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t monotonic_soc_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t system_soc_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_tx_ssp_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_tx_dc_pwr_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_tx_iin_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_cmd_reg_addr_store(const struct class *c,
					const struct class_attribute *attr,
					const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
...
static ssize_t wls_cmd_reg_addr_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_cmd_reg_data_store(const struct class *c,
					const struct class_attribute *attr,
					const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
...
static ssize_t wls_cmd_reg_data_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_negotiated_pwr_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_status_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_type_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_mfr_code_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_ping_freq_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_op_freq_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_sig_str_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_rpp_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_WLS_RPP, &rpp);

	return scnprintf(buf, PAGE_SIZE, "%d\n", rpp);
}
static CLASS_ATTR_RO(wls_rpp);

static ssize_t wls_current_icl_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_target_icl_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_WLS_TARGET_ICL, &icl);

	return scnprintf(buf, PAGE_SIZE, "%d\n", icl);
}
static CLASS_ATTR_RO(wls_target_icl);

static ssize_t wls_current_vout_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_target_vout_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_reg_sys_sts_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_cep_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_WLS_CEP, &cep);

	return scnprintf(buf, PAGE_SIZE, "%d\n", cep);
}
static CLASS_ATTR_RO(wls_cep);

static ssize_t wls_tx_notify_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_monitor_for_debug_store(const struct class *c,
					const struct class_attribute *attr,
					const char *buf,size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
...
static ssize_t wls_monitor_for_debug_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t wls_id_auth_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_WLS_IDAUTH_HWVER,
								&id_auth);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)id_auth);
}
static CLASS_ATTR_RO(wls_id_auth);

static ssize_t usb2_ccstat_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_CC_STATUS, &cc_stat);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)cc_stat);
}
static CLASS_ATTR_RO(usb2_ccstat);

static ssize_t usb2_nvm_write_show(const struct class *c,
				const struct class_attribute *attr,char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_NVM_RESET,
								&nvm_write);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)nvm_write);
}
static CLASS_ATTR_RO(usb2_nvm_write);

static ssize_t lrc_input_suspend_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t lrc_input_suspend_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t lrc_charge_disable_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t lrc_charge_disable_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t smart_charging_activation_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t smart_charging_activation_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t smart_charging_interruption_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t smart_charging_interruption_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t smart_charging_status_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t bootup_shutdown_phase_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t bootup_shutdown_phase_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t system_temp_level_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t system_temp_level_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t system_temp_level_max_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n", MAX_THERM_LEVEL);
}
...
static ssize_t usb_cc_open_status_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t real_temp_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t real_temp_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t batt_temp_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t vcell_max_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t product_code_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t sdm_therm_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t sdm_therm_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t pdo1_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t pdo3_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t pdo4_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t pdo5_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t pdo6_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t pdo7_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t pdo_h_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t apsd_result_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t current_step_phase_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t current_jeita_zone_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t battery_charger_status_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t d2cp_cfg_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t d2cp_cfg_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_D2CP_CFG, &d2cp_cfg);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)d2cp_cfg);
}
static CLASS_ATTR_RW(d2cp_cfg);

static ssize_t icm_state_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t fv_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t fcc_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t rvschg_suspend_store(const struct class *c,
					const struct class_attribute *attr,
					const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
...
static ssize_t rvschg_suspend_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t learned_capacity_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_LEARNED_CAPACITY,
							&learned_capacity);
...
static ssize_t learning_counter_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	u32 learning_counter;

	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_LEARNING_COUNTER,
							&learning_counter);
...
static ssize_t restored_capacity_store(const struct class *c,
					const struct class_attribute *attr,
					const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	int restored_capacity;

	if (kstrtoint(buf, 10, &restored_capacity))
		return -EINVAL;

	somc_bcext_set_prop(bcext_dev, BATTMNGR_SOMC_PROP_RESTORED_CAPACITY,
						(u32)restored_capacity);

	return count;
}
static ssize_t restored_capacity_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_RESTORED_CAPACITY,
							&restored_capacity);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)restored_capacity);
}
static CLASS_ATTR_RW(restored_capacity);

static ssize_t usb_icl_votable_status_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	size = somc_output_voter_param(bcext_dev->usb_icl_votable, buf,
								PAGE_SIZE);

	return size;
}
static CLASS_ATTR_RO(usb_icl_votable_status);

static ssize_t fcc_votable_status_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	size = somc_output_voter_param(bcext_dev->fcc_votable, buf, PAGE_SIZE);

	return size;
}
static CLASS_ATTR_RO(fcc_votable_status);

static ssize_t fake_chg_votable_status_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	size = somc_output_voter_param(bcext_dev->fake_chg_votable, buf,
								PAGE_SIZE);

	return size;
}
static CLASS_ATTR_RO(fake_chg_votable_status);

static ssize_t fake_chg_disallow_votable_status_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	size = somc_output_voter_param(bcext_dev->fake_chg_disallow_votable,
								buf, PAGE_SIZE);

	return size;
}
static CLASS_ATTR_RO(fake_chg_disallow_votable_status);

static ssize_t aicl_result_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_AICL_RESULT,
							&aicl_result);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)aicl_result);
}
static CLASS_ATTR_RO(aicl_result);

static ssize_t charger_max_wattage_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	u32 charger_max_wattage;

	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_CHAERGER_MAX_WATTAGE,
							&charger_max_wattage);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)charger_max_wattage);
}
static CLASS_ATTR_RO(charger_max_wattage);

static ssize_t active_charger_type_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	u32 active_charger_type;

	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_ACTIVE_CHAGER_TYPE,
							&active_charger_type);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)active_charger_type);
}
static CLASS_ATTR_RO(active_charger_type);

static ssize_t fcss_err_cnt_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_FCSS_ERR_CNT,
							&fcss_err_cnt);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)fcss_err_cnt);
}
static CLASS_ATTR_RO(fcss_err_cnt);

static ssize_t weak_limit_current_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	u32 weak_limit_current;

	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_WEAK_LIMIT_CURRENT,
							&weak_limit_current);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)weak_limit_current);
}
static CLASS_ATTR_RO(weak_limit_current);

static ssize_t charger_type_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	u32 battman_charger_type;
	int charger_type;

	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_ACTIVE_CHAGER_TYPE,
							&battman_charger_type);

	switch (battman_charger_type) {
	case SSDEV_PARTNER_NONE:
		charger_type = CHARGER_TYPE_NONE;
		break;
	case SSDEV_PARTNER_SNK_USB_SDP:
		charger_type = CHARGER_TYPE_USB_SDP;
		break;
	case SSDEV_PARTNER_SNK_USB_OCP:
		charger_type = CHARGER_TYPE_USB_OCP;
		break;
	case SSDEV_PARTNER_SNK_USB_CDP:
		charger_type = CHARGER_TYPE_USB_CDP;
		break;
	case SSDEV_PARTNER_SNK_USB_DCP:
		charger_type = CHARGER_TYPE_USB_DCP;
		break;
	case SSDEV_PARTNER_SNK_USB_FLOAT:
		charger_type = CHARGER_TYPE_USB_FLOAT;
		break;
	case SSDEV_PARTNER_SNK_TYPEC_DEFAULT:
		charger_type = CHARGER_TYPE_USB_TYPEC_DEFAULT;
		break;
	case SSDEV_PARTNER_SNK_TYPEC_RP_MEDIUM_1P5A:
		charger_type = CHARGER_TYPE_USB_TYPEC_1P5A;
		break;
	case SSDEV_PARTNER_SNK_TYPEC_RP_HIGH_3A:
		charger_type = CHARGER_TYPE_USB_TYPEC_3A;
		break;
	case SSDEV_PARTNER_SNK_PD:
		charger_type = CHARGER_TYPE_USB_PD;
		break;
	case SSDEV_PARTNER_SNK_PPS:
		charger_type = CHARGER_TYPE_USB_PD_PPS;
		break;
	case SSDEV_PARTNER_WLS_SNK_BPP:
		charger_type = CHARGER_TYPE_WLS_BPP;
		break;
	case SSDEV_PARTNER_WLS_SNK_EPP:
		charger_type = CHARGER_TYPE_WLS_EPP;
		break;
	default:
		charger_type = CHARGER_TYPE_UNKNOWN;
	}

	return scnprintf(buf, PAGE_SIZE, "%d\n", charger_type);
}
static CLASS_ATTR_RO(charger_type);

static ssize_t is_pd_charger_used_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	u32 battman_charger_type;
	int is_pd_charger_used;
	...
	case SSDEV_PARTNER_SNK_PD:
	case SSDEV_PARTNER_SNK_PPS:
		is_pd_charger_used = 1;
		break;
	default:
		is_pd_charger_used = 0;
	}

	return scnprintf(buf, PAGE_SIZE, "%d\n", is_pd_charger_used);
}
static CLASS_ATTR_RO(is_pd_charger_used);

static ssize_t is_decent_charging_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n",
				(charger_max_wattage >= 18000000) ? 1 : 0);
}
static CLASS_ATTR_RO(is_decent_charging);

static ssize_t is_fastest_charging_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	return scnprintf(buf, PAGE_SIZE, "%d\n",
				(charger_max_wattage >= 30000000) ? 1 : 0);
}
static CLASS_ATTR_RO(is_fastest_charging);

static ssize_t usb2_alert_irq_status_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	if (bcext_dev->usb2_feature_enabled)
		somc_bcext_get_prop(bcext_dev,
					BATTMNGR_SOMC_PROP_USB2_ALERT_INT,
					&stat);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)stat);
}
static CLASS_ATTR_RO(usb2_alert_irq_status);

static ssize_t usb2_attach_irq_status_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	if (bcext_dev->usb2_feature_enabled)
		somc_bcext_get_prop(bcext_dev,
					BATTMNGR_SOMC_PROP_USB2_ATTACH_INT,
					&stat);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)stat);
}
static CLASS_ATTR_RO(usb2_attach_irq_status);

static ssize_t vbat_full_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_VBAT_FULL,
					&vbat_full);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)vbat_full);
}
static CLASS_ATTR_RO(vbat_full);

static ssize_t qbg_calc_time_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_QBG_CALC_TIME,
					&qbg_calc_time);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)qbg_calc_time);
}
static CLASS_ATTR_RO(qbg_calc_time);

static ssize_t qbg_data_full_cnt_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	u32 qbg_data_full_cnt = 0;

	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_QBG_DATA_FULL_CNT,
					&qbg_data_full_cnt);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)qbg_data_full_cnt);
}
static CLASS_ATTR_RO(qbg_data_full_cnt);

static ssize_t invalid_resistance_cnt_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	u32 invalid_resistance_cnt = 0;

	somc_bcext_get_prop(bcext_dev,
				BATTMNGR_SOMC_PROP_INVALID_RESISTANCE_CNT,
				&invalid_resistance_cnt);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)invalid_resistance_cnt);
}
static CLASS_ATTR_RO(invalid_resistance_cnt);

static ssize_t soc_reset_adjust_active_show(const struct class *c,
	const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	u32 soc_reset_adjust_active = 0;

	somc_bcext_get_prop(bcext_dev,
				BATTMNGR_SOMC_PROP_SOC_RESET_ADJUST_ACTIVE,
				&soc_reset_adjust_active);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)soc_reset_adjust_active);
}
static CLASS_ATTR_RO(soc_reset_adjust_active);

static ssize_t soc_reset_adjust_msoc_show(const struct class *c,
	const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	u32 soc_reset_adjust_msoc = 0;

	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_SOC_RESET_ADJUST_MSOC,
					&soc_reset_adjust_msoc);

	return scnprintf(buf, PAGE_SIZE, "%d\n", (int)soc_reset_adjust_msoc);
}
static CLASS_ATTR_RO(soc_reset_adjust_msoc);

static ssize_t ets_usb_icl_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t ets_usb_icl_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t ets_wls_icl_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t ets_wls_icl_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t ets_fcc_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t ets_fcc_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t ets_i_term_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t ets_i_term_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t ets_vconn_store(const struct class *c,
	const struct class_attribute *attr, const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t ets_vconn_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t ets_cc_status_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t ets_snk_status_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t ets_usb_type_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static ssize_t pmic_reg_addr_store(const struct class *c,
					const struct class_attribute *attr,
					const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
	...
	somc_bcext_set_prop(bcext_dev, BATTMNGR_SOMC_PROP_PMIC_REG_ADDR,
								reg_addr);

	return count;
}
static ssize_t pmic_reg_addr_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_PMIC_REG_ADDR,
								&reg_addr);

	return scnprintf(buf, PAGE_SIZE, "0x%08X\n", reg_addr);
}
static CLASS_ATTR_RW(pmic_reg_addr);

static ssize_t pmic_reg_data_store(const struct class *c,
					const struct class_attribute *attr,
					const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
	...
	somc_bcext_set_prop(bcext_dev, BATTMNGR_SOMC_PROP_PMIC_REG_DATA,
								reg_data);

	return count;
}
static ssize_t pmic_reg_data_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_PMIC_REG_DATA,
								&reg_data);

	return scnprintf(buf, PAGE_SIZE, "0x%02X\n", reg_data);
}
static CLASS_ATTR_RW(pmic_reg_data);

static ssize_t ttf_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_TTF, &reg_data);

	return scnprintf(buf, PAGE_SIZE, "%d\n", reg_data);
}
static CLASS_ATTR_RO(ttf);

static ssize_t ttf_normal_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_NORMAL_TTF,
								&reg_data);

	return scnprintf(buf, PAGE_SIZE, "%d\n", reg_data);
}
static CLASS_ATTR_RO(ttf_normal);

static ssize_t ttf_smartcharge_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_SMART_CHARGE_TTF,
								&reg_data);

	return scnprintf(buf, PAGE_SIZE, "%d\n", reg_data);
}
static CLASS_ATTR_RO(ttf_smartcharge);

static ssize_t smart_charge_mode_store(const struct class *c,
					const struct class_attribute *attr,
					const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
	...
	if (kstrtouint(buf, 10, &reg_data)) {
		return -EINVAL;
	}

	somc_bcext_set_prop(bcext_dev, BATTMNGR_SOMC_PROP_SMART_CHARGE_MODE,
								reg_data);

	return count;
}
static ssize_t smart_charge_mode_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_SMART_CHARGE_MODE,
								&reg_data);

	return scnprintf(buf, PAGE_SIZE, "%d\n", reg_data);
}
static CLASS_ATTR_RW(smart_charge_mode);

static ssize_t smart_charge_stop_level_store(const struct class *c,
					const struct class_attribute *attr,
					const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
	...
	somc_bcext_set_prop(bcext_dev,
			BATTMNGR_SOMC_PROP_SMART_CHARGE_SUSPEND_LEVEL,
			reg_data);

	return count;
}
static ssize_t smart_charge_stop_level_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
...
static CLASS_ATTR_RW(smart_charge_stop_level);

static ssize_t battery_care_active_store(const struct class *c,
					const struct class_attribute *attr,
					const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
	...
	somc_bcext_set_prop(bcext_dev, BATTMNGR_SOMC_PROP_BATTERY_CARE_ACTIVE,
								reg_data);

	return count;
}
static ssize_t battery_care_active_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_BATTERY_CARE_ACTIVE,
								&reg_data);

	return scnprintf(buf, PAGE_SIZE, "%d\n", reg_data);
}
static CLASS_ATTR_RW(battery_care_active);

static ssize_t is_basic_plus_charging_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev,
			BATTMNGR_SOMC_PROP_CHAERGER_MAX_WATTAGE,
			&charger_max_wattage);

	return scnprintf(buf, PAGE_SIZE, "%d\n",
			(charger_max_wattage >= 7500000) ? 1 : 0);
}
static CLASS_ATTR_RO(is_basic_plus_charging);

static ssize_t force_invalid_battery_store(const struct class *c,
					const struct class_attribute *attr,
					const char *buf, size_t count)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	u32 force_invalid_battery_active;

	if (kstrtouint(buf, 10, &force_invalid_battery_active)) {
		return -EINVAL;
	}

	somc_bcext_set_prop(bcext_dev, BATTMNGR_SOMC_PROP_FORCE_INVALID_BATTERY,
						force_invalid_battery_active);

	return count;
}
static ssize_t force_invalid_battery_show(const struct class *c,
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	...
	somc_bcext_get_prop(bcext_dev, BATTMNGR_SOMC_PROP_FORCE_INVALID_BATTERY,
						&force_invalid_battery_active);

	return scnprintf(buf, PAGE_SIZE, "%d\n", force_invalid_battery_active);
}
static CLASS_ATTR_RW(force_invalid_battery);
...
static struct attribute *somc_bcext_class_attrs[] = {
	...
	&class_attr_wls_id_auth.attr,
	&class_attr_usb2_ccstat.attr,
	&class_attr_usb2_nvm_write.attr,
	&class_attr_learned_capacity.attr,
	&class_attr_learning_counter.attr,
	&class_attr_restored_capacity.attr,
	&class_attr_usb_icl_votable_status.attr,
	&class_attr_fcc_votable_status.attr,
	&class_attr_fake_chg_votable_status.attr,
	&class_attr_fake_chg_disallow_votable_status.attr,
	&class_attr_aicl_result.attr,
	&class_attr_charger_max_wattage.attr,
	&class_attr_active_charger_type.attr,
	&class_attr_fcss_err_cnt.attr,
	&class_attr_weak_limit_current.attr,
	&class_attr_charger_type.attr,
	&class_attr_is_pd_charger_used.attr,
	&class_attr_is_decent_charging.attr,
	&class_attr_is_fastest_charging.attr,
	&class_attr_usb2_alert_irq_status.attr,
	&class_attr_usb2_attach_irq_status.attr,
	&class_attr_vbat_full.attr,
	&class_attr_qbg_calc_time.attr,
	&class_attr_qbg_data_full_cnt.attr,
	&class_attr_invalid_resistance_cnt.attr,
	&class_attr_soc_reset_adjust_active.attr,
	&class_attr_soc_reset_adjust_msoc.attr,
	...
	&class_attr_pmic_reg_addr.attr,
	&class_attr_pmic_reg_data.attr,
	&class_attr_ttf.attr,
	&class_attr_ttf_normal.attr,
	&class_attr_ttf_smartcharge.attr,
	&class_attr_smart_charge_mode.attr,
	&class_attr_smart_charge_stop_level.attr,
	&class_attr_battery_care_active.attr,
	&class_attr_is_basic_plus_charging.attr,
	&class_attr_force_invalid_battery.attr,
	NULL,
};
...
static irqreturn_t idtp9382a_irq_handler(int irq, void *data)
{
	...
	bcext_info("irq_status=%d\n", stat);
	return IRQ_HANDLED;
}

static irqreturn_t usb2_alert_irq_handler(int irq, void *data)
{
	struct somc_bcext_dev *bcext_dev = data;
	int stat;

	stat = gpio_get_value(bcext_dev->usb2_gpio_alert_irq);
	bcext_dev->usb2_alert_irq_status = stat;
	somc_bcext_set_prop(bcext_dev, BATTMNGR_SOMC_PROP_USB2_ALERT_INT,
								(u32)stat);
	return IRQ_HANDLED;
}

static irqreturn_t usb2_attach_irq_handler(int irq, void *data)
{
	struct somc_bcext_dev *bcext_dev = data;
	int stat;

	stat = gpio_get_value(bcext_dev->usb2_gpio_attach_irq);
	bcext_dev->usb2_attach_irq_status = stat;
	somc_bcext_set_prop(bcext_dev, BATTMNGR_SOMC_PROP_USB2_ATTACH_INT,
								(u32)stat);
	return IRQ_HANDLED;
}
					...
					unsigned long event, void *data)
{
	...
	if (strcmp(psy->desc->name, "battery") == 0) {
		...
		rc = power_supply_get_property(psy, POWER_SUPPLY_PROP_ONLINE,
									&prop);
		if (rc)
			bcext_err("Couldn't get online (rc=%d)\n", rc);
		else
				...
				void *data, int fcc_ma, const char *client)
{
	...
	somc_bcext_set_prop(bcext_dev, BATTMNGR_SOMC_PROP_FCC_VOTE,
								(u32)fcc_ma);
					...
					struct somc_bcext_dev *bcext_dev)
{
	...
	if (bcext_dev->glink_client) {
		...
		if (rc < 0)
			bcext_err("Error unregistering from pmic_glink (rc=%d)\n",
									rc);
...
static int somc_bcext_parse_dt(struct somc_bcext_dev *bcext_dev)
{
	...
	if (of_find_property(node, "somc,thermal-fcc-ma", &byte_len) &&
		...
		if (rc < 0) {
			bcext_err("Couldn't read thermal-fcc-ma rc = %d\n", rc);
			return rc;
		}
	...
	if (of_find_property(node, "somc,thermal-usb-icl-ma", &byte_len) &&
		...
		rc = of_property_read_u32_array(node,
				...
				therm_mitig_params, NUM_THERM_MITIG_STEPS);
		if (rc < 0) {
			bcext_err("Couldn't read thermal-usb-icl-ma (rc=%d)\n",
									rc);
			return rc;
		}
		for (i = 0; i < NUM_THERM_MITIG_STEPS; i++)
	...
	if (of_find_property(node, "somc,thermal-usb-pd-icl-ma", &byte_len) &&
		...
		rc = of_property_read_u32_array(node,
				...
				therm_mitig_params, NUM_THERM_MITIG_STEPS);
		if (rc < 0) {
			bcext_err("Couldn't read thermal-usb-pd-icl-ma (rc=%d)\n",
									rc);
			return rc;
		}
		for (i = 0; i < NUM_THERM_MITIG_STEPS; i++)
	...
	if (of_find_property(node, "somc,thermal-charging-indication",
		...
		rc = of_property_read_u32_array(node,
				...
				therm_mitig_params, NUM_THERM_MITIG_STEPS);
		if (rc < 0) {
			bcext_err("Couldn't read thermal-charging-indication (rc=%d)\n",
									rc);
			return rc;
		}
		for (i = 0; i < NUM_THERM_MITIG_STEPS; i++)
	...
	if (of_find_property(node, "somc,thermal-usb-cc-open", &byte_len) &&
		...
		rc = of_property_read_u32_array(node,
				...
				therm_mitig_params, NUM_THERM_MITIG_STEPS);
		if (rc < 0) {
			bcext_err("Couldn't read thermal-usb-cc-open (rc=%d)\n",
									rc);
			return rc;
		}
		for (i = 0; i < NUM_THERM_MITIG_STEPS; i++)
	...
	if (of_property_read_bool(node, "somc,usb2-feature-enabled"))
		bcext_dev->usb2_feature_enabled = true;

	bcext_dev->usb2_gpio_alert_irq = of_get_named_gpio(node,
						"somc,usb2-pd-alert-gpio", 0);
	bcext_dev->usb2_gpio_attach_irq = of_get_named_gpio(node,
						"somc,usb2-pd-attach-gpio", 0);
...
static int somc_bcext_probe(struct platform_device *pdev)
{
	...
	if (rc < 0) {
		bcext_err("Couldn't register psy notifier (rc=%d)\n", rc);
		goto error;
	}
	...
	if (IS_ERR(bcext_dev->glink_client)) {
		...
		if (rc != -EPROBE_DEFER)
			bcext_err("Error in registering with pmic_glink (rc=%d)\n",
									rc);
		goto error;
	}
	...
	bcext_dev->vcls = class_create("wlc_switch");
	if (IS_ERR(bcext_dev->vcls)) {
		rc = PTR_ERR(bcext_dev->vcls);
		bcext_err("Could not create class (rc=%d)\n", rc);
		goto error;
	}
	...
	if (IS_ERR(bcext_dev->vdev)) {
		bcext_err("Could not create device\n");
		rc = -ENODEV;
		goto error;
	}
	...
	bcext_dev->unplug_wakelock = wakeup_source_register(bcext_dev->dev,
						"unplug_wakelock");
	if (!bcext_dev->unplug_wakelock) {
		bcext_err("can't create wakeup source\n");
		rc = -ENOMEM;
		goto error;
	}
	...
	if (!bcext_dev->unplug_key) {
		bcext_err("can't allocate unplug virtual button\n");
		rc = -ENOMEM;
		goto error;
	}
	...
	if (rc) {
		bcext_err("can't register power (key=%d)\n", rc);
		rc = -ENOMEM;
		goto error;
	}
	...
	if (rc < 0) {
		bcext_err("Failed to create battery_class (rc=%d)\n", rc);
		goto error;
	}
	...
	if (gpio_is_valid(bcext_dev->wls_gpio_irq)) {
		...
		if (rc < 0) {
			bcext_err("can't request irq (rc=%d)\n", rc);
			goto error;
		}

		rc = gpio_direction_input(bcext_dev->wls_gpio_irq);
		if (rc < 0) {
			bcext_err("can't set input gpio (rc=%d)\n", rc);
			goto error;
		}
		...
		if (bcext_dev->wls_irq < 0) {
			...
			bcext_err("can't gpio_to_irq (rc=%d)\n", rc);
			goto error;
		}
		...
		if (rc) {
			bcext_err("can't devm_request_threaded_irq (rc=%d)\n",
									rc);
			goto error;
		}
		...
		bcext_err("wls_gpio_irq is not supported\n");
	}

	if (bcext_dev->usb2_feature_enabled &&
		gpio_is_valid(bcext_dev->usb2_gpio_alert_irq)) {
		rc = gpio_request(bcext_dev->usb2_gpio_alert_irq,
							"usb2_alert_irq");
		if (rc < 0) {
			bcext_err("can't request irq (rc=%d)\n", rc);
			goto error;
		...
		rc = gpio_direction_input(bcext_dev->usb2_gpio_alert_irq);
		if (rc < 0) {
			bcext_err("can't set input gpio (rc=%d)\n", rc);
			goto error;
		}

		bcext_dev->usb2_alert_irq =
				gpio_to_irq(bcext_dev->usb2_gpio_alert_irq);
		if (bcext_dev->usb2_alert_irq < 0) {
			rc = bcext_dev->usb2_alert_irq;
			bcext_err("can't gpio_to_irq (rc=%d)\n", rc);
			goto error;
		}
		...
		rc = devm_request_threaded_irq(bcext_dev->dev,
				bcext_dev->usb2_alert_irq,
				NULL, usb2_alert_irq_handler,
				IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING |
				IRQF_ONESHOT, "usb2_alert", bcext_dev);
		if (rc) {
			bcext_err("can't devm_request_threaded_irq (rc=%d)\n",
									rc);
			goto error;
		...
		enable_irq_wake(bcext_dev->usb2_alert_irq);

		usb2_alert_irq_handler(0, bcext_dev);
	}

	if (bcext_dev->usb2_feature_enabled &&
		gpio_is_valid(bcext_dev->usb2_gpio_attach_irq)) {
		rc = gpio_request(bcext_dev->usb2_gpio_attach_irq,
							"usb2_attach_irq");
		if (rc < 0) {
			bcext_err("can't request irq (rc=%d)\n", rc);
			goto error;
		...
		rc = gpio_direction_input(bcext_dev->usb2_gpio_attach_irq);
		if (rc < 0) {
			bcext_err("can't set input gpio (rc=%d)\n", rc);
			goto error;
		}

		bcext_dev->usb2_attach_irq =
				gpio_to_irq(bcext_dev->usb2_gpio_attach_irq);
		if (bcext_dev->usb2_attach_irq < 0) {
			rc = bcext_dev->usb2_attach_irq;
			bcext_err("can't gpio_to_irq (rc=%d)\n", rc);
			goto error;
		}
		...
		rc = devm_request_threaded_irq(bcext_dev->dev,
				bcext_dev->usb2_attach_irq,
				NULL, usb2_attach_irq_handler,
				IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING |
				IRQF_ONESHOT, "usb2_attach", bcext_dev);
		if (rc) {
			bcext_err("can't devm_request_threaded_irq (rc=%d)\n",
									rc);
			goto error;
		...
		enable_irq_wake(bcext_dev->usb2_attach_irq);

		usb2_attach_irq_handler(0, bcext_dev);
	}
