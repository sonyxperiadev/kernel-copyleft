...
static inline bool dsi_pll_10nm_is_hw_revision(struct dsi_pll_resource *rsc)
{
	return (rsc->pll_revision == DSI_PLL_10NM) ? true : false;
}
...
static void dsi_pll_config_slave(struct dsi_pll_resource *rsc)
{
	...
	if (!orsc) {
		DSI_PLL_DBG(rsc, "slave PLL unavilable, assuming standalone config\n");
		return;
	}
...
static void dsi_pll_setup_config(struct dsi_pll_10nm *pll, struct dsi_pll_resource *rsc)
{
...
static void dsi_pll_calc_dec_frac(struct dsi_pll_10nm *pll, struct dsi_pll_resource *rsc)
{
	struct dsi_pll_config *config = &pll->pll_configuration;
...
static void dsi_pll_calc_ssc(struct dsi_pll_10nm *pll, struct dsi_pll_resource *rsc)
{
	struct dsi_pll_config *config = &pll->pll_configuration;
	struct dsi_pll_regs *regs = &pll->reg_setup;
	u32 ssc_per;
...
static void dsi_pll_ssc_commit(struct dsi_pll_10nm *pll, struct dsi_pll_resource *rsc)
{
	...
	if (pll->pll_configuration.enable_ssc) {
		...
		DSI_PLL_REG_W(pll_base, PLL_SSC_DIV_ADJPER_LOW_1, regs->ssc_adjper_low);
		DSI_PLL_REG_W(pll_base, PLL_SSC_DIV_ADJPER_HIGH_1, regs->ssc_adjper_high);
		DSI_PLL_REG_W(pll_base, PLL_SSC_CONTROL, SSC_EN | regs->ssc_control);
	}
}

static void dsi_pll_config_hzindep_reg(struct dsi_pll_10nm *pll, struct dsi_pll_resource *rsc)
{
	void __iomem *pll_base = rsc->pll_base;
...
static void dsi_pll_commit(struct dsi_pll_10nm *pll, struct dsi_pll_resource *rsc)
{
	void __iomem *pll_base = rsc->pll_base;
...
static unsigned long dsi_pll_vco_recalc_rate(struct dsi_pll_resource *pll)
{
	...
	config = &pll_10nm->pll_configuration;
...
static int dsi_pll_10nm_set_byteclk_div(struct dsi_pll_resource *pll, bool commit)
{
	...
	if (bitclk_rate <= phy_rate_split) {
		table = pll_10nm_dphy_lb;
		table_size = ARRAY_SIZE(pll_10nm_dphy_lb);
	} else {
		table = pll_10nm_dphy_hb;
		table_size = ARRAY_SIZE(pll_10nm_dphy_hb);
	}
...
static int dsi_pll_10nm_set_pclk_div(struct dsi_pll_resource *pll, bool commit)
{
...
static int dsi_pll_10nm_vco_set_rate(struct dsi_pll_resource *pll_res)
{
...
static void dsi_pll_10nm_dynamic_refresh(struct dsi_pll_10nm *pll, struct dsi_pll_resource *rsc)
{
	u32 data;
...
static int dsi_pll_10nm_dynamic_clk_vco_set_rate(struct dsi_pll_resource *rsc)
{
	int rc;
	struct dsi_pll_10nm *pll;
	u32 rate;
	...
	dsi_pll_10nm_dynamic_refresh(pll, rsc);

	return 0;
}

static int dsi_pll_10nm_enable(struct dsi_pll_resource *rsc)
{
	int rc = 0;
...
static int dsi_pll_10nm_disable(struct dsi_pll_resource *rsc)
{
	int rc = 0;
...
int dsi_pll_10nm_configure(void *pll, bool commit)
{
	...
	rc = dsi_pll_10nm_set_byteclk_div(rsc, commit);

	if (commit) {
		rc = dsi_pll_10nm_set_pclk_div(rsc, commit);
		rc = dsi_pll_10nm_vco_set_rate(rsc);
	} else {
		rc = dsi_pll_10nm_dynamic_clk_vco_set_rate(rsc);
	}
...
int dsi_pll_10nm_toggle(void *pll, bool prepare)
{
	...
	if (prepare) {
		rc = dsi_pll_10nm_enable(pll_res);
		if (rc)
			DSI_PLL_ERR(pll_res, "enable failed: %d\n", rc);
	} else {
		rc = dsi_pll_10nm_disable(pll_res);
		if (rc)
