...
static struct msm_platform_core_capability core_data_tofino[] = {
	/* {type, value} */
	{ENC_CODECS, H264 | HEVC | HEIC},
	{DEC_CODECS, H264 | HEVC | VP9 | HEIC},
	{MAX_SESSION_COUNT, 16},
...
static struct msm_platform_inst_capability instance_data_tofino[] = {
	/* {cap, domain, codec,
	 *      min, max, step_or_mask, value,
	 *      v4l2_id,
	 *      hfi_id,
	...
	/* ((4096 * 2176) / 256) * 60 fps */
	{MBPS, DEC, CODECS_ALL, 64, 2088960, 1, 2088960},
	/* ((1920x1088)/256)@60fps */
	{POWER_SAVE_MBPS, ENC, CODECS_ALL, 0, 489600, 1, 489600},
...
static int msm_vidc_init_data(struct msm_vidc_core *core)
{
	...
	/* default data is waipio data */
	core->platform->data = waipio_data;
	platform_data = &core->platform->data;

	/* Update fuse for ukee. If ukee, core and instance data would be updated */
	if (of_device_is_compatible(core->pdev->dev.of_node, "qcom,msm-vidc-ukee")) {
		platform_data->efuse_data = efuse_data_tofino;
		platform_data->efuse_data_size = ARRAY_SIZE(efuse_data_tofino);
	}
	...
	if (platform_data->sku_version == SKU_VERSION_1) {
		platform_data->core_data = core_data_tofino;
		platform_data->core_data_size =
			ARRAY_SIZE(core_data_tofino);
		platform_data->instance_data = instance_data_tofino;
		platform_data->instance_data_size =
			ARRAY_SIZE(instance_data_tofino);
