...
static int fts_input_report_touch_pv2(struct fts_ts_data *ts_data, u8 *touch_buf)
{
    ...
    for (i = 0; i < event_num; i++) {
        ...
        events[i].p = touch_buf[FTS_TOUCH_OFF_PRESSURE + base]; //0x3F;
#if FTS_REPORT_PRESSURE_EN
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    pdata->avdd_gpio = of_get_named_gpio(np, "focaltech,avdd-en-gpio",
                        0);
...
static void fts_notifier_callback_work(struct work_struct *work)
{
    ...
    if (active_panel) {
        fts_register_for_panel_events(ts_data->dev->of_node, ts_data);
    }
}
		 ...
		 struct panel_event_notification *notification, void *client_data)
{
	...
	case DRM_PANEL_EVENT_FPS_CHANGE:
		FTS_DEBUG("Received fps change old fps:%d new fps:%d\n",
...
int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    if (ret) {
        FTS_ERROR("unkowned information, unregister driver");
        goto err_power_init;
    }
