...
*
* File Name: Focaltech_test_ft5572.c
*
    ...
    struct fts_test *tdata, int *res, u8 *ab_ch, bool *result)
{
    ...
    int max_cc = FTS_MAX_SHORT_ADC;
    int ch_num = tdata->sc_node.tx_num + tdata->sc_node.rx_num + 1;
    ...
    *result = true;
    min_cc = fts_abs((22 * min_cc - 1000) * 2047 / (6400 + 32 * min_cc));
    for (i = 0; i < ch_num; i++) {
        if ((res[i] >= max_cc) || (res[i] < min_cc)) {
            ab_ch_num++;
            ab_ch[ab_ch_num] = i;
            *result = false;
        }
    ...
    if (ab_ch_num) {
        ...
        ab_ch[0] = ab_ch_num;
        FTS_TEST_DBG("[FTS_TS]ab_ch(total=%d):", ab_ch[0]);
...
static int get_cb_ft5572(int *cb_buf, int byte_num, bool is_cf)
{
    int ret = 0;
    int i = 0;
    u8 cb[SC_NUM_MAX] = { 0 };
    ...
    fts_test_write_reg(DEVIDE_MODE_ADDR, 0x40);
    ...
    for (i = 0; i < byte_num; i = i + 2) {
        cb_buf[i >> 1] = (int)(u16)((cb[i] << 8) + cb[i + 1]);
    }
...
static int scap_cb_ccbypass(struct fts_test *tdata, int *scap_cb, bool *result)
{
    ...
    if (thr->basic.scap_cb_wp_on_check && fw_wp_check) {
        ...
        ret = get_cb_ft5572(scb_tmp, byte_num, false);
        if (ret < 0) {
 ...
 */
static int start_scan_ft5572(int frame_num)
{
    int ret = 0;
    u8 addr = 0;
    u8 val = 0;
...
static int get_noise_ft5572(struct fts_test *tdata, int *noise_data, u8 fre)
{
    int ret = 0;
    int byte_num = 0;
    ...
    ret = start_scan_ft5572(tdata->ic.mc_sc.thr.basic.noise_framenum);
    if (ret < 0) {
...
static int read_rawdata_ft5572(
...
static int get_rawdata_mc_sc_ft5572(enum wp_type wp, int *data)
{
    int ret = 0;
    u8 val = 0;
    u8 addr = 0;
    ...
    ret = read_rawdata_ft5572(tdata, addr, val, rawdata_addr, byte_num, data);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("read rawdata fail\n");
        return ret;
    }
...
static int get_null_noise(struct fts_test *tdata)
{
    ...
    ret = fts_test_read_reg(0x5A, &val);
    if (ret) {
        FTS_TEST_SAVE_ERR("read 0x5A fail,ret=%d\n", ret);
        return ret;
    }

    FTS_TEST_INFO("value of 0x5A is %d", val);
    if (val == 0) {
        FTS_TEST_SAVE_INFO("0x5a=0, not support null noise");
        return 0;
    }
...
static int ft5572_rawdata_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    int i = 0;
    int j = 0;
    ...
    if (ret) {
        FTS_TEST_SAVE_ERR("set 0x5B fail,ret=%d\n", ret);
        goto restore_reg;
    }
...
restore_reg:
    ...
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("restore reg5B fail,ret=%d\n", ret);
    }
...
static int ft5572_uniformity_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    int row = 0;
    int col = 1;
...
static int ft5572_scap_cb_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    u8 sc_mode = 0;
    bool tmp_result = false;
...
static int ft5572_scap_rawdata_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    int i = 0;
    bool tmp_result = false;
    ...
    if (thr->basic.scap_rawdata_wp_on_check && fw_wp_check) {
        ...
        ret = get_rawdata_mc_sc_ft5572(WATER_PROOF_ON, srawdata_tmp);
        if (ret < 0) {
    ...
    if (thr->basic.scap_rawdata_wp_off_check && fw_wp_check) {
        ...
        srawdata_tmp = scap_rawdata + srawdata_cnt;
        ret = get_rawdata_mc_sc_ft5572(WATER_PROOF_OFF, srawdata_tmp);
        if (ret < 0) {
...
static int ft5572_short_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    int ch_num = 0;
...
static int ft5572_panel_differ_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    bool tmp_result = false;
    int i = 0;
...
restore_reg:
    /* set the origin value */
    ret = fts_test_write_reg(FACTORY_REG_DATA_TYPE, 0x00);
    if (ret < 0) {
...
static int ft5572_noise_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    bool tmp_result = false;
    int i = 0;
    ...
    ret = get_noise_ft5572(tdata, &noise[0], 0x00);
    if (ret < 0) {
...
static int start_test_ft5572(void)
{
    int ret = 0;
    struct fts_test *tdata = fts_ftest;
    ...
    if (true == test_item->noise_test) {
        ret = ft5572_noise_test(tdata, &temp_result);
    ...
    if (true == test_item->rawdata_test) {
        ret = ft5572_rawdata_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
        }
    }
    ...
    if (true == test_item->rawdata_uniformity_test) {
        ret = ft5572_uniformity_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
        }
    }
    ...
    if (true == test_item->panel_differ_test) {
        ret = ft5572_panel_differ_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
        }
    }
    ...
    if (true == test_item->scap_rawdata_test) {
        ret = ft5572_scap_rawdata_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
        }
    }
    ...
    if (true == test_item->scap_cb_test) {
        ret = ft5572_scap_cb_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
        }
    }
    ...
    if (true == test_item->short_test) {
        ret = ft5572_short_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
        }
    }
...
static int param_init_ft5572(void)
{
    struct mc_sc_threshold *thr = &fts_ftest->ic.mc_sc.thr;
...
static void save_data_ft5572(char *buf, int *data_length)
{
    struct fts_test *tdata = fts_ftest;
    struct mc_sc_testitem *test_item = &tdata->ic.mc_sc.u.item;
    struct mc_sc_threshold *thr = &fts_ftest->ic.mc_sc.thr;
...
struct test_funcs test_func_ft5572 = {
    .ctype = {0x93},
    .hwtype = IC_HW_MC_SC,
    .key_num_total = 0,
    .mc_sc_short_v2 = true,
    ...
    .param_init = param_init_ft5572,
    .start_test = start_test_ft5572,
    .save_data_private = save_data_ft5572,
};
