...
int fts_gesture_suspend(struct fts_ts_data *ts_data)
{
    ...
    fts_write_reg(FTS_REG_RW, FTS_DOZE_30HZ);
    fts_msleep(1);
    fts_read_reg(FTS_REG_RW, &reg);
    FTS_INFO("gesture suspend read 0x89 reg:%x",reg);
...
int fts_gesture_resume(struct fts_ts_data *ts_data)
{
    ...
    fts_write_reg(FTS_REG_RW, FTS_DOZE_60HZ);
    fts_msleep(1);
    fts_read_reg(FTS_REG_RW, &reg);
    FTS_INFO("gesture resume read 0x89 reg:%x",reg);
