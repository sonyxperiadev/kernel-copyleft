...
/**
  ...
  */
int getFWdata(struct fts_ts_info *info, const char *pathToFile, u8 **data, int *size)
{
...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
int readFwFile(struct fts_ts_info *info, const char *path, Firmware *fw, int keep_cx)
{
	int res;
	int orig_size;
	u8 *orig_data = NULL;
...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
int flashProcedure(struct fts_ts_info *info, const char *path, int force, int keep_cx)
{
	Firmware fw;
	int res;
...
{
	u8 cmd_reg = remap_reg(FTS_CMD_HW_REG_R, getClient(info)->bus_type);
...
int hold_m3(struct fts_ts_info *info)
{
	...
	if (getClient(info)->bus_type == BUS_SPI) {
		...
		cmd[0] = SPI4_MASK;
		ret = fts_writeU8UX(info, FTS_CMD_HW_REG_W, ADDR_SIZE_HW_REG, ADDR_ICR, cmd,
				    1);
		if (ret < OK) {
...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
int parseBinFile(struct fts_ts_info *info, u8 *fw_data, int fw_size, Firmware *fwData, int keep_cx)
{
...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
int flash_erase_page_by_page(struct fts_ts_info *info, ErasePage keep_cx, Firmware *fw)
{
	u8 status, i = 0;
...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
int flash_burn(struct fts_ts_info *info, Firmware fw, int force_burn, int keep_cx)
{
	int res;
...
start:
	...
	if (keep_cx > 0) {
		if (fw.sec2_size != 0 && force_burn == CRC_CX)
			...
			res = flash_erase_page_by_page(info, SKIP_PANEL_CX_INIT, &fw);
	} else {
		res = flash_erase_page_by_page(info, SKIP_PANEL_INIT, &fw);
		if (fw.sec2_size == 0)
			logError(1,
