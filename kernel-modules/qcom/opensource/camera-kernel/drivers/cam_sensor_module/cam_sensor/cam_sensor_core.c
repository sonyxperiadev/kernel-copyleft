...
{
	CAM_ERR(CAM_SENSOR, "cam_sensor_write_hwinfo cell_index = %d", cell_index);
	switch(cell_index){
		case 0:
			get_hardware_info_data(HWID_MAIN_CAM, "IMX766 TXD Main");
			CAM_INFO(CAM_SENSOR,"hwinfo cell_index =%d, IMX766 main", cell_index);
			break;
		case 1:
			get_hardware_info_data(HWID_SUB_CAM, "SC820CS TXD Front");
			CAM_INFO(CAM_SENSOR,"hwinfo cell_index =%d, SC820CS front", cell_index);
			break;
		case 2:
			get_hardware_info_data(HWID_MAIN_CAM_2, "SC1320CS txd Wide");
			CAM_INFO(CAM_SENSOR,"hwinfo cell_index =%d, SC1320CS wide", cell_index);
		}
}
...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (chipid == 0x0766) {
		...
		if (0 != strcmp(sensor_module_name, s_ctrl->sensor_name)) {
