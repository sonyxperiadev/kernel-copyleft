...
void i2c_block_write_reg(uint32_t reg_addr, unsigned short *data, uint32_t size)
{
	...
	while (base < size) {
		...
		cam_cci_i2c_write_continuous_table(&dw9784_o_ctrl.io_master_info, &dw9784_i2c_setting, CAM_SENSOR_I2C_WRITE_BURST);
		base += block_size;
	}
}
...
int dw9784_download_open_camera(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (dw9784_fw_ver_chk() < DW9784_FW_VERSION ) {
		g_updateFw = 1;
		logi("[dw9784_download_open_camera] updating fw to the newer version firmware: 0x%04X from fw.h", DW9784_FW_VERSION);
	} else {
		logi("[dw9784_download_open_camera] no need updating fw from fw.h, firmware version: 0x%04X is latest", DW9784_FW_VERSION);
	}
...
int dw9784_ois_status(struct cam_ois_ctrl_t *o_ctrl){
	...
	case 0x1000:
		...
		break;
	case 0x1001:
		logi("[dw9784_ois_status] servo on");
		break;
	case 0x1002:
...
void dw9784_dump(struct cam_ois_ctrl_t *o_ctrl)
{
	unsigned short qtimer_contrl;
	unsigned short cur_af_position;
	unsigned short time1, time2, time3, time4;

	read_reg_16bit_value_16bit(0x70DD, &qtimer_contrl);
	read_reg_16bit_value_16bit(0x7070, &cur_af_position);

	read_reg_16bit_value_16bit(0x70DE, &time1);
	read_reg_16bit_value_16bit(0x70DF, &time2);
	read_reg_16bit_value_16bit(0x70E0, &time3);
	read_reg_16bit_value_16bit(0x70E1, &time4);
	logi("[dw9784_ois_status] control[0x70DD]:0x%x, Qtime:[0x70DE]=0x%x [0x70DF]=0x%x [0x70E0]=0x%x [0x70E1]=0x0x%x cur_af_position=0x0x%x",
		qtimer_contrl, time1, time2, time3, time4, cur_af_position);
}
...
int REF_STROKE = 120;
int REF_GYRO_RESULT = 1000;

#define X_AXIS							0
#define Y_AXIS							1
#define GET_POSITION_COUNT (3)

static signed short sin_half(unsigned short deg){
    //long long x = deg * 31415 / 180;
	//long long ret = (160000*x*(31415-x))/(4934802200-4*x*(31415-x));
    long long x = deg * 314 / 180;
	long long ret = (1600*x*(314-x))/(493480-4*x*(314-x));
    return ret;
}
signed short my_sin(unsigned short deg){
    if(deg > 360)
        deg %= 360;
    if(deg > 180){
        return -1 * sin_half(deg - 180);
    }else{
        return sin_half(deg);
    }
}
signed short my_cos(unsigned short deg){
    unsigned short d = deg + 90;
    return my_sin(d);
}

int circle_motion_test(struct cam_ois_ctrl_t *o_ctrl, unsigned short RADIUS, unsigned short ACCURACY, unsigned short DEG_STEP, unsigned short WAIT_TIME0, unsigned short WAIT_TIME1, unsigned short WAIT_TIME2)
{
	int REF_POSITION_RADIUS[2];
	...
	signed short hallx, hally;

#if 0
	int ret = 0;
    set_ois_init_setting();
    if(!init_success){
        logi("[dw9784_motion_test], init_setting fail");
        ret = ERROR_ALLOC;
        goto err_alloc;
    }
#endif
	//dw9784_o_ctrl = o_ctrl;
	memcpy(&dw9784_o_ctrl, o_ctrl, sizeof(struct cam_ois_ctrl_t));

	if(FUNC_PASS != dw9784_servo_on()){
		logi("[dw9784_motion_test]servo on failed");
		return FUNC_FAIL;
	}

	read_reg_16bit_value_16bit(0x7210, &lens_ofst_x);		//Lens offset
	read_reg_16bit_value_16bit(0x7280, &lens_ofst_y);
	logi("[dw9784_motion_test]Lens ofst X = 0x%04X -- Lens ofst Y = 0x%04X", lens_ofst_x, lens_ofst_y);

	read_reg_16bit_value_16bit(0x7182, &GYRO_GAIN[X_AXIS]);	//Read Gyro gain
	read_reg_16bit_value_16bit(0x7183, &GYRO_GAIN[Y_AXIS]);
	logi("[dw9784_motion_test]Gyro Gain X = 0x%04X -- Gyro Gain Y = 0x%04X", GYRO_GAIN[X_AXIS], GYRO_GAIN[Y_AXIS]);

	/*gyro gain imx486 above 7000*/
	REF_POSITION_RADIUS[X_AXIS] = (REF_GYRO_RESULT * GYRO_GAIN[X_AXIS]) / 8192; //[code/deg]
	REF_POSITION_RADIUS[Y_AXIS] = (REF_GYRO_RESULT * GYRO_GAIN[Y_AXIS]) / 8192;

	logi("[dw9784_motion_test]REF_POSITION_RADIUS_X = %d -- REF_POSITION_RADIUS_Y = %d", REF_POSITION_RADIUS[X_AXIS], REF_POSITION_RADIUS[Y_AXIS]);
	...
	logi("[dw9784_motion_test]ADC_PER_MIC_X = %06d[code/um] -- ADC_PER_MIC_Y = %06d[code/um]", ADC_PER_MIC[X_AXIS], ADC_PER_MIC[Y_AXIS]);

	POSITION_ACCURACY[X_AXIS] = (signed short)(ADC_PER_MIC[X_AXIS] * ACCURACY);
	POSITION_ACCURACY[Y_AXIS] = (signed short)(ADC_PER_MIC[Y_AXIS] * ACCURACY);

	logi("[dw9784_motion_test]POSITION_ACCURACYX = %d[code] -- POSITION_ACCURACY = %d[code] -- ACCURACY : %d[um]", POSITION_ACCURACY[X_AXIS], POSITION_ACCURACY[Y_AXIS], ACCURACY);

	RADIUS_GYRO_RESULT = (signed short)((REF_GYRO_RESULT * RADIUS) / REF_STROKE); //[deg/um]

	logi("[dw9784_motion_test]RADIUS_GYRO_RESULT = %06d", RADIUS_GYRO_RESULT);
	...
	logi("[dw9784_motion_test]TARGET_RADIUS_X = %06d[code] -- _Y = %06d[code]", TARGET_RADIUS[X_AXIS], TARGET_RADIUS[Y_AXIS]);

	deg = 0;	//1st position
	TARGET_POSITION[X_AXIS] = TARGET_RADIUS[X_AXIS] * my_cos(deg) / 100;
	TARGET_POSITION[Y_AXIS] = TARGET_RADIUS[Y_AXIS] * my_sin(deg) / 100;

	write_reg_16bit_value_16bit(0x7100, TARGET_POSITION[X_AXIS]);		//Target position
	write_reg_16bit_value_16bit(0x7130, TARGET_POSITION[Y_AXIS]);

	mdelay(WAIT_TIME0);	//Move to 1st position
	circle_cnt = 0;
	ng_points = 0;

	for (deg = 1; deg <= 360; deg += DEG_STEP)
	{
		int x = TARGET_RADIUS[X_AXIS] * my_cos(deg) / 100;
		int y = TARGET_RADIUS[Y_AXIS] * my_sin(deg) / 100;
		TARGET_POSITION[X_AXIS] = (signed short)x;
		TARGET_POSITION[Y_AXIS] = (signed short)y;
		logi("[dw9784_motion_test] deg: %hu\tcalculated position x: %hd\tposition y: %hd", deg, (signed short)x, (signed short)y);

		write_reg_16bit_value_16bit(0x7100, TARGET_POSITION[X_AXIS]);
		write_reg_16bit_value_16bit(0x7130, TARGET_POSITION[Y_AXIS]);
		mdelay(WAIT_TIME1);		//Delay for each step
		...
		//read hall position three times and get average value
		for (get = 0; get < GET_POSITION_COUNT; get ++)
		{
			read_reg_16bit_value_16bit(0x7101, &MEASURE_POSITION[X_AXIS]);	//Hall A/D code
			read_reg_16bit_value_16bit(0x7131, &MEASURE_POSITION[Y_AXIS]);
			mdelay(WAIT_TIME2);	//Delay for each Hall Read
			hallx += MEASURE_POSITION[X_AXIS];
			hally += MEASURE_POSITION[Y_AXIS];
		}
		logi("[dw9784_motion_test] deg: %hu\t    actual position X: %hd\tposition Y: %hd",deg, hallx/GET_POSITION_COUNT, hally/GET_POSITION_COUNT);
		CAM_DBG(CAM_OIS, "[dw9784_motion_test] abs calulated positon x %hu, position y %hu", abs(TARGET_POSITION[X_AXIS]), abs(TARGET_POSITION[Y_AXIS]));
		CAM_DBG(CAM_OIS, "[dw9784_motion_test] abs actual    positon x %hu, position y %hu", abs(hallx/GET_POSITION_COUNT), abs(hally/GET_POSITION_COUNT));
		DIFF[X_AXIS] = abs(abs(TARGET_POSITION[X_AXIS]) - abs(hallx/GET_POSITION_COUNT));
		DIFF[Y_AXIS] = abs(abs(TARGET_POSITION[Y_AXIS]) - abs(hally/GET_POSITION_COUNT));
		CAM_DBG(CAM_OIS, "[dw9784_motion_test] diff x %hu, diff y %hu", DIFF[X_AXIS], DIFF[Y_AXIS]);

		circle_adc[0][circle_cnt] = TARGET_POSITION[X_AXIS];
		circle_adc[1][circle_cnt] = (signed short)hallx/GET_POSITION_COUNT;
		circle_adc[2][circle_cnt] = TARGET_POSITION[Y_AXIS];
		circle_adc[3][circle_cnt] = (signed short)hally/GET_POSITION_COUNT;	
		circle_cnt++;

		// Calculate total points that over Spec
		if ((DIFF[X_AXIS] > POSITION_ACCURACY[X_AXIS]) ||  (DIFF[Y_AXIS] > POSITION_ACCURACY[Y_AXIS]))
		{
			logi("[dw9784_motion_test]  failed at degree %hu, x_diff: %hu, y_diff: %hu",deg, DIFF[X_AXIS], DIFF[Y_AXIS]);
			ng_points ++;
		}

	}

	write_reg_16bit_value_16bit(0x7100, 0);
	write_reg_16bit_value_16bit(0x7130, 0);

	logi("[dw9784_motion_test]NG points = %d", ng_points );
