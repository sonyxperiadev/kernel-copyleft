...
/* Decode and log protocol headers */
#define ATH_PKTLOG_PROTO            0x00000001
/* Triggered stop as seeing TCP SACK packets */
#define ATH_PKTLOG_TRIGGER_SACK     0x00000002
/* Triggered stop as throughput drops below a threshold */
#define ATH_PKTLOG_TRIGGER_THRUPUT  0x00000004
/* Triggered stop as PER goes above a threshold */
#define ATH_PKTLOG_TRIGGER_PER      0x00000008
/* Triggered stop as # of phyerrs goes above a threshold */
#define ATH_PKTLOG_TRIGGER_PHYERR   0x00000010
...
void usage()
{
	fprintf(stderr,
	    ...
	    "usage: pktlogconf [-a adapter] [-e[event-list]] [-d]"
			" [-s log-size] [-t -k -l]\n"
	    "                  [-b -p -i] \n"
	    "    -h    show this usage\n"
	    ...
	    "          event-list is an optional comma separated"
			" list of one or more\n"
	    "          of the following: rx tx rcf rcu ani  "
			"(eg., pktlogconf -erx,rcu,tx)\n"
	    "    -s    change the size of log-buffer to \"log-size\" bytes\n"
	    "    -t    enable logging of TCP headers\n"
	    "    -k    enable triggered stop by a threshold"
			" number of TCP SACK packets\n"
	    "    -l    change the number of packets to log"
			" after triggered stop\n"
...
int main(int argc, char *argv[])
{
	...
	int thruput_thresh = -1, per_thresh = -1, phyerr_thresh = -1;
	int trigger_interval = -1;
	...
	for (;;) {
		...
		/* triggered stop after # of TCP SACK packets are seen */
		if (!optarg)
			break;
		...
		case 'd':
			...
			opt_d = 1;
			break;
		/* triggered stop after throughput drop below this threshold */
		case 'b':
			options |= ATH_PKTLOG_TRIGGER_THRUPUT;
			thruput_thresh = atoi(optarg);
			break;
		/* triggered stop after PER increase over this threshold */
		case 'p':
			options |= ATH_PKTLOG_TRIGGER_PER;
			per_thresh = atoi(optarg);
			break;
		/* triggered stop after # of phyerrs are seen */
		case 'y':
			options |= ATH_PKTLOG_TRIGGER_PHYERR;
			phyerr_thresh = atoi(optarg);
			break;
		/* time period of counting trigger statistics */
		case 'i':
	...
	/*
	 * This protection is needed since system wide logging
	...
	if (opt_a) {
		...
		snprintf(sysctl_tail_length, sizeof(sysctl_tail_length),
		    "/proc/sys/" PKTLOG_PROC_DIR
		    "/%s/tail_length", ad_name);
		snprintf(sysctl_thruput_thresh, sizeof(sysctl_thruput_thresh),
		    "/proc/sys/" PKTLOG_PROC_DIR
		    "/%s/thruput_thresh", ad_name);
		snprintf(sysctl_per_thresh, sizeof(sysctl_per_thresh),
		    "/proc/sys/" PKTLOG_PROC_DIR
		    "/%s/per_thresh", ad_name);
		snprintf(sysctl_phyerr_thresh, sizeof(sysctl_phyerr_thresh),
		    "/proc/sys/" PKTLOG_PROC_DIR
		    "/%s/phyerr_thresh", ad_name);
		snprintf(sysctl_trigger_interval,
		    sizeof(sysctl_trigger_interval),
		    "/proc/sys/" PKTLOG_PROC_DIR
		    "/%s/trigger_interval", ad_name);
	} else {
		snprintf(sysctl_enable, sizeof(sysctl_enable),
		    "/proc/sys/" PKTLOG_PROC_DIR
		    "/" PKTLOG_PROC_SYSTEM "/enable");
		snprintf(sysctl_size, sizeof(sysctl_size),
		    "/proc/sys/" PKTLOG_PROC_DIR
		    "/" PKTLOG_PROC_SYSTEM "/size");
		snprintf(sysctl_options, sizeof(sysctl_options),
		    "/proc/sys/" PKTLOG_PROC_DIR
		    "/" PKTLOG_PROC_SYSTEM "/options");
		snprintf(sysctl_sack_thr, sizeof(sysctl_sack_thr),
		    "/proc/sys/" PKTLOG_PROC_DIR
		    "/" PKTLOG_PROC_SYSTEM "/sack_thr");
		snprintf(sysctl_tail_length, sizeof(sysctl_tail_length),
		    "/proc/sys/" PKTLOG_PROC_DIR
		    "/" PKTLOG_PROC_SYSTEM "/tail_length");
		snprintf(sysctl_thruput_thresh, sizeof(sysctl_thruput_thresh),
		    "/proc/sys/" PKTLOG_PROC_DIR
		    "/" PKTLOG_PROC_SYSTEM "/thruput_thresh");
		snprintf(sysctl_per_thresh, sizeof(sysctl_per_thresh),
		    "/proc/sys/" PKTLOG_PROC_DIR
		    "/" PKTLOG_PROC_SYSTEM "/per_thresh");
		snprintf(sysctl_phyerr_thresh, sizeof(sysctl_phyerr_thresh),
		    "/proc/sys/" PKTLOG_PROC_DIR
		    "/" PKTLOG_PROC_SYSTEM "/phyerr_thresh");
		snprintf(sysctl_trigger_interval,
		    sizeof(sysctl_trigger_interval),
		    "/proc/sys/" PKTLOG_PROC_DIR
	...
	if (sack_thr > 0) {
		if (options & ATH_PKTLOG_PROTO) {
			if (pktlog_size(sysctl_sack_thr,
			    sysctl_enable, sack_thr) != 0) {
				fprintf(stderr,
				    "pktlogconf: log sack_thr"
				    " setting failed\n");
				exit(-1);
			}
			if (pktlog_size(sysctl_tail_length,
			    sysctl_enable, tail_length) != 0) {
				fprintf(stderr, "pktlogconf: log tail_length"
						" setting failed\n");
				exit(-1);
			}
		} else {
	...
	if (thruput_thresh > 0) {
		if (pktlog_size(sysctl_thruput_thresh,
		    sysctl_enable, thruput_thresh) != 0) {
			fprintf(stderr, "pktlogconf: log thruput_thresh"
					" setting failed\n");
			exit(-1);
		}
	}
	if (per_thresh > 0) {
		if (pktlog_size(sysctl_per_thresh,
		    sysctl_enable, per_thresh) != 0) {
			fprintf(stderr, "pktlogconf: log per_thresh"
					" setting failed\n");
			exit(-1);
		}
	}
	if (phyerr_thresh > 0) {
		if (pktlog_size(sysctl_phyerr_thresh,
		    sysctl_enable, phyerr_thresh) != 0) {
			fprintf(stderr, "pktlogconf: log phyerr_thresh"
					" setting failed\n");
			exit(-1);
		}
	}
	if (trigger_interval > 0) {
		if (pktlog_size(sysctl_trigger_interval,
		    sysctl_enable, trigger_interval) != 0) {
			fprintf(stderr, "pktlogconf: log trigger_interval"
					" setting failed\n");
			exit(-1);
		}
	}
	...
	if (size >= 0)
		if (pktlog_size(sysctl_size,
		    sysctl_enable, size) != 0) {
			fprintf(stderr, "pktlogconf: log size"
					" setting failed\n");
			exit(-1);
		}

	if (opt_e) {
		if (pktlog_enable(sysctl_enable,
		    filter) != 0) {
			fprintf(stderr, "pktlogconf: log filter"
					" setting failed\n");
			exit(-1);
		}
		if (pktlog_options(sysctl_options,
		    options) != 0) {
			fprintf(stderr, "pktlogconf: options"
					" setting failed\n");
			exit(-1);
		}
	}
