    ...
    uint64_t timestamp)
{
	...
	if (mod_print[moduleid] != NULL) {
		if (!(mod_print[moduleid](0, moduleid, vdevid,
        ...
        const char *fmt, ...)
{
	...
	if (vap_id < DBGLOG_MAX_VDEVID) {
		...
		j = snprintf(buf, sizeof(buf), "%s[%llu] vap-%u ", time,
			     timestamp, vap_id);
	} else {
		miliseconds_to_time(timestamp, time);
		j = snprintf(buf, sizeof(buf), "%s[%llu] ", time, timestamp);
	}
...
dbglog_default_print_handler(A_UINT32 mod_id, A_UINT16 vap_id, A_UINT32 dbg_id,
    A_UINT64 timestamp, A_UINT16 numargs, A_UINT32 *args)
{
	int i, j;
	char tempbuf[RECLEN];
	char time[MAX_TIME_LEN];
	...
	if (vap_id < DBGLOG_MAX_VDEVID) {
		miliseconds_to_time(timestamp, time);
		j = snprintf(tempbuf, sizeof(tempbuf), "%s[%llu] vap-%u %s ( ",
			     time, timestamp, vap_id,
			     dbglog_get_msg(mod_id, dbg_id));
	} else {
		miliseconds_to_time(timestamp, time);
		j = snprintf(tempbuf, sizeof(tempbuf), "%s[%llu] %s ( ", time,
			     timestamp, dbglog_get_msg(mod_id, dbg_id));
	}
...
dbglog_parse_debug_logs(u_int8_t *datap, u_int16_t len, u_int16_t dropped)
{
	...
	while ((count + 2) < length) {
		...
		if (mod_print[moduleid] == NULL) {
			...
			if(!(mod_print[moduleid](0, moduleid, vapid,
				debugid, timestamp,
				numargs,
				(((A_UINT32 *)buffer) + 2 + count)))) {
    ...
    A_UINT32 *args)
{
	...
	case PS_STA_PSPOLL_ALLOW:
		if (numargs == 3) {
			dbglog_printf(timestamp, vap_id, "STA PS-Poll %s "
							 "flags=%x time=%u",
    ...
    A_UINT32 *args)
{
	...
	case ANI_DBGID_MRC_CCK:
		dbglog_printf(timestamp, vap_id,"ANI mrc_cck %d",
		    args[0]);
		break;
	...
	case ANI_DBGID_POLL_PERIOD:
		dbglog_printf(timestamp, vap_id,"ANI Configure poll "
						"period to %d",
		    args[0]);
		break;
	case ANI_DBGID_LISTEN_PERIOD:
		dbglog_printf(timestamp, vap_id,"ANI Configure listen "
						"period to %d",
		    args[0]);
		break;
	case ANI_DBGID_OFDM_LEVEL_CFG:
		dbglog_printf(timestamp, vap_id,"ANI Configure ofdm "
						"level to %d",
		    args[0]);
		break;
	case ANI_DBGID_CCK_LEVEL_CFG:
		dbglog_printf(timestamp, vap_id,"ANI Configure cck level "
						"to %d",
		    args[0]);
		break;
	default:
    ...
    A_UINT32 * args)
{
	...
	case COEX_PSP_ERROR:
		if (numargs >= 1 && args[0] < 9) {
			...
			for (i = 1; i < numargs; i++) {
				j += snprintf((char *)(buf + j),
					      sizeof(buf) - j, ", %u", args[i]);
			}
			diag_print_legacy_logs((const char *)buf);
		} else {
			return FALSE;
