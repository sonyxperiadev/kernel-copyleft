...
/* args: argc==4 (rawTsfHi, rawTsfLo, nanTsfHi, nanTsfLo),
   argc==2(offsetHi, offsetLo) */
    ...
    A_UINT32 *args)
{
	...
	case NAN_DBGID_WMI_EVT_SENT:
		...
		dbglog_printf(timestamp, vap_id,
		    "%s, pEventPkt 0x%08X, pEventBuf 0x%08X,"
		    " event size %u, data size %u",
	...
	case NAN_DBGID_TLV_READ:
		...
		dbglog_printf(timestamp, vap_id,
		    "%s, type %s (%u), read length %u",
		    DBG_MSG_ARR[dbg_id], dbglog_get_nan_tlv_type_str(args[0]),
		    args[0], args[1]);
		break;
	...
	case NAN_DBGID_PUBSUB_MATCHED:
		...
		dbglog_printf(timestamp, vap_id,
		    ...
		    "svc id %02X:%02X:%02X:%02X:%02X:%02X, svcCtrl 0x%02X,"
		    " len %u",
		    PUBSUB_STR(args[0]), args[0],
	...
	case NAN_DBGID_SUBSCRIBE_UNMATCH_TIMEOUT_UPDATE:
		...
		dbglog_printf(timestamp, vap_id,
		    "%s, handle %u, match handle %u, old timeout %u,"
		    " new timeout %u",
		    DBG_MSG_ARR[dbg_id], args[0], args[1], args[2], args[3]);
		break;
	...
	case NAN_DBGID_SUBSCRIBE_MATCH_EXPIRED:
		...
		dbglog_printf(timestamp, vap_id,
		    "%s, handle %u, match handle %u, match timestamp %u,"
		    " timestamp %u",
		    DBG_MSG_ARR[dbg_id], args[0], args[1], args[2], args[3]);
		break;
	...
	case NAN_DBGID_CHREQ_REMOVE:
		...
		dbglog_printf(timestamp, vap_id, "%s: chan %u, interval %u,"
						 " status %d",
		    DBG_MSG_ARR[dbg_id], args[0], args[1], args[2]);
		break;
	...
	case NAN_DBGID_CHREQ_GRANT:
		...
		dbglog_printf(timestamp, vap_id,
		    "%s: chan %u, %s, tsfLo 0x%08X",
		    DBG_MSG_ARR[dbg_id], args[0],
		    dbglog_get_nan_chan_type(args[1]),
		    args[2]);
		break;
	...
	case NAN_DBGID_RX_CALLBACK:
		...
		dbglog_printf(timestamp, vap_id,
		    "%s: type 0x%X, length %u, timestamp 0x%08X, rssi %d,"
		    " freq %d",
		    DBG_MSG_ARR[dbg_id], args[0], args[1], args[2], args[3],
		    args[4]);
		break;
	...
	case NAN_DBGID_TX_COMPLETE:
		...
		dbglog_printf(timestamp, vap_id,
		    "%s: type 0x%X, handle %u, bufp %u, status %u,"
		    " timestamp 0x%08X",
		    DBG_MSG_ARR[dbg_id], args[0], args[1], args[2], args[3],
		    args[4]);
		break;
	...
	case NAN_DBGID_BEACON_RX:
		...
		if (numargs == 5) {
			...
			if (ctl == 0) {
				...
				dbglog_printf(timestamp, vap_id,
				    " - Times: NAN TSF 0x%08lx_%08lx, "
				    "hwRxTs 0x%08lx, statusFlags 0x%08lx",
				    args[1],
				    args[2],
				    args[3],
				    args[4]);
				...
				dbglog_printf(timestamp, vap_id,
				    " - Attrs: [mpref=%d randf=%d], "
				    "[amRank=0x%08lx_%08lx amHC=%d "
				    "amBTT=%08lx]",
				    args[0] & 0xFF,
				    (args[0] >> 8) & 0xFF,
				    args[1],
			...
			A_UINT16 i;
			for (i=0; i < numargs; ++i) {
				snprintf(&str[i*9], sizeof(&str[i*9]),
				    "%08X ", args[i]);
			}
	...
	case NAN_DBGID_CLUSTER_MERGE:
		...
		dbglog_printf(timestamp, vap_id,
		    "%s: **MERGING** to clusterId 0x%04x "
		    "[amRank=0x%08lx_%08lx, TS=%08lx_%08lx]",
		    DBG_MSG_ARR[dbg_id], args[0], args[1], args[2],
		    args[3], args[4]);
		break;
	...
	case NAN_DBGID_APPLY_BEACON_TSF:
		...
		dbglog_printf(timestamp, vap_id,
		    "%s: timestamp=0x%08X_%08X, age=%08X",
		    DBG_MSG_ARR[dbg_id], args[0], args[1], args[2]);
		break;
	...
	case NAN_DBGID_NEW_ANCHORMASTER:
		...
		}
		{
			...
			dbglog_printf(timestamp, vap_id,
			    ...
			    "rank is 0x%08X_%08x (mp=%d, rf=%d)",
			    DBG_MSG_ARR[dbg_id], mac[0], mac[1],
			    mac[2], mac[3], mac[4],
	...
	case NAN_DBGID_ANCHORMASTER_RECORD_EXPIRED:
		if (numargs == 4) {
			...
			dbglog_printf(timestamp, vap_id,
			    "%s: rank=0x%08X_%08x HC=%d "
			    "BTT=0x%08X ==> Setting HC to %d",
			    DBG_MSG_ARR[dbg_id], args[0], args[1],
	...
	case NAN_DBGID_SYNC_BEACON_DW_STATS:
		...
		}
		{
			...
			c1_close = args[1] >> 16,
				...
				dbglog_printf(timestamp, vap_id,
				    "%s: total rx %d, c1=(%d/%d/%d) "
				    "c1=(%d/%d/%d)",
				    DBG_MSG_ARR[dbg_id], args[0],
				    c1_close, c1_middle, c1_poor,
	...
	case NAN_DBGID_PUBSUB_MATCHED_SKIPPED_SSI:
		...
		dbglog_printf(timestamp, vap_id,
		    ...
		    "svc id %02X:%02X:%02X:%02X:%02X:%02X, "
		    "svcCtrl 0x%02X, len%u",
		    PUBSUB_STR(args[0]), args[0],
		    (args[2] & 0xFF), ((args[2] >> 8) && 0xFF),
		    ((args[1] >> 24) & 0xFF), ((args[1] >> 16) & 0xFF),
		    ((args[1] >> 8) & 0xFF), (args[1] & 0xFF),
	...
	case NAN_DBGID_TW_PARAMS:
		...
		dbglog_printf(timestamp, vap_id, "%s: twSize=%u, n=%u,"
						 "twIndex=%u",
		    DBG_MSG_ARR[dbg_id], args[0], args[1], args[2]);
		break;
	...
	case NAN_DBGID_BEACON_SENDER:
		if (numargs == 1) {
			dbglog_printf(timestamp, vap_id, "%s: failed, "
							 "reason %x",
			    DBG_MSG_ARR[dbg_id], args[0]);
		} else if (numargs == 3) {
			dbglog_printf(timestamp, vap_id,
			    "%s: %s idx=%u sender 0x%08x",
			    DBG_MSG_ARR[dbg_id], args[0] ? "alloc" : "reclaim",
			    args[1], args[2]);
		} else {
			dbglog_printf(timestamp, vap_id, "%s, missing arg",
			    DBG_MSG_ARR[dbg_id]);
	...
	default:
		dbglog_printf(timestamp, vap_id,
		    "NAN unknown event id %u (should never reach this case)",
		    dbg_id);
		return FALSE;
	}
    ...
    A_UINT32 *args)
{
	...
	for (arg_idx=1; arg_idx < numargs; ++arg_idx) {
		for (byte_idx=0, shift=24; byte_idx < 4;
		      ++byte_idx, shift -= 8) {
			if (byteCount < length) {
				byteCount++;
				snprintf(&str[STR_INDEX((arg_idx-1), byte_idx)],
				    sizeof(&str[STR_INDEX((arg_idx-1), byte_idx)]),
				    "%02X ",
    ...
    A_UINT32 *args)
{
	...
	case 8:
		CHECK_LENGTH(length, sizeof(A_UINT8));
		dbglog_printf(timestamp, vap_id, "    connMap: 0x%04X",
		    args[1]);
		// FIXME: decode this further.
		break;

	default:
		dbglog_nan_tlv_print_handler(vap_id, dbg_id, timestamp,
		    numargs, args);
		break;
	}
}
    ...
    A_UINT32 *args)
{
	...
	case 0:
		CHECK_LENGTH(length, sizeof(A_UINT16));
		dbglog_printf(timestamp, vap_id, "    matchHandle: %u",
		    args[1]);
		break;
    ...
    A_UINT32 *args)
{
	...
	case 9:
		...
		dbglog_printf(timestamp, vap_id,
		    ...
		    args[2], args[1],
		    (A_UINT32) (VAR64BIT(args[1], args[2])/1000000ull));
		break;
	...
	case 11:
		CHECK_LENGTH(length, sizeof(A_UINT64));
		dbglog_printf(timestamp, vap_id,
		    "    totalTimeSpentAsNonMasterSync: 0x%08lx_%08lx, "
		    "%lu seconds",
		    args[2], args[1],
		    (A_UINT32) (VAR64BIT(args[1], args[2])/1000000ull));
		break;
	...
	case 12:
		CHECK_LENGTH(length, sizeof(A_UINT64));
		dbglog_printf(timestamp, vap_id,
		    "    totalTimeSpentAsNonMasterNonSync: 0x%08lx_%08lx, "
		    "%lu seconds",
		    args[2], args[1],
		    (A_UINT32) (VAR64BIT(args[1], args[2])/1000000ull));
		break;
    ...
    A_UINT32 *args)
{
	...
	default:
		...
		case NAN_STATS_ID_DE_PUBLISH:
			dbglog_nan_publish_stats_print_handler(
			    param_idx, vap_id, dbg_id, timestamp,
			    numargs, args);
			break;
...
dbglog_nan_enable_req_msg_print_handler(
    ...
    A_UINT32 *args)
{
	dbglog_nan_tlv_print_handler(vap_id, dbg_id,
	    timestamp, numargs, args);
}
    ...
    A_UINT32 *args)
{
	...
	case 1:
		CHECK_LENGTH(length, sizeof(A_UINT16));
		dbglog_printf(timestamp, vap_id, "    value: %u",
		    args[1]);
		break;
    ...
    A_UINT32 *args)
{
	...
	case 1:
		CHECK_LENGTH(length, sizeof(A_BOOL));
		dbglog_printf(timestamp, vap_id, "    rising: %s",
		    BOOL_STR(args[1]));
		break;

	case 2:
		CHECK_LENGTH(length, sizeof(A_BOOL));
		dbglog_printf(timestamp, vap_id, "    falling: %s",
		    BOOL_STR(args[1]));
		break;

	case 3:
		CHECK_LENGTH(length, sizeof(A_BOOL));
		dbglog_printf(timestamp, vap_id, "    clear: %s",
		    BOOL_STR(args[1]));
		break;

	case 4:
		CHECK_LENGTH(length, sizeof(A_UINT32));
		dbglog_printf(timestamp, vap_id, "    threshold: %u",
		    args[1]);
		break;
