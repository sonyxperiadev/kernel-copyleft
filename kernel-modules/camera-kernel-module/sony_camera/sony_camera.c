...
struct sony_camera_data {
	...
	uint32_t							thermal_cnt;
	char								*thermal_name[SONY_CAMERA_MAX_TEMPERATURE_NUM_PER_CAMERA];
...
static struct mutex pmic_cs0_lock;
static struct mutex vio_lock;
	...
	uint32_t id)
{
	...
	if (rc < 0) {
		camera_data[id].match_id.enabled = false;
	} else {
		camera_data[id].match_id.enabled = true;
	...
	count = of_property_count_strings(of_node, "thermal_names");
	if (count < 0) {
		LOGE("read property failed rc %d", rc);
		rc = -EFAULT;
		goto fail;
	}
	camera_data[id].thermal_cnt = count;
	for (i = 0; i < count; i++) {
		rc = of_property_read_string_index(of_node, "thermal_names", i,
			(const char **)(&camera_data[id].thermal_name[i]));
		if (rc < 0) {
			LOGE("read property failed i=%d count=%d", i, count);
			goto fail;
		}
	...
	int size, uint8_t *buf, enum sony_camera_sub_device_type device_type)
{
	...
	if (addr_type == SONY_CAMERA_I2C_TYPE_INVALID ||
		addr_type >= SONY_CAMERA_I2C_TYPE_MAX) {
		LOGE("addr type was invalid %d", addr_type);
		rc = -EINVAL;
		goto end;
	}
	if (size > SONY_CAMERA_MAX_I2C_DATA) {
		LOGE("size was invalid %d", size);
		rc = -EINVAL;
		goto end;
	}
	...
	int size, uint8_t *buf, enum sony_camera_sub_device_type device_type)
{
	...
	if (addr_type == SONY_CAMERA_I2C_TYPE_INVALID ||
		addr_type >= SONY_CAMERA_I2C_TYPE_MAX) {
		LOGE("addr type was not valid, %d", addr_type);
		rc = -EINVAL;
		goto end;
	}
	if (size > SONY_CAMERA_I2C_MAX_LEN) {
		LOGE("write size was invalid, %d", size);
		rc = -EINVAL;
		goto end;
	}
	if (addr_type == SONY_CAMERA_I2C_TYPE_BYTE) {
	...
	uint16_t len, uint8_t *buf)
{
	...
	if (len > SONY_CAMERA_I2C_MAX_LEN) {
		LOGE("write i2c length invalid %d", len);
		rc = -EINVAL;
	} else {
	...
	struct thermal_zone_device *thermal, int *temp)
{
	...
	for (id = 0; id < g_sensor_num; id++) {
		if (camera_data[id].probe_done) {
			for (thermal_id = 0; thermal_id < camera_data[id].thermal_cnt; thermal_id++) {
				if (!strncmp(thermal->type, camera_data[id].thermal_name[thermal_id], sizeof(thermal->type))) {
					found = true;
					break;
				}
			}
	...
	unsigned int cmd, unsigned long parm, bool compat)
{
	...
	case SONY_CAMERA_CMD_I2C_READ:
		...
		if (data->state != SONY_CAMERA_STATE_POWER_UP) {
			mutex_unlock(&data->state_lock);
			LOGE("Already power down, can not read i2c, slave_addr 0x%x, addr 0x%x",
				setting.slave_addr, setting.addr);
			rc = -EINVAL;
			goto exit;
		}
		mutex_unlock(&data->state_lock);
	...
	case SONY_CAMERA_CMD_I2C_WRITE:
		...
		if (data->state != SONY_CAMERA_STATE_POWER_UP) {
			mutex_unlock(&data->state_lock);
			LOGE("Already power down, can not write i2c, slave_addr 0x%x, addr 0x%x",
				setting.slave_addr, setting.addr);
			rc = -EINVAL;
			goto exit;
		}
	...
	case SONY_CAMERA_CMD_SPI_READ:
	{
		...
		if (data->state != SONY_CAMERA_STATE_POWER_UP) {
			mutex_unlock(&data->state_lock);
			LOGE("Already power down, can not read spi");
			rc = -EINVAL;
			goto exit;
		}
		mutex_unlock(&data->state_lock);
	...
	case SONY_CAMERA_CMD_SPI_WRITE:
	{
		...
		if (data->state != SONY_CAMERA_STATE_POWER_UP) {
			mutex_unlock(&data->state_lock);
			LOGE("Already power down, can not write spi");
			rc = -EINVAL;
			goto exit;
		}
		mutex_unlock(&data->state_lock);
...
static void sony_camera_qup_i2c_subdev_remove(struct i2c_client *qup_client)
{
...
static void sony_camera_qup_spi_subdev_remove(struct spi_device *spi)
{
}
...
static int sony_camera_platform_probe(struct platform_device *p_dev)
{
	...
	if (camera_data[id].thremal_enable) {
		uint8_t n = 0;
		for (n = 0; n < camera_data[id].thermal_cnt; n++) {
			camera_data[id].thermal_zone_dev[n] =
				thermal_zone_device_register(camera_data[id].thermal_name[n],
				0, 0, 0, &sony_camera_thermal_ops, 0, 0, 0);
			if (IS_ERR(camera_data[id].thermal_zone_dev[n])) {
				LOGE("thermal_zone_device_register (%u)", id);
				rc = PTR_ERR(camera_data[id].thermal_zone_dev[n]);
				goto fail;
			}
		}
	}
...
static void __exit sony_camera_exit_module(void)
{
	...
	debugfs_remove_recursive(g_dbg_file_folder_ptr);
	g_dbg_file_folder_ptr = NULL;
	mutex_destroy(&pmic_cs0_lock);
	mutex_destroy(&vio_lock);
}
