...
static int hdmi_detect_probe(struct platform_device *pdev)
{
	...
	data->gpio = of_get_named_gpio(node, "interrupt-gpio", 0);
	if (data->gpio < 0) {
		dev_err(&pdev->dev, "%s: failed to get interrupt-gpio: %d\n", __func__, data->gpio);
		goto fail_gpio;
	}
	rc = of_property_read_u32(node, "interrupt-flags", &irq_flags);
	if (rc) {
		dev_err(&pdev->dev, "%s: failed to read interrupt-flags property: %d\n", __func__, rc);
		goto fail_gpio;
	}
