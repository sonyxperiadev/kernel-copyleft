...
#define FPC1145_RESET_US 3000
#define FPC1145_RESET_RANGE1_US 10
#define PWR_ON_STEP_SLEEP 100
#define PWR_ON_STEP_RANGE1 900
...
struct fpc1145_data {
	...
	int irq_gpio;
	int rst_gpio;
	struct mutex lock; /* Rule out running some functions simultaneously */
	bool prepared;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%15s,%c", name, &op) != NUM_PARAMS_REG_ENABLE_SET)
		return -EINVAL;
	if (op == 'e')
		enable = true;
	else if (op == 'd')
...
static int hw_reset(struct fpc1145_data *fpc1145)
{
	...
	if (rc)
		goto exit;
	usleep_range(FPC1145_RESET_US,
			FPC1145_RESET_US + FPC1145_RESET_RANGE1_US);
...
static const struct of_device_id fpc1145_of_match[] = {
