...
int dsi_panel_driver_gpio_request(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(spec_pdata->disp_vddr_en_gpio)) {
		rc = gpio_request(spec_pdata->disp_vddr_en_gpio,"disp_vddr_en_gpio");

		if (rc) {
			pr_err("request for disp_vddr_en_gpio failed, rc=%d\n", rc);
			goto error_release_disp_vddr_en;
		}
	}
...
error_release_disp_vddr_en:
	if (gpio_is_valid(spec_pdata->disp_vddr_en_gpio))
		gpio_free(spec_pdata->disp_vddr_en_gpio);
...
int dsi_panel_driver_post_power_off(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_driver_vddr_enable(panel, false);
	if(rc) {
...
int dsi_panel_driver_pre_power_on(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_driver_vddr_enable(panel, true);
	if(rc) {
		pr_err("%s: failed to enable vddr, rc=%d\n", __func__, rc);
	}
...
int dsi_panel_driver_parse_gpios(struct dsi_panel *panel)
{
	...
	spec_pdata->disp_vddr_en_gpio = of_get_named_gpio(panel->panel_of_node,
						"somc,disp-vddr-en-gpio", 0);
	if(!gpio_is_valid(spec_pdata->disp_vddr_en_gpio)) {
		pr_err("%s: failed get disp vddr enable gpio\n", __func__);
	}
...
int dsi_panel_driver_vddr_enable(struct dsi_panel *panel, bool en){
	...
	if(!gpio_is_valid(spec_pdata->disp_vddr_en_gpio)) {
		DSI_ERR("%s: disp_vddr_en_gpio is invalid\n", __func__);
		rc = -EINVAL;
		goto exit;
	}

	rc = gpio_direction_output(spec_pdata->disp_vddr_en_gpio, en);
	if (rc) {
		DSI_ERR(":%s: set vddr output direction failed, rc=%d\n", __func__, rc);
		goto exit;
	}

	gpio_set_value(spec_pdata->disp_vddr_en_gpio, en);
