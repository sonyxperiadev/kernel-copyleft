...
static int dsi_panel_get_board_id()
{
    ...
    strcpy(board_id, get_adc_kb());

    DSI_ERR("%s: board id = %s\n", __func__, board_id);

	if(!strcmp(board_id, "EVT0-1"))
		return 1;
	else if(!strcmp(board_id, "EVT0-2"))
		return 1;
	else if(!strcmp(board_id, "EVT"))
		return 1;
	else if(!strcmp(board_id, "DVT"))
		return 1;
	else
		return 0;
}
