...
static int last_logs_probe(struct platform_device *pdev)
{
	...
	last_logs_region region;
	struct device_node *node, *device_node;
	struct resource r;
	struct proc_dir_entry *dent_dir;
	struct platform_device *p_dev;
	int ret;

	device_node = of_find_compatible_node(NULL, NULL, "qcom,tz-log");
	if (!device_node) {
		dev_err(&pdev->dev, "Failed to get node for tz-log\n");
		goto last_logs;
	}
	p_dev = of_find_device_by_node(device_node);
	of_node_put(device_node);
	if (!p_dev) {
		dev_err(&pdev->dev, "device absent for node: %s\n", device_node->name);
		goto last_logs;
	}

	dent_dir = platform_get_drvdata(p_dev);
	if (!dent_dir) {
		dev_err(&pdev->dev, "deferred probe pending for %s\n", pdev->name);
		return -EPROBE_DEFER;
	}
