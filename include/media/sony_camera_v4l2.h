...
struct sony_camera_module {
	char				*name;
	const struct sony_camera_seq	*seq_on;
	const struct sony_camera_seq	*seq_off;
};
...
struct sony_camera_info {
	...
	int				gpio_af;
	int				subdev_code;
	const struct sony_camera_module	*modules;
	int				modules_num;
	const char			*default_module_name;
};

extern const struct sony_camera_info camera_info[];
