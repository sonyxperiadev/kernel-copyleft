...
long egisfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FP_FREE_RESOURCE:
		...
		break;
	case GET_SN:			//serial/hardinfo 0xad
		DEBUG_PRINT("EGISTEC fp_ioctl <<< GET_SN  \n");
		if (copy_from_user(&sn, (int __user *)arg, sizeof(sn))) {
			retval = -EFAULT;
			return retval;
		}
                DEBUG_PRINT("fp_ioctl SN = %s\n", sn);
		
        if(sn[2] == '3' && sn[3] == '7')
			get_hardware_info_data(HWID_FINGERPRINT,"egistec,et613;module is CT");
		else if(sn[2] == '3' && sn[3] == '3')
			get_hardware_info_data(HWID_FINGERPRINT,"egistec,et613;module is DT");
		get_hardware_info_data(HWID_FINGERPRINT_SN,sn);
...
int egisfp_platforminit(struct egisfp_dev_t *egis_dev)
{
	...
	if (egis_dev != NULL)
	{
		if (!egis_dev->platforminit_done)
		{
			...
			if (status < 0)
			{
				ERROR_PRINT(" %s : gpio_requset reset pin failed %d\n", __func__,status);
//				goto egisfp_reset_request_fail;
			...
			if (status < 0)
			{
				ERROR_PRINT(" %s : gpio_requset irq pin failed %d ,num:%d\n", __func__,status,egis_dev->irqPin);
//				goto egisfp_irq_request_fail;
			...
			if (egis_dev->pwr_by_gpio && egis_dev->ctrl_power)
			{
				...
				INFO_PRINT(" %s : successful status = %d gpio num -> vdd = %d rst = %d Irq = %d \n", __func__, status, egis_dev->vdd_18v_Pin, egis_dev->rstPin, egis_dev->irqPin);
			}
			else
	...
	if (node)
	{
		...
		if (egis_dev->ctrl_power)
		{
			...
			if (egis_dev->pwr_by_gpio)
			{
				...
				INFO_PRINT(" %s : vdd_18v_pin gpio num is %d \n", __func__, egis_dev->vdd_18v_Pin);
