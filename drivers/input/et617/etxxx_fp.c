...
void egisfp_remove(struct platform_device *pdev);
...
//				ret |= pinctrl_select_state(egis_dev->pinctrl, egis_dev->vdd_high);
			...
			ret |= pinctrl_select_state(egis_dev->pinctrl, egis_dev->irq_active);
...
//				ret |= pinctrl_select_state(egis_dev->pinctrl, egis_dev->vdd_high);
		...
		default:
		{
                        ret |= pinctrl_select_state(egis_dev->pinctrl, egis_dev->irq_low);
			if (egis_dev->pwr_by_gpio)
			{
...
/*					status = gpio_request(egis_dev->vdd_18v_Pin, "egis_dev-18v-gpio");
			...
			if (egis_dev->dd)
			{
				...
				egis_dev->irq_low = pinctrl_lookup_state(egis_dev->pinctrl, "egis_irq_low");
				if (IS_ERR(egis_dev->irq_low))
				{
					status = PTR_ERR(egis_dev->irq_low);
					ERROR_PRINT(" %s : can't find fingerprint pinctrl egis_irq_low \n", __func__);
					goto egisfp_pinctrl_fail;
				}

				if (pinctrl_select_state(egis_dev->pinctrl, egis_dev->irq_low))
					goto egisfp_pinctrl_fail;
