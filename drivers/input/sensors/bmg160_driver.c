...
/* generic */
#define BMG_MAX_RETRY_I2C_XFER (5)
...
static int bmg_pinctrl_init(struct bmg_client_data *client_data)
{
	struct i2c_client *client = client_data->client;

	client_data->pinctrl = devm_pinctrl_get(&client->dev);
	if (IS_ERR_OR_NULL(client_data->pinctrl)) {
		dev_err(&client->dev, "Failed to get pinctrl\n");
		return PTR_ERR(client_data->pinctrl);
	}

	client_data->pin_default = pinctrl_lookup_state(client_data->pinctrl,
			"bmg_default");
	if (IS_ERR_OR_NULL(client_data->pin_default)) {
		dev_err(&client->dev, "Failed to look up default state\n");
		return PTR_ERR(client_data->pin_default);
	}

	client_data->pin_sleep = pinctrl_lookup_state(client_data->pinctrl,
			"bmg_sleep");
	if (IS_ERR_OR_NULL(client_data->pin_sleep)) {
		dev_err(&client->dev, "Failed to look up sleep state\n");
		return PTR_ERR(client_data->pin_sleep);
	}
...
static int bmg_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!bmg_pinctrl_init(client_data)) {
		err = pinctrl_select_state(client_data->pinctrl, client_data->pin_default);
		if (err) {
...
static int bmg_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	ret = pinctrl_select_state(client_data->pinctrl, client_data->pin_sleep);
	if (ret)
		dev_err(&client->dev, "Can't select pinctrl state\n");
...
static int bmg_resume(struct i2c_client *client)
{
	...
	ret = pinctrl_select_state(client_data->pinctrl, client_data->pin_default);
	if (ret)
		dev_err(&client->dev, "Can't select pinctrl state\n");
...
static const struct of_device_id bmg160_of_match[] = {
	...
	{ .compatible = "bosch-sensortec,bmg160", },
	{ .compatible = "bmg160", },
	{ }
};
