...
#define GSENSOR_DBG
#ifdef GSENSOR_DBG
static int gsensor_debug_interrupt = 0;
		...
		u8 *data, u16 len)
{
	...
	if (BMA_MAX_RETRY_I2C_XFER <= retry) {
		GSENSOR_DEBUG(LEVEL0, "I2C xfer error");
		return -EIO;
	}
						...
						unsigned char enabled_mode)
{
	...
	if (mode < 6) {
		...
		comres = bma2x2_smbus_read_byte(client,
				...
				&data2);
		GSENSOR_DEBUG(LEVEL1,"mode(%d)",mode);
		...
		case BMA2X2_MODE_SUSPEND:
			...
			if (ret < 0)
				GSENSOR_DEBUG(LEVEL0, "Error switching to suspend");
				break;
		...
		const char *buf, size_t count)
{
...
#ifdef CONFIG_SENSORS_BMI058
	GSENSOR_DEBUG(LEVEL0, "diff y is %d,value1 is %d, value2 is %d\n", diff,
				value1, value2);
	test_result_branch = 2;
#else
	GSENSOR_DEBUG(LEVEL0, "diff x is %d,value1 is %d, value2 is %d\n", diff,
				value1, value2);
	...
	GSENSOR_DEBUG(LEVEL0, "diff z is %d,value1 is %d, value2 is %d\n", diff, value1, value2);
	...
	bma2x2_soft_reset(bma2x2->bma2x2_client);
	mdelay(5);
	GSENSOR_DEBUG(LEVEL0, "self test finished\n");
...
static void bma2x2_work_func(struct work_struct *work)
{
    ...
    //GSENSOR_DEBUG(LEVEL3," bma2x2_work_func() entry");
    if(atomic_read(&bma2x2->delay)>=BMA2X2_MAX_DELAY){
        GSENSOR_DEBUG(LEVEL3,"atomic_read(&bma2x2->delay)>=BMA2X2_MAX_DELAY");
        return;
    }
    else
        delay = msecs_to_jiffies(atomic_read(&bma2x2->delay));
	...
	GSENSOR_DEBUG(LEVEL3," bma2x2_work_func(): delay=%ld", delay);
	GSENSOR_DEBUG(LEVEL2," x = %3d, y = %3d, z = %3d", acc.x, acc.y, acc.z);
		...
		struct device_attribute *attr, char *buf)
{
	...
	struct bma2x2_data *bma2x2 = i2c_get_clientdata(client);
	unsigned char data=0;
	bma2x2_smbus_read_byte(bma2x2->bma2x2_client,0,&data);
	bma2x2->chip_id = data;
		...
		const char *buf, size_t count)
{
    ...
    GSENSOR_DEBUG(LEVEL1,"delay %d ms",(int)data);
	if (data > BMA2X2_MAX_DELAY)
		data = BMA2X2_MAX_DELAY;
	atomic_set(&bma2x2->delay, (unsigned int) data);

     if(atomic_read(&bma2x2->enable) && !(bma2x2->interrupt))
     {
             GSENSOR_DEBUG(LEVEL0,"cancel delay %d ms small",(int)data);
             cancel_delayed_work_sync(&bma2x2->work);
             schedule_delayed_work(&bma2x2->work,data);
    }
...
static void bma2x2_set_enable(struct device *dev, int enable)
{
	...
	GSENSOR_DEBUG(LEVEL1,"pre_enable(%d), enable(%d)\n", pre_enable, enable);

    if(pre_enable == enable)
        return;

    GSENSOR_DEBUG(LEVEL1,"Enable(%d)\n",enable);
    mutex_lock(&bma2x2->enable_mutex);
    atomic_set(&bma2x2->enable, enable);
    if (enable)
    {
        ...
        if(!bma2x2->interrupt)
        {
            ...
            GSENSOR_DEBUG(LEVEL2, "Polling Mode\n");
#endif
        ...
        {   /* Interrupt Mode */
            if(!bma2x2->irq_flag)
            {
                bma2x2->irq_flag =true;
                enable_irq(bma2x2->IRQ);
            }
            bma2x2->polling_count = 0;
#ifndef CONFIG_BMA_ENABLE_NEWDATA_INT
            schedule_delayed_work(&bma2x2->irq_work,msecs_to_jiffies(0));
            GSENSOR_DEBUG(LEVEL2, "Interrupt Mode\n");
#endif
        }
    }
    else
    {
        GSENSOR_DEBUG(LEVEL2, "Disabled Mode\n");

        if (bma2x2->irq_flag)
        {
            bma2x2->irq_flag = false;
 ...
 */
		timeout++;
		if (timeout == 50) {
			GSENSOR_DEBUG(LEVEL1, "get fast calibration ready error\n");
			return -EINVAL;
		};

	} while (tmp == 0);

	GSENSOR_DEBUG(LEVEL1, "x axis fast calibration finished\n");
	return count;
}
 ...
 */
	...
	GSENSOR_DEBUG(LEVEL0, "y axis fast calibration finished\n");
	return count;
}
 ...
 */
	...
	GSENSOR_DEBUG(LEVEL1, "z axis fast calibration finished\n");
	return count;
}
...
static ssize_t bma2x2_interrupt_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct bma2x2_data *bma2x2 = i2c_get_clientdata(client);

    GSENSOR_DEBUG(LEVEL1, "%d", bma2x2->interrupt);
    return snprintf(buf, PAGE_SIZE, "%d\n", bma2x2->interrupt);
}

static ssize_t bma2x2_interrupt_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct bma2x2_data *bma2x2 = i2c_get_clientdata(client);
    ...
    GSENSOR_DEBUG(LEVEL1, "Interrupt(%d)", (bool)data);

    bma2x2->interrupt = (bool)data;
    if (!bma2x2->interrupt)
    {	/* Polling Mode */
        if (bma2x2->irq_flag)
        {
...
static ssize_t bma2x2_debug_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    GSENSOR_DEBUG(LEVEL1, "gsensor_debug_interrupt=%d, gsensor_debug_level=%d", gsensor_debug_interrupt, gsensor_debug_level);
    return snprintf(buf, PAGE_SIZE, "%d %d\n", gsensor_debug_interrupt, gsensor_debug_level);
}

static ssize_t bma2x2_debug_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    unsigned code1=0, code2=0;
    char *token[10];
    int i=0;
    for (i = 0; i < 2; i++)
    ...
    GSENSOR_DEBUG(LEVEL0, "Set Parameters [%d][%d] ", code1, code2);
    switch(code1)
    {
    case 0:
        gsensor_debug_interrupt = code2;
        break;
    case 1:
        gsensor_debug_level = code2;
        break;
    default:
        GSENSOR_DEBUG(LEVEL0, "[%d][%d] set fail", code1, code2);
        break;
    }
    return count;
}
...
static void bma2x2_tap_timeout_handle(unsigned long data)
{
	...
	GSENSOR_DEBUG(LEVEL1, "tap interrupt handle, timeout\n");
...
static DEVICE_ATTR(interrupt, S_IRUGO|S_IWUSR,
		bma2x2_interrupt_show, bma2x2_interrupt_store);
static DEVICE_ATTR(dbg, S_IRUGO|S_IWUSR,
		bma2x2_debug_show, bma2x2_debug_store);
...
#endif
#ifdef CONFIG_DOUBLE_TAP
static DEVICE_ATTR(tap_time_period, S_IRUGO|S_IWUSR,
		bma2x2_tap_time_period_show, bma2x2_tap_time_period_store);
static DEVICE_ATTR(en_double_tap, S_IRUGO|S_IWUSR,
...
static void bma2x2_irq_work_func(struct work_struct *work)
{
 ...
 if((bma2x2->polling_count==0 ||bma2x2->polling_count>=9)&& gsensor_debug_interrupt)
 {
     GSENSOR_DEBUG(LEVEL0, "%d) x = %3d, y = %3d, z = %3d",bma2x2->polling_count,\
         acc.x,acc.y,acc.z);
 }

 if(abs(acc.x) > BMA2X2_1_5_G||abs(acc.y) > BMA2X2_1_5_G||abs(acc.z) > BMA2X2_1_5_G)
 {
     bma2x2->polling_count = 0;
 }
 bma2x2->polling_count++;

 if(bma2x2->polling_count < 10)
 {
     schedule_delayed_work(&bma2x2->irq_work,msecs_to_jiffies(100));
 }

//bma2x2_get_interruptstatus1(bma2x2->bma2x2_client, &status);
//GSENSOR_DEBUG(LEVEL0, "Interrupt status(%x)",status);
...
static irqreturn_t bma2x2_irq_handler(int irq, void *handle)
{
    ...
    GSENSOR_DEBUG(LEVEL2,"IRQ Debug");
...
//    const __be32 *prop;
    ...
    GSENSOR_DEBUG(LEVEL0, "GPIO = %d,layout=%d\n",pdata->int_pin,pdata->bst_pd->place);
    ...
    pdata->vdd = regulator_get(dev, "bmavdd");
    if (IS_ERR(pdata->vdd))
    {
        GSENSOR_DEBUG(LEVEL0, "Get regulator(bmavdd) failed.\n");
        return -ENODEV;
    }

    pdata->vio = regulator_get(dev, "bmavio");
    if (IS_ERR(pdata->vio))
    {
        GSENSOR_DEBUG(LEVEL0, "Get regulator(bmavio) failed.\n");
        return -ENODEV;
    }
    ...
    {
        GSENSOR_DEBUG(LEVEL0, "%s bosch,vdd-current-level property.\n", prop ? "invalid format" : "no");
        return -ENODEV;
    }
    else
    {
    ...
    GSENSOR_DEBUG(LEVEL1,"GPIO = %d,layout=%x\n",pdata->int_pin,pdata->layout);
#endif
    return 0;
}
#endif /* defined(BMA2X2_ENABLE_INT1)||defined(BMA2X2_ENABLE_INT2) */
		...
		const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		GSENSOR_DEBUG(LEVEL0, "i2c_check_functionality error\n");
		err = -EIO;
		goto exit;
	}
    ...
    /*8,single tap	  10,orient	  11,flat*/
    bma2x2_set_Int_Enable(client, 8, 1);
    bma2x2_set_Int_Enable(client, 10, 1);
    bma2x2_set_Int_Enable(client, 11, 1);
...
#endif
    //data->interrupt = 1;
    INIT_DELAYED_WORK(&data->irq_work, bma2x2_irq_work_func);
    GSENSOR_DEBUG(LEVEL0,"%s:Setup IRQ\n",__func__);

    client->irq = gpio_to_irq(data->int_pin);
	data->IRQ = client->irq;
...
//	INIT_WORK(&data->irq_work, bma2x2_irq_work_func);
	...
	if (IS_ERR(data->g_sensor_class)) {
		...
		GSENSOR_DEBUG(LEVEL0, "could not allocate g_sensor_class\n");
	...
	if (unlikely(IS_ERR(data->g_sensor_dev))) {
		...
		GSENSOR_DEBUG(LEVEL0, "could not allocate g_sensor_dev\n");
	...
	if (IS_ERR(data->g_sensor_class_doubletap)) {
		...
		GSENSOR_DEBUG(LEVEL0, "could not allocate g_sensor_class_doubletap\n");
		goto err_create_class;
	}
	...
	if (unlikely(IS_ERR(data->g_sensor_dev_doubletap))) {
		...
		GSENSOR_DEBUG(LEVEL0, "could not allocate g_sensor_dev_doubletap\n");
...
static int bma2x2_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	ret = pinctrl_select_state(data->pinctrl, data->pin_sleep);
	if (ret)
		dev_err(&client->dev, "Can't select pinctrl state\n");
...
static int bma2x2_resume(struct i2c_client *client)
{
	...
	ret = pinctrl_select_state(data->pinctrl, data->pin_default);
	if (ret)
		dev_err(&client->dev, "Can't select pinctrl state\n");
