...
#define ADUX1050_DRIVER_DBG(format, arg...) pr_debug("[ADUX1050]:"format,\
						    ## arg)

#ifdef CONFIG_ADUX1050_AWFUL_LOG
#define ADUX1050_AWFUL_LOG(format, arg...) pr_debug("[ADUX1050]:"format, ## arg)
#else
#define ADUX1050_AWFUL_LOG(format, arg...) do { if (0); } while (0)
#endif
...
static int adux1050_store_register_values(struct adux1050_chip *adux1050)
{
	...
	if (adux1050->dt_device_node) {
		...
		if (property && len == sizeof(int)) {
			...
			dev_dbg(adux1050->dev, "Invalid req_base on %s\n",
				adux1050->dt_device_node->full_name);
		}
		...
		if (property && len == sizeof(int)) {
			...
			dev_dbg(adux1050->dev, "Invalid req_base1 on %s\n",
				adux1050->dt_device_node->full_name);
		}
		...
		if (property && len == sizeof(int)) {
			...
			dev_dbg(adux1050->dev, "Invalid req_base2 on %s\n",
				adux1050->dt_device_node->full_name);
		}
		...
		if (property && len == sizeof(int)) {
			...
			dev_dbg(adux1050->dev, "Invalid req_base3 on %s\n",
				adux1050->dt_device_node->full_name);
		}
	}
#endif
	...
	for (stg_cnt = 0; stg_cnt < adux1050->num_stages; stg_cnt++) {
		...
		if (err < DEF_WR)
			dev_err(adux1050->dev, "I2C Read Error %d in %s",
					err, __func__);
		ADUX1050_DRIVER_DBG("RESULT_STG%d: 0x%04x\n",
				    stg_cnt, result_cdc);
...
static int adux1050_hw_init(struct adux1050_chip *adux1050)
{
	...
	if (adux1050->high_thresh_enable) {
		...
		ADUX1050_DRIVER_DBG("Report Initial High State\n");
	}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (lp_cnt = 0; lp_cnt < adux1050->stored_data_cnt; lp_cnt++) {
		ADUX1050_AWFUL_LOG("Reg Read cmd:reg 0x%04x Data 0x%04x\n",
				    addr + lp_cnt,
	 ...
	 struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ADUX1050_DRIVER_DBG("Offset Dac Write: reg= 0x%x, value= 0x%x\n",
		addr, wr_data[0]);
 ...
 */
/*--------------------------------------------------------------*/
static DEVICE_ATTR(adux1050_enable, S_IRUGO|S_IWUSR|S_IWGRP,
		   show_enable, store_enable);
/*--------------------------------------------------------------*/
#ifdef CONFIG_ADUX1050_EVAL
static struct device_attribute dev_attr_sensor_name =
	__ATTR(adux1050_device_name, S_IRUGO,
...
static int adux1050_hw_detect(struct adux1050_chip *adux1050)
{
	...
	if (likely((data & ADUX1050_ID_MASK) == ADUX1050_GENERIC_ID)) {
		...
		dev_dbg(adux1050->dev, "Found ADUX1050, rev:%x met_id:%x",
		...
		const char *id, unsigned short *read_value)
{
	...
	*read_value = be32_to_cpu(*param);
	ADUX1050_DRIVER_DBG("%s read from DT, %s:%d\n",
		...
		const struct i2c_device_id *id)
{
	...
	if (dev->platform_data == NULL) {
		...
		pr_debug("%s: Platform Data Not Found\n", __func__);
		...
		if (client->dev.of_node != NULL) {
			...
			if (irqf && (df_prop_length == sizeof(int))) {
				adux1050->pdata->irq_flags =
						be32_to_cpu(*irqf);
				pr_debug("%s Usg irq_flag from DT %d\n",
			...
			pr_debug("%s - Usg local pltfm data\n", __func__);
		}
#endif

	} else {
		pr_debug("%s - Pltfm Data Found\n", __func__);
	...
	if (adux1050->pdata->is_set_irq_gpio_no) {
		unsigned gpioNo = adux1050->pdata->irq_gpio_no;

		ADUX1050_DRIVER_DBG("%s: gpioNo %d\n", __func__, gpioNo);
	...
	adux1050->power_mode_flag = ADUX1050_DISABLE;
#endif
	dev_dbg(adux1050->dev, "%s Completed\n", __func__);
	return 0;
