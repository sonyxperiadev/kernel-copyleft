...
static int stmvl53l0_init_client(struct stmvl53l0_data *data)
{
	...
	if (Status == VL53L0_ERROR_NONE && data->reset) {
		if (papi_func_tbl->PerformRefCalibration != NULL) {
			...
			if (Status == VL53L0_ERROR_NONE)
				pr_err("%s: Ref calibration: VhvSettings 0x%02X, PhaseCal 0x%02X\n",
						TAG, VhvSettings, PhaseCal);
		}
	}
	...
	if (Status == VL53L0_ERROR_NONE && data->reset) {
		if (papi_func_tbl->PerformRefSpadManagement != NULL) {
			...
			if (Status == VL53L0_ERROR_NONE)
				pr_err("%s: Reference SPADs calibration: refSpadCount 0x%08X, isApertureSpads 0x%02X\n",
						TAG, refSpadCount, isApertureSpads);
		}
	...
	init_mode_e mode)
{
	...
	if (mode == OFFSETCALIB_MODE) {
		...
		papi_func_tbl->PerformOffsetCalibration(vl53l0_dev,
			...
			&OffsetMicroMeter);
		pr_err("%s: Offset calibration:0x%08X\n", TAG, OffsetMicroMeter);
#ifdef CALIBRATION_FILE
		...
		*/
		...
		pr_err("%s: Xtalk calibration:0x%08X\n", TAG, XTalkCompensationRateMegaCps);
#ifdef CALIBRATION_FILE
