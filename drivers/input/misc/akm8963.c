...
static struct akm8963_regulator_data akm8963_regu_data;
				 ...
				 const char *buf, size_t count)
{
	...
	if (kstrtol(buf, 10, &single_value))
		return -EINVAL;
...
static int akm8963_regulator_init(struct device *dev)
{

	int rc = 0;

	struct akm8963_regulator_data *regdata = &akm8963_regu_data;
...
static int  akm8963_regulator_control(bool enable, struct device *dev)
{
	int rc = 0;
...
int akm8963_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct akm8963_platform_data *pdata;
	struct akm8963_regulator_data *regdata;

	dev_dbg(&client->dev, "%s called\n", __func__);

	/***** Set layout information *****/
	pdata = &akm8963_pdata;
	regdata = &akm8963_regu_data;

	pdata->irq_gpio = of_get_named_gpio_flags(client->dev.of_node,
				"akm8963,irq_gpio", 0, &pdata->irq_gpio_flags);

	if (pdata->irq_gpio < 0) {
		dev_err(&client->dev,
			"%s: negative gpio number ", __func__);
		err = -EIO;
		goto err_gpio_number;
	}

	err = gpio_request(pdata->irq_gpio, "AKM8963_I2C_NAME");
	if (err)
		printk(KERN_ERR "%s: gpio_request failed, err=%d\n",
					__func__, err);
	pdata->layout = 0;
	pdata->outbit = 0;
	pdata->gpio_DRDY = 0;
	...
	regdata->reg_vdd = NULL;
	regdata->reg_vio = NULL;
	regdata->id_vdd = "akm8963_vdd";
	regdata->id_vio = "akm8963_vio";
	...
	/* init regulator */
	akm8963_regulator_init(&client->dev);

	/* power on */
	akm8963_regulator_control(1, &client->dev);
