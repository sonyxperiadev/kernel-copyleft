	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "disable", strlen("disable"))) {
		...
		pr_err("et512 device_prepare_return error : %d\n", -EINVAL);
		return -EINVAL;
	}
	return rc ? rc : count;
}
...
*/
	...
	if (pdev->dev.of_node) {
		...
		if (of_find_property(np, "et512_vcc-supply", NULL)) {
			vcc_ana = regulator_get(&pdev->dev, "et512_vcc");
			if (IS_ERR(vcc_ana)) {
