...
static int hall_gpio_num;
static int hall_out_status_show(struct seq_file *s, void *unused)
{
        int state;
        state = gpio_get_value(hall_gpio_num);
        pr_info("[Sensor] %s , hall_gpio_num=%d , state=%d\n", __FUNCTION__ , hall_gpio_num, state  );
        seq_printf(s, "%d\n", state);
        return 0;
}
...
static int hall_parse_dt(struct device *dev, struct hall_data *data)
{
        ...
        hall_gpio_num = data->gpio;
        pr_info("[Sensor] %s , gpio=%d , tmp=%d , active_low=%d\n", __FUNCTION__, data->gpio, tmp, data->active_low);
...
static int hall_driver_probe(struct platform_device *dev)
{
        ...
        printk(KERN_ALERT"[Sensor] %s , enter\n", __FUNCTION__ );
