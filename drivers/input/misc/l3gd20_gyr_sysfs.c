...
static int l3gd20_i2c_read(struct l3gd20_data *gyro,
				  u8 *buf, int len)
{
	int err;
...
static int l3gd20_i2c_write(struct l3gd20_data *gyro,
						u8 *buf,
						u8 len)
{
	int err;
...
static int l3gd20_update_fs_range(struct l3gd20_data *gyro, u8 new_fs)
{
	int err = 0;
	if (gyro->enabled) {
		u8 buf[2];
		...
		buf[1] = new_fs|buf[0];
		buf[0] = CTRL_REG4;
		err = l3gd20_i2c_write(gyro, buf, sizeof(buf));
		if (err)
			return err;
	}
	gyro->resume_state[3] = (new_fs & FS_MASK)
				| (~FS_MASK & gyro->resume_state[3]);
	return err;
}
				...
				int poll_interval)
{
	...
	if (gyro->enabled) {
		config[0] = CTRL_REG1;
		err = l3gd20_i2c_write(gyro, config, sizeof(config));
		if (err)
			return err;
	}
			     ...
			     struct l3gd20_xyz *data)
{
	...
	err = l3gd20_i2c_read(gyro, gyro_out, sizeof(gyro_out));
	if (err)
		return err;
...
static void l3gd20_report_values(struct l3gd20_data *gyro,
					struct l3gd20_xyz *data)
{
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	switch (val) {
	case L3GD20_FS_250DPS:
		range = 250;
		break;
	case L3GD20_FS_500DPS:
		range = 500;
		break;
	case L3GD20_FS_2000DPS:
		range = 2000;
		break;
	}
	return snprintf(buf, PAGE_SIZE, "%d\n", range);
				...
				const char *buf, size_t size)
{
	...
	x[0] = gyro->reg_addr;
	x[1] = val;
	rc = l3gd20_i2c_write(gyro, x, sizeof(x));
	mutex_unlock(&gyro->lock);
...
static void l3gd20_input_work_func(struct work_struct *work)
{
	struct l3gd20_data *gyro = container_of(
			(struct delayed_work *)work,
			struct l3gd20_data,
			input_work);

	struct l3gd20_xyz data_out = { 0, 0, 0 };
	int err;
...
static int __devinit l3gd20_validate_pdata(struct l3gd20_data *gyro)
{
...
int l3gd20_input_open(struct input_dev *input)
{
...
void l3gd20_input_close(struct input_dev *dev)
{
...
static int __devinit l3gd20_input_init(struct l3gd20_data *gyro)
{
	int err;

	INIT_DELAYED_WORK(&gyro->input_work, l3gd20_input_work_func);
	...
	gyro->input_dev->open = l3gd20_input_open;
	gyro->input_dev->close = l3gd20_input_close;
	gyro->input_dev->name = L3GD20_DEV_NAME;
					...
					const struct i2c_device_id *devid)
{
	...
	gyro->resume_state[3] = 0x00 | BDU_ENABLE;
...
#else
#define l3gd20_suspend NULL
#define l3gd20_resume NULL
#endif
...
static const struct dev_pm_ops l3gd20_pm = {
