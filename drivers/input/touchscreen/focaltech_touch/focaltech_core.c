...
#define INTERVAL_READ_REG_RESUME            20  /* unit:ms */
...
extern int fts_ex_mode_charger(void);
#if FTS_MT_PROTOCOL_B_EN
...
static void fts_init_power(struct fts_ts_platform_data *pdata, bool enable)
{
    FTS_FUNC_ENTER();
    if (enable)
        gpio_direction_output(pdata->avdd_en_gpio, 1);
    else
        gpio_direction_output(pdata->avdd_en_gpio, 0);
    FTS_FUNC_EXIT();
}
...
static int fts_power_source_ctrl(struct fts_ts_data *ts_data, int enable)
{
 ...
 //           ret = regulator_enable(ts_data->vdd);
 	    fts_init_power(ts_data->pdata, true);
            if (ret) {
	    ...
	    fts_init_power(ts_data->pdata, false);
                if (ret) {
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    pdata->avdd_en_gpio = of_get_named_gpio_flags(np, "focaltech,avdd-en-gpio",
		      0, &pdata->avdd_en_gpio_flags);
    if (pdata->avdd_en_gpio < 0)
	FTS_ERROR("Unable to get avdd_en_gpio");
    ...
    FTS_INFO("max touch number:%d, irq gpio:%d, reset gpio:%d, avdd en gpio:%d",
             pdata->max_touch_number, pdata->irq_gpio, pdata->reset_gpio, pdata->avdd_en_gpio);
...
static void fts_ts_register_for_panel_events(struct device_node *dp,
                                        struct fts_ts_data *ts_data);
extern void *panel_event_notifier_register(enum panel_event_notifier_tag tag,
		enum panel_event_notifier_client client_handle,
		struct drm_panel *panel,
		panel_event_notifier_handler notif_handler, void *pvt_data);

static void fts_notifier_callback_work(struct work_struct *work)
{
    int ret = 0;
...
static int drm_check_dt(struct fts_ts_data *ts_data)
{
    int i = 0;
    int count = 0;
    bool find_panel_flag = true;
    int find_panel_count = 0;
...
#if IS_ENABLED(CONFIG_DRM)
#if IS_ENABLED(CONFIG_DRM_PANEL)
    INIT_WORK(&ts_data->notifier_work, fts_notifier_callback_work);
    queue_work(ts_data->ts_workqueue, &ts_data->notifier_work);
#endif //CONFIG_DRM
#endif //CONFIG_DRM_PANEL
    FTS_FUNC_EXIT();
    return 0;
...
#if IS_ENABLED(CONFIG_DRM)
#if IS_ENABLED(CONFIG_DRM_PANEL)
    cancel_work_sync(&ts_data->notifier_work);
