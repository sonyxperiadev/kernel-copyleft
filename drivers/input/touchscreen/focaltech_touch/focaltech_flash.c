...
const char ctp_data[] = "FT3519T";
const char ctp_modul[] = "Samsung";
char ctp_ver[30] = {0};
...
static bool fts_fwupg_need_upgrade(struct fts_upgrade *upg, u8 *ver_in_host, u8 *ver_in_tp)
{
    int ret = 0;
    bool fwvalid = false;
    u8 fw_ver_in_host = 0;
	...
	*ver_in_host = fw_ver_in_host;
	*ver_in_tp = fw_ver_in_tp;
...
static void fts_set_hardware_info_data(u8 ver_in_host, u8 ver_in_tp)
{
    sprintf(ctp_ver, "fw_tp:%02x, fw_host:%02x", ver_in_tp, ver_in_host);

    get_hardware_info_data(HWID_CTP_DRIVER, ctp_data);

    get_hardware_info_data(HWID_CTP_MODULE, ctp_modul);

    get_hardware_info_data(HWID_CTP_FW_INFO, ctp_ver);
}
...
int fts_fwupg_upgrade(struct fts_upgrade *upg)
{
    ...
    upgrade_flag = fts_fwupg_need_upgrade(upg, &ver_host, &ver_tp);
    ...
    fts_set_hardware_info_data(ver_host, ver_tp);
