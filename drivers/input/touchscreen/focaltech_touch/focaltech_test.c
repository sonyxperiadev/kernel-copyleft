	...
	struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	size += snprintf(buf, PAGE_SIZE, "TestResult=%d\n", fts_ftest->result);
...
static struct proc_dir_entry *tp_test_proc;
#define FTS_PROC_TP_DIFFER "fts_tp_test"


static ssize_t tp_test_proc_write(
...
static int fts_test_proc_show(struct seq_file *m, void *v)
{
	seq_printf(m, "%s", fts_ftest->csv_buffer);
...
{
	return single_open(file, fts_test_proc_show, NULL);
}


static const struct file_operations tp_test_proc_fops =
{
	.open	 = fts_test_proc_open,
	.read	 = seq_read,
	.write  = tp_test_proc_write,
};
...
static int fts_test_func_init(struct fts_ts_data *ts_data)
{
	...
	tp_test_proc = proc_create(FTS_PROC_TP_DIFFER, 0444, NULL, &tp_test_proc_fops);
	if (tp_test_proc == NULL) {
