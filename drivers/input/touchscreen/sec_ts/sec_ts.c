...
static int sec_ts_parse_dt(struct i2c_client *client)
{
	...
	/*reset_gpio*/
	pdata->reset_gpio = of_get_named_gpio(np, "sec,platform-touch-reset-gpio", 0);
	if (!gpio_is_valid(pdata->reset_gpio)) {
		pdata->reset_gpio = RESET_GPIO;
	}
	input_dbg(true, &client->dev, "%s: reset_gpio [%d]\n", __func__, pdata->reset_gpio);

	/*device id*/
	prop = of_find_property(np, "sec,device_id", &len);
	if ((!prop) || len != DEVICE_ID_WD_MAX) {
		pdata->device_id[0] = DEVICE_ID_0;
		pdata->device_id[1] = DEVICE_ID_1;
		pdata->device_id[2] = DEVICE_ID_2;
	} else {
		of_property_read_u8_array(np, "sec,device_id", pdata->device_id, DEVICE_ID_WD_MAX);
	}
	input_info(true, &client->dev, "%s: device_id [%x] [%x] [%x]\n", __func__,
			pdata->device_id[0], pdata->device_id[1], pdata->device_id[2]);
...
static void sec_ts_watchdog_func(struct work_struct *work_watchdog)
{
	...
	ret = sec_ts_i2c_read(ts, SEC_TS_READ_ID, deviceId, DEVICE_ID_WD_MAX);
	if (ret < 0) {
		input_err(true, &ts->client->dev, "%s: failed to read device id(%d)\n", __func__, ret);
		goto unlock;
	}

	if (deviceId[0] != pdata->device_id[0] || deviceId[1] != pdata->device_id[1] || deviceId[2] != pdata->device_id[2]) {
...
int sec_ts_start_device(struct sec_ts_data *ts)
{
	...
	if (ts->plat_data->sod_mode.status && ts->power_status == SEC_TS_STATE_LPM) {
		if (!ts->plat_data->aod_mode.status)
