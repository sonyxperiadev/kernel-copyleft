...
unsigned int EE_SHORT_TEST_LIMIT_PART1 =100;
unsigned int EE_SHORT_TEST_LIMIT_PART2 = 96;
...
#define ABS_0D_OPEN_TEST_LIMIT 100             // ( unit = ratio )

#define ELEC_OPEN_TEST_TX_ON_COUNT 2
#define ELEC_OPEN_TEST_RX_ON_COUNT 2
/*
#define ELEC_OPEN_INT_DUR_ONE 16
#define ELEC_OPEN_INT_DUR_TWO 52
#define ELEC_OPEN_TEST_LIMIT_ONE 500
#define ELEC_OPEN_TEST_LIMIT_TWO 80
*/
/*noise test  global variables*/
unsigned int  NOISE_TEST_LIMIT = 100;

/*elec_open global variables*/
unsigned int  ELEC_OPEN_INT_DUR_ONE = 16;
unsigned int  ELEC_OPEN_INT_DUR_TWO = 52;
unsigned int ELEC_OPEN_TEST_LIMIT_ONE  = 500;
unsigned int  ELEC_OPEN_TEST_LIMIT_TWO = 80;
...
do {\
	...
	if (retval < 0) {\
		TP_LOGE("Failed to disable CBC (" #ctrl_num ")\n");\
		return retval;\
	} \
...
static short *g_tddi_noise_data_log;
...
static short *g_tddi_ee_short_data_log_1;
static short *g_tddi_ee_short_data_log_2;

// a global buffer to record the testing result of amp open test
// size = tx_num * rx_num * sizeof(char)
static unsigned char *g_tddi_amp_open_data_output;
static short *g_tddi_amp_open_data_log_1;
static short *g_tddi_amp_open_data_log_2;

// a global buffer to record the testing result of amp electrode open test
// size = tx_num * rx_num * sizeof(char)
static short *g_tddi_amp_electrode_open_data_log_1;
static short *g_tddi_amp_electrode_open_data_log_2;

// a global buffer to record the calculation data from 0d open test
// size = number of 0d buttons * sizeof(int)
...
//			TP_LOGI("add extended amp button\n");
//			tx += 1;
//		}
	...
	case F54_ADC_RANGE:
		if (f54->query.has_signal_clarity) {
			...
			if (retval < 0) {
				TP_LOGD("Failed to read control reg_41\n");
				f54->report_size = 0;
				break;
			}
	...
	TP_LOGI("tx (%d), rx (%d), report size (%d)\n", tx, rx, f54->report_size);
...
static int test_do_preparation(void)
{
	...
	default:
		...
		if (retval < 0) {
			TP_LOGE("Failed to do force update\n");
			return retval;
		}

		retval = test_do_command(COMMAND_FORCE_CAL);
		if (retval < 0) {
			TP_LOGE("Failed to do force cal\n");
			return retval;
		}
	}
...
/* tddi f54 test reporting + */
	...
	if (retval < 0) {
		TP_LOGE("Failed to get report image\n");
		goto exit;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
			...
			sizeof(data));
	if (retval < 0) {
		TP_LOGE("Failed to write report type\n");
		goto exit;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			...
			sizeof(data));
	if (retval < 0) {
		TP_LOGE("Failed to read report index\n");
		return retval;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
			...
			sizeof(data));
	if (retval < 0) {
		TP_LOGE("Failed to write report index\n");
		return retval;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			...
			sizeof(data));
	if (retval < 0) {
		TP_LOGE("Failed to read no auto cal setting\n");
		return retval;
	}
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
			...
			sizeof(data));
	if (retval < 0) {
		TP_LOGE("Failed to write no auto cal setting\n");
		return retval;
	}
...
static int tddi_ratio_calculation(signed short *p_image)
{
	...
	if (!p_right_median) {
		TP_LOGE("Failed to alloc mem for p_right_median\n");
		retval = -ENOMEM;
		goto exit;
	}
	...
	if (!p_left_median) {
		TP_LOGE("Failed to alloc mem for p_left_median\n");
		retval = -ENOMEM;
		goto exit;
	}
	...
	if (!p_right_column_buf ) {
		TP_LOGE("Failed to alloc mem for p_right_column_buf\n");
		retval = -ENOMEM;
		goto exit;
	}
	...
	if (!p_left_column_buf ) {
		TP_LOGE("Failed to alloc mem for p_left_column_buf\n");
		retval = -ENOMEM;
		goto exit;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	g_tddi_ee_short_data_log_2 = kzalloc(tx_num * rx_num * 2, GFP_KERNEL);
	if (!g_tddi_ee_short_data_log_2) {
	...
	for (i = 0, offset = 0; i < tx_num * rx_num; i++) {
		...
		g_tddi_ee_short_data_log_1[i] = tddi_rt95_part_one[i];
	}
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			...
			if (tddi_rt95_part_one[i*rx_num + j] > EE_SHORT_TEST_LIMIT_PART1) {
				TP_LOGE("fail at (tx%-2d, rx%-2d) = %-4d in part 1 image (limit = %d)\n",
						i, j, tddi_rt95_part_one[i*rx_num + j], EE_SHORT_TEST_LIMIT_PART1);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!g_tddi_ee_short_data_output ||
	    !g_tddi_ee_short_data_log_1 ||
	    !g_tddi_ee_short_data_log_2)
		return -EINVAL;
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			cnt = snprintf(buf, TEST_TEMP_DATA_SIZE - count,
					"%-3d ",
					g_tddi_ee_short_data_log_1[i*rx_num + j]);
			buf += cnt;
			count += cnt;
		}
		cnt = snprintf(buf, TEST_TEMP_DATA_SIZE - count, "\n");
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			cnt = snprintf(buf, TEST_TEMP_DATA_SIZE - count,
					"%-3d ",
					g_tddi_ee_short_data_log_2[i*rx_num + j]);
			buf += cnt;
			count += cnt;
		}
		cnt = snprintf(buf, TEST_TEMP_DATA_SIZE - count, "\n");
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			if (g_tddi_ee_short_data_output[i * rx_num + j] != _TEST_PASS) {
				...
				cnt = snprintf(buf, TEST_TEMP_DATA_SIZE - count, "fail:%-2d,%-2d\n", i, j);
				buf += cnt;
				count += cnt;
			}
		}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!g_tddi_noise_data_output) {
		TP_LOGE("Failed to alloc mem for g_tddi_noise_data_output\n");
		return -ENOMEM;
	}

	if (g_tddi_noise_data_log)
		kfree(g_tddi_noise_data_log);

	g_tddi_noise_data_log = kzalloc(tx_num * rx_num * 2, GFP_KERNEL);
	if (!g_tddi_noise_data_log) {
		TP_LOGE("Failed to alloc mem for g_tddi_noise_data_log\n");
		return -ENOMEM;
	}
	...
	if (!tddi_noise_data) {
		TP_LOGE("Failed to alloc mem for tddi_noise_data\n");
		retval = -ENOMEM;
		goto exit;
	}
	...
	if (!tddi_noise_max) {
		TP_LOGE("Failed to alloc mem for tddi_noise_max\n");
		retval = -ENOMEM;
		goto exit;
	}
	...
	if (!tddi_noise_min) {
		TP_LOGE("Failed to alloc mem for tddi_noise_min\n");
		retval = -ENOMEM;
		goto exit;
	}
	...
	for (repeat = 0 ; repeat < NOISE_TEST_NUM_OF_FRAMES; repeat++){
		...
		retval = test_sysfs_read_report(dev, attr, "94", count,
					false, false);
		if (retval < 0) {
			TP_LOGE("Failed to read report 94 at %d round. exit\n", repeat);
			retval = -EIO;
			g_flag_readrt_err = true;
			goto exit;
		}
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			...
			g_tddi_noise_data_log[i*rx_num +j] = g_tddi_noise_data_output[i*rx_num + j];
			...
			if (g_tddi_noise_data_output[i*rx_num + j] > NOISE_TEST_LIMIT )  {
				TP_LOGE("fail at (tx%-2d, rx%-2d) = %-4d (limit = %d)\n",
						i, j, g_tddi_noise_data_output[i*rx_num + j], NOISE_TEST_LIMIT);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!g_tddi_noise_data_output || !g_tddi_noise_data_log)
		return -EINVAL;
	...
	if (g_flag_readrt_err) {
		...
		kfree(g_tddi_noise_data_log);
		g_tddi_noise_data_log = NULL;

		return snprintf(buf, TEST_TEMP_DATA_SIZE, "\nERROR: fail to read report image\n");
	}

	cnt = snprintf(buf, TEST_TEMP_DATA_SIZE - count,
			"noise limit %-3d (> failure)\n",
			NOISE_TEST_LIMIT);
	buf += cnt;
	count += cnt;
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			cnt = snprintf(buf, TEST_TEMP_DATA_SIZE - count, "%-3d ", g_tddi_noise_data_log[i*rx_num + j]);
			buf += cnt;
			count += cnt;
		}
		cnt = snprintf(buf, TEST_TEMP_DATA_SIZE - count, "\n");
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			if (g_tddi_noise_data_output[i * rx_num + j] != _TEST_PASS) {
				...
				cnt = snprintf(buf, TEST_TEMP_DATA_SIZE - count, "fail on (tx%-2d, rx%-2d)\n", i, j);
				buf += cnt;
				count += cnt;
			}
		}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			g_tddi_amp_open_data_log_1[i * rx_num + j] = p_rt92_delta_image[i * rx_num + j];
			...
			if ((p_rt92_delta_image[i * rx_num + j] < AMP_OPEN_TEST_LIMIT_PHASE1_LOWER) ||
				...
				TP_LOGE("fail at (tx%-2d, rx%-2d) = %-4d at phase 1 (limit = %d, %d)\n",
						i, j, p_rt92_delta_image[i*rx_num + j],
...
#ifdef F54_SHOW_MAX_MIN
	TP_LOGI("ph.1 data range (max, min) = (%-4d, %-4d)\n", max, min);
#endif
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			g_tddi_amp_open_data_log_2[i * rx_num + j] = p_rt92_delta_image[i * rx_num + j];
			...
			if ((p_rt92_delta_image[i * rx_num + j] < AMP_OPEN_TEST_LIMIT_PHASE2_LOWER) ||
				...
				TP_LOGE("fail at (tx%-2d, rx%-2d) = %-4d at phase 2 (limit = %d, %d)\n",
						i, j, p_rt92_delta_image[i*rx_num + j],
...
#ifdef F54_SHOW_MAX_MIN
	TP_LOGI("ph.2 data range (max, min) = (%-4d, %-4d)\n", max, min);
#endif
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!g_tddi_amp_open_data_output ||
	    !g_tddi_amp_open_data_log_1 ||
	    !g_tddi_amp_open_data_log_2)
		return -EINVAL;
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			if (g_tddi_amp_open_data_output[i * rx_num + j] != _TEST_PASS) {
				...
				cnt = snprintf(buf, PAGE_SIZE - count, "fail:%-2d,%-2d\n", i, j);
				buf += cnt;
				count += cnt;
			}
		}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	unsigned char button_count = NUM_BUTTON;
	...
	if (!g_abs_0d_open_data_output) {
		TP_LOGE("Failed to alloc mem for g_abs_0d_open_data_output\n");
		return -ENOMEM;
	}
	...
	retval = test_sysfs_read_report(dev, attr, "92", count,
				false, false);
	if (retval < 0) {
		TP_LOGE("Failed to read report 92 in step 1\n");
		retval = -EIO;
		g_flag_readrt_err = true;
		goto exit;
	}
	...
	if (retval < 0) {
		TP_LOGE("Failed to read original 0d_voltaes in step 2\n");
		retval = -EIO;
		goto exit;
	}
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
			...
			sizeof(data));
	if (retval < 0) {
		TP_LOGE("Failed to modify 0d voltages to f54_ctrl_223 in step 3\n");
		retval = -EIO;
		goto exit;
	}
	retval = test_do_command(COMMAND_FORCE_UPDATE);
	if (retval < 0) {
		TP_LOGE("Failed to do force update in step 3\n");
		retval = -EIO;
		goto exit;
	}
	...
	retval = test_sysfs_read_report(dev, attr, "92", count,
				false, false);
	if (retval < 0) {
		TP_LOGE("Failed to read report 92 in step 4\n");
		retval = -EIO;
		g_flag_readrt_err = true;
		goto exit;
	}
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
			...
			sizeof(original_data_f54_ctr223));
	if (retval < 0) {
		TP_LOGE("Failed to restore 0d voltages to f54_ctrl_223 in step 5\n");
		retval = -EIO;
		goto exit;
	}
	retval = test_do_command(COMMAND_FORCE_UPDATE);
	if (retval < 0) {
		TP_LOGE("Failed to do force update in step 5\n");
		retval = -EIO;
		goto exit;
	}
	...
	for (i = 0; i < button_count; i++) {
		g_abs_0d_open_data_output[i] =
			abs(p_rt92_image_1[(tx_num-1)*rx_num + i] - p_rt92_image_2[(tx_num-1)*rx_num + i]);
		g_abs_0d_open_data_output[i] =
			(g_abs_0d_open_data_output[i] * 100)/p_rt92_image_1[(tx_num-1)*rx_num + i];

		if (g_abs_0d_open_data_output[i] < ABS_0D_OPEN_TEST_LIMIT)
			g_abs_0d_open_data_output[i] = _TEST_FAIL; // fail
		else
			g_abs_0d_open_data_output[i] = _TEST_PASS; // pass
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < button_count; i++) {
		if (g_abs_0d_open_data_output[i] != _TEST_PASS) {
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (g_tddi_amp_electrode_open_data_log_1)
		kfree(g_tddi_amp_electrode_open_data_log_1);
	g_tddi_amp_electrode_open_data_log_1 = kzalloc(tx_num * rx_num * 2, GFP_KERNEL);
	if (!g_tddi_amp_electrode_open_data_log_1) {
		TP_LOGE("Failed to alloc mem for g_tddi_amp_electrode_open_data_log_1\n");
		return -ENOMEM;
	}

	if (g_tddi_amp_electrode_open_data_log_2)
		kfree(g_tddi_amp_electrode_open_data_log_2);
	g_tddi_amp_electrode_open_data_log_2 = kzalloc(tx_num * rx_num * 2, GFP_KERNEL);
	if (!g_tddi_amp_electrode_open_data_log_2) {
		TP_LOGE("Failed to alloc mem for g_tddi_amp_electrode_open_data_log_2\n");
		return -ENOMEM;
	}
	...
	if (retval < 0) {
		TP_LOGE("Failed to read original data from f54_ctrl91\n");
		retval = -EIO;
		goto exit;
	}
	...
	if (retval < 0) {
		TP_LOGE("Failed to read original data from f54_ctrl99\n");
		retval = -EIO;
		goto exit;
	}
	...
	if (retval < 0) {
		TP_LOGE("Failed to read original data from f54_ctrl182\n");
		retval = -EIO;
		goto exit;
	}
	...
	if (retval < 0) {
		TP_LOGE("Failed to set f54_ctrl91 in step 1\n");
		retval = -EIO;
		goto exit;
	}
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
			...
			sizeof(control.reg_99->data));
	if (retval < 0) {
		TP_LOGE("Failed to set f54_ctrl99 in step 2\n");
		retval = -EIO;
		goto exit;
	}
	...
	if (retval < 0) {
		TP_LOGE("Failed to set f54_reg_182 in step 3\n");
		retval = -EIO;
		goto exit;
	}
/*
	retval = test_do_command(COMMAND_FORCE_UPDATE);
	if (retval < 0) {
		TP_LOGE("Failed to do force update before changing integration duration\n");
		retval = -EIO;
		goto exit;
	}
*/
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			...
			sizeof(control.reg_99->data));
	if (retval < 0) {
		TP_LOGE("Failed to read data from f54_ctrl99 in step 4\n");
		retval = -EIO;
		goto exit;
	}
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
			...
			sizeof(control.reg_99->data));
	if (retval < 0) {
		TP_LOGE("Failed to seet ELEC_OPEN_INT_DUR_ONE(%d) in step 4\n",
	...
	retval = test_do_command(COMMAND_FORCE_UPDATE);
	if (retval < 0) {
		TP_LOGE("Failed to do force update in step 4\n");
		retval = -EIO;
		goto exit;
	}
	...
	retval = test_sysfs_read_report(dev, attr, "92", count,
				false, false);
	if (retval < 0) {
		TP_LOGE("Failed to read report 92 in step 5. exit\n");
		retval = -EIO;
		g_flag_readrt_err = false;
		goto exit;
	}
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			...
			sizeof(control.reg_99->data));
	if (retval < 0) {
		TP_LOGE("Failed to read data from f54_ctrl99 in step 6\n");
		retval = -EIO;
		goto exit;
	}
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
			...
			sizeof(control.reg_99->data));
	if (retval < 0) {
		TP_LOGE("Failed to seet ELEC_OPEN_INT_DUR_TWO(%d) in step 6\n", ELEC_OPEN_INT_DUR_TWO);
		retval = -EIO;
		goto exit;
	}
	...
	retval = test_do_command(COMMAND_FORCE_UPDATE);
	if (retval < 0) {
		TP_LOGE("Failed to do force update in step 6\n");
		retval = -EIO;
		goto exit;
	}
	...
	retval = test_sysfs_read_report(dev, attr, "92", count,
				false, false);
	if (retval < 0) {
		TP_LOGE("Failed to read report 92 in step 7. exit\n");
		retval = -EIO;
		goto exit;
	}
	secure_memcpy(p_report_data_8, tx_num * rx_num * 2,
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
			...
			sizeof(original_f54_ctrl91));
	if (retval < 0) {
		TP_LOGE("Failed to restore f54_ctrl91 data\n");
		retval = -EIO;
		goto exit;
	}
	retval = synaptics_rmi4_reg_write(rmi4_data,
			...
			sizeof(original_f54_ctrl99));
	if (retval < 0) {
		TP_LOGE("Failed to restore f54_ctrl99 data\n");
		retval = -EIO;
		goto exit;
	}
	retval = synaptics_rmi4_reg_write(rmi4_data,
			...
			sizeof(original_f54_ctrl182));
	if (retval < 0) {
		TP_LOGE("Failed to restore f54_ctrl182 data\n");
		retval = -EIO;
		goto exit;
	}
	retval = test_do_command(COMMAND_FORCE_UPDATE);
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			g_tddi_amp_electrode_open_data_log_1[i * rx_num + j]
					= p_rt92_delta_image[i * rx_num + j];
			...
			if (p_rt92_delta_image[i * rx_num + j] < ELEC_OPEN_TEST_LIMIT_ONE) {
				...
				TP_LOGE("fail at (tx%-2d, rx%-2d) = %-4d at phase 1 (limit = %d)\n",
 						i, j, p_rt92_delta_image[i*rx_num + j], ELEC_OPEN_TEST_LIMIT_ONE);
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			g_tddi_amp_electrode_open_data_log_2[i * rx_num + j]
					= p_rt92_delta_image[i * rx_num + j];
#ifdef F54_SHOW_MAX_MIN
			min = min_t(signed short, p_rt92_delta_image[i*rx_num + j], min);
			max = max_t(signed short, p_rt92_delta_image[i*rx_num + j], max);
			...
			if (p_rt92_delta_image[i * rx_num + j] < ELEC_OPEN_TEST_LIMIT_TWO) {
				...
				TP_LOGE("fail at (tx%-2d, rx%-2d) = %-4d at phase 2 (limit = %d)\n",
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!g_tddi_amp_open_data_output ||
	    !g_tddi_amp_electrode_open_data_log_1 ||
	    !g_tddi_amp_electrode_open_data_log_2)
		return -EINVAL;
	...
	if (g_flag_readrt_err) {
		...
		kfree(g_tddi_amp_electrode_open_data_log_1);
		g_tddi_amp_electrode_open_data_log_1 = NULL;

		kfree(g_tddi_amp_electrode_open_data_log_2);
		g_tddi_amp_electrode_open_data_log_2 = NULL;
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			cnt = snprintf(buf, TEST_TEMP_DATA_SIZE - count, "%-4d ",
					g_tddi_amp_electrode_open_data_log_1[i * rx_num + j]);
			buf += cnt;
			count += cnt;
		}
	...
	min = max = g_tddi_amp_electrode_open_data_log_2[0];

	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			min = (min < g_tddi_amp_electrode_open_data_log_2[i * rx_num + j]) ? min : g_tddi_amp_electrode_open_data_log_2[i * rx_num + j];
			max = (max > g_tddi_amp_electrode_open_data_log_2[i * rx_num + j]) ? max : g_tddi_amp_electrode_open_data_log_2[i * rx_num + j];
		}
	}
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (retval < 0) {
		TP_LOGE("Failed to copy report data\n");
		goto exit;
	}
...
static int save_test_data(char *file_name, char *data_buf, int iLen)
{
	...
	if (NULL == pfile)
		pfile = filp_open(file_name, O_CREAT | O_RDWR | O_TRUNC , S_IRUSR | S_IWUSR);
...
static ssize_t test_sysfs_self_test_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int test_fail = 0;
	...
	if (testdata == NULL) {
		TP_LOGE("Can not alloc mem for testdata\n");
		test_fail++;
		goto exit;
	}
	...
	if (temp == NULL) {
		TP_LOGE("Can not alloc mem for temp\n");
		test_fail++;
		goto exit2;
	}

	/* Step 1: Noise Test */
	cnt += sprintf(testdata + cnt, "\n******** tddi_noise ********\n");

	memset(temp, 0, TEST_TEMP_DATA_SIZE);
	ret = test_sysfs_tddi_noise_store(dev, attr, "1", 1);
	if (ret < 0) {
		TP_LOGE("tddi_noise store failed\n");
		test_fail++;
		goto exit3;
	}

	ret = test_sysfs_tddi_noise_show(dev, attr, temp);
	if (ret < 0) {
		TP_LOGE("tddi_noise show failed\n");
		test_fail++;
		goto exit3;
	}

	TP_LOGI("Noise Test result: %s\n", temp + strlen(temp) - 5);
	...
	memset(temp, 0, TEST_TEMP_DATA_SIZE);
	ret = test_sysfs_tddi_ee_short_store(dev, attr, "1", 1);
	if (ret < 0) {
		TP_LOGE("tddi_ee_short store failed\n");
		test_fail++;
		goto exit3;
	}

	ret = test_sysfs_tddi_ee_short_show(dev, attr, temp);
	if (ret < 0) {
		TP_LOGE("tddi_ee_short show failed\n");
		test_fail++;
		goto exit3;
	}
	...
	memset(temp, 0, TEST_TEMP_DATA_SIZE);
	ret = test_sysfs_tddi_amp_open_store(dev, attr, "1", 1);
	if (ret < 0) {
		TP_LOGE("tddi_amp_open store failed\n");
		goto exit3;
	}

	ret = test_sysfs_tddi_amp_open_show(dev, attr, temp);
	if (ret < 0) {
		TP_LOGE("tddi_amp_open show failed\n");
		goto exit3;
	}
	...
	cnt += sprintf(testdata + cnt, "%s", temp);
#else
	cnt += sprintf(testdata + cnt, "\n******** tddi_amp_electrode_open ********\n");

	memset (temp, 0, TEST_TEMP_DATA_SIZE);
	ret = test_sysfs_tddi_amp_electrode_open_store(dev, attr, "1", 1);
	if (ret < 0) {
		TP_LOGE("tddi_amp_electrode_open store failed\n");
		test_fail++;
		goto exit3;
	}

	ret = test_sysfs_tddi_amp_electrode_open_show(dev, attr, temp);
	if (ret < 0) {
		TP_LOGE("tddi_amp_electrode_open show failed\n");
		test_fail++;
		goto exit3;
	}
...
static void test_f21_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!query_2) {
		TP_LOGE("Failed to alloc mem for query_2\n");
		goto exit;
	}
	...
	if (!query_11) {
		TP_LOGE("Failed to alloc mem for query_11\n");
		goto exit;
	}
	...
	if (!query_2->query11_is_present) {
		TP_LOGE("No F21 force capabilities\n");
		goto exit;
	}
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			...
			f21->max_num_of_txrx);
	if (retval < 0) {
		TP_LOGE("Failed to read F21 force txrx assignment\n");
		goto exit;
	}
		...
		unsigned char page)
{
	...
	if (!f21) {
		TP_LOGE("Failed to alloc mem for F21\n");
		return;
	}
