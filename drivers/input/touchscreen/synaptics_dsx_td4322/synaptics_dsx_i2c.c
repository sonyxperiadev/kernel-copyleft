...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
	...
	if (prop && prop->length) {
		...
		retval = of_property_read_u32(np, "synaptics,power-on-state",
				&value);
		if (retval < 0) {
			TP_LOGE("Unable to read synaptics,power-on-state property\n");
			return retval;
		} else {
	...
	if (prop && prop->length) {
		retval = of_property_read_u32(np, "synaptics,power-delay-ms",
				&value);
		if (retval < 0) {
			TP_LOGE("Unable to read synaptics,power-delay-ms property\n");
			return retval;
		} else {
	...
	if (prop && prop->length) {
		...
		retval = of_property_read_u32(np, "synaptics,reset-on-state",
				&value);
		if (retval < 0) {
			TP_LOGE("Unable to read synaptics,reset-on-state property\n");
			return retval;
		} else {
		...
		retval = of_property_read_u32(np, "synaptics,reset-active-ms",
				&value);
		if (retval < 0) {
			TP_LOGE("Unable to read synaptics,reset-active-ms property\n");
			return retval;
		} else {
	...
	if (prop && prop->length) {
		retval = of_property_read_u32(np, "synaptics,reset-delay-ms",
				&value);
		if (retval < 0) {
			TP_LOGE("Unable to read synaptics,reset-delay-ms property\n");
			return retval;
		} else {
	...
	if (prop && prop->length) {
		retval = of_property_read_u32(np, "synaptics,max-y-for-2d",
				&value);
		if (retval < 0) {
			TP_LOGE("Unable to read synaptics,max-y-for-2d property\n");
			return retval;
		} else {
	...
	if (prop && prop->length) {
		retval = of_property_read_u32(np, "synaptics,ub-i2c-addr",
				&value);
		if (retval < 0) {
			TP_LOGE("Unable to read synaptics,ub-i2c-addr property\n");
			return retval;
		} else {
...
{
	int retval;
#ifdef CEI_I2C_ADDR_20_WORKAROUNG
	struct i2c_msg msgs[2];
	unsigned char data = 0x00;
	unsigned char reg = 0xee;
#endif
...
#ifdef CEI_I2C_ADDR_20_WORKAROUNG
	/*
	 * FIXME: this workaroung is for some Toouch modules that
	 * i2c slave address is 0x20. Need remove this workaroung
	 * after DP build.
	 */
	msgs[0].addr = 0x20,
	msgs[0].flags = 0,
	...
	/* NOTE: read 0xEE always return 0x34 */

	TP_LOGI("[i2c-addr-0x20-workaroung] i2c_transfer return value = %d"
			" (read data 0x%X)\n", retval, data);

	if (retval == 2) {
		TP_LOGI("[i2c-addr-0x20-workaroung] change i2c_client addr to 0x20\n");
		client->addr = 0x20;
	}

	TP_LOGI("[i2c-addr-0x20-workaroung] client->addr = 0x%X\n", client->addr);
#endif /* CEI_I2C_ADDR_20_WORKAROUNG */
