...
/* __LXS_SUPPORT_ABT */
int __weak lxs_hal_abt_sysfs(struct lxs_ts *ts, int on_off)

{
	t_dev_info_once(ts->dev, "ABT disabled\n");
	return 0;
}
...
static const struct lxs_hal_reg lxs_hal_reg_table = {
	...
	.tc_tsp_test_result		= TC_TSP_TEST_RESULT,
	.tc_flash_dn_status		= TC_FLASH_DN_STS,
	.tc_confdn_base_addr	= TC_CONFDN_BASE_ADDR,
	.tc_flash_dn_ctl		= TC_FLASH_DN_CTL,
	/* */
	.prd_ic_start_reg		= PRD_IC_START_REG,
	.prd_ic_readystatus		= PRD_IC_READYSTATUS,
	/* */
	.abt_report_mode_read	= ABT_REPORT_MODE_READ,
	.abt_report_mode_write	= ABT_REPORT_MODE_WRITE,
	.abt_compress_write		= ABT_COMPRESS_WRITE,
};
...
static int lxs_hal_add_sysfs(struct lxs_ts *ts)
{
	...
	ret = lxs_hal_abt_sysfs(ts, 1);
	if (ret < 0)
		t_dev_err(ts->dev, "LXS(%s) abt sysfs register failed\n", ts->chip_name);
...
static int lxs_hal_chk_status_type(struct lxs_ts *ts)
{
	...
	if (chip->opt.t_oled)
		chip->status_mask_ic_abnormal &= ~BIT(0);
...
{
	struct lxs_ts_chip *chip = &ts->chip;
	int is_np = (chip->lcd_mode == LCD_MODE_U3);
	int boot_fail_cnt = (is_np) ? chip->boot_fail_cnt : 0;
	int ret = 0;
...
static void lxs_hal_ic_stat_boot(struct lxs_ts *ts, int sts)
{
	struct lxs_ts_chip *chip = &ts->chip;
	...
	case LCD_MODE_U3:
		...
		chip->boot_fail_cnt += !!(sts == -ETDBOOTFAIL);
		break;
	}
}
...
/* unique id */
static int __lxs_hal_uni_id(struct lxs_ts *ts, int prt_on)
{
	struct lxs_ts_chip *chip = &ts->chip;
	struct lxs_hal_reg *reg = chip->reg;
	u32 uid[5] = { 0, };
	...
	memset(&chip->uni_id, 0, sizeof(chip->uni_id));

	switch (chip->opt.t_uni_id) {
	case 1:
		ret = lxs_hal_write_value(ts, reg->spr_code_offset, 0x8009);
		if (ret < 0)
			return ret;

		ret = lxs_hal_reg_read(ts, reg->code_access_addr, uid, 5<<2);
		lxs_hal_write_value(ts, reg->spr_code_offset, 0);
		if (ret < 0)
			return ret;

		t_dev_dbg_base(ts->dev, "uid : 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X\n",
			uid[0], uid[1], uid[2], uid[3], uid[4]);

		chip->uni_id[0] = uid[0] & 0xFFFF;
		chip->uni_id[1] = uid[1];
		chip->uni_id[2] = ((uid[2] & 0xFF)<<24) | ((uid[3] & 0xFF)<<16) | (uid[4] & 0xFFFF);

		memset(chip->uni_id_str, 0, sizeof(chip->uni_id_str));
		snprintf(chip->uni_id_str, sizeof(chip->uni_id_str) - 1, "0x%04X%08X%08X",
			chip->uni_id[0], chip->uni_id[1], chip->uni_id[2]);

		t_dev_info_sel(ts->dev, prt_on, "[T] chip unique id %s\n", chip->uni_id_str);
		break;
	}
...
//	char sys_id_str[16] = {0, };
	...
	struct lxs_hal_rw_multi multi[] = {
		...
		{ 0, reg->tc_product_id1, product, fw->pid_lim, "product_id" },
		{ -1, -1, NULL, },
	};
	int ret = 0;
	...
	lxs_hal_fw_set_prod_id(fw, (u8 *)product, fw->pid_lim);
...
static int lxs_hal_init_reg_set(struct lxs_ts *ts)
{
	...
	ret = lxs_hal_write_value(ts, reg->tc_device_ctl + 1, 1);
	if (ret < 0)
		goto out;
...
static int lxs_hal_hw_reset(struct lxs_ts *ts, int ctrl, int pwr_con)
{
	...
	if (err) {
		...
		ts->chip.reset_is_on_going = false;
		return err;
	}
...
int lxs_hal_reset(struct lxs_ts *ts, int ctrl, int pwr_con)
{
	...
	if (chip->reset_is_on_going) {
		t_dev_warn(ts->dev, "%s: reset is ongoing\n", __func__);
		lxs_ts_delay(100);
	}
...
void lxs_hal_reset_halt(struct lxs_ts *ts)
{
	struct lxs_ts_chip *chip = &ts->chip;
...
void lxs_hal_connect(struct lxs_ts *ts)
{
	struct lxs_ts_chip *chip = &ts->chip;
	struct lxs_hal_reg *reg = chip->reg;
	...
	if (wireless_state)
		wireless_val = (wireless_state > 1) ? wireless_state : TS_CONNECT_WIRELESS;
	...
	case 1:
		...
		break;
	default:
		chip->charger = (!!charger_state) | wireless_val;
		break;
	}

	t_dev_info(ts->dev,
		"charger: 0x%X (0x%X, 0x%X)\n",
		chip->charger, charger_state, wireless_state);
...
static void lxs_hal_tc_restore(struct lxs_ts *ts)
{
	...
	if (ts->fquirks->tc_restore)
		ts->fquirks->tc_restore(ts);
}
...
static int lxs_hal_tc_driving_pre(struct lxs_ts *ts, int mode)
{
	...
	if (ts->fquirks->fps_set_pre)
		ts->fquirks->fps_set_pre(ts, mode);

	return ret;
}

static int lxs_hal_tc_driving_post(struct lxs_ts *ts, int mode)
{
	if (ts->fquirks->fps_set_post)
		ts->fquirks->fps_set_post(ts, mode);
...
int lxs_hal_tc_driving(struct lxs_ts *ts, int mode)
{
	...
	lxs_hal_tc_driving_post(ts, mode);
...
static int lxs_hal_resume_touch(struct lxs_ts *ts)
{
	...
	t_dev_info(ts->dev, "%s resume touch: aod %d\n",
		ts->chip_name, chip->enable_aod);
...
void lxs_hal_enable_pending(struct lxs_ts *ts,
		int step, const char *title)
{
	struct lxs_ts_chip *chip = &ts->chip;
	int f_p = 0;
	int i_p = 0;
	...
	case 1:
		while (work_busy(&ts->upgrade_work.work)) {
			if (!f_p)
				t_dev_info(ts->dev, "%s: waiting upgrade work...\n", title);
			f_p++;
	...
	default:
		...
		while (work_busy(&ts->init_work.work)) {
			if (!i_p)
				t_dev_info(ts->dev, "%s: waiting init work...\n", title);
			i_p++;
...
int lxs_hal_enable_device(struct lxs_ts *ts, int enable)
{
	...
	if (work_busy(&ts->upgrade_work.work) & WORK_BUSY_RUNNING) {
		lxs_hal_enable_pending(ts, 1, NULL);
	} else if (ts->state_core != TS_CORE_NORMAL) {
		t_dev_err(ts->dev, "%s: not normal state(%d)\n", __func__, enable);
		return -EINVAL;
	}

	cwork = cancel_delayed_work_sync(&ts->upgrade_work);
	if (cwork == 1) {
		ts->force_fwup = TS_FORCE_FWUP_CLEAR;
		ts->test_fwpath[0] = '\0';
		t_dev_info(ts->dev, "%s: F/W upgrade canceled\n", __func__);
	}

	lxs_hal_enable_pending(ts, 0, NULL);

	mutex_lock(&ts->lock);

	if (atomic_read(&chip->boot) == TC_IC_BOOT_FAIL)
		chip->boot_fail_cnt -= (chip->boot_fail_cnt) ? !!(cwork == 1) : 0;
...
{
	struct lxs_ts_chip *chip = &ts->chip;
	int do_reset = lxs_ts_is_power_off(ts);
	...
	case LCD_MODE_U3:
		if (atomic_read(&chip->boot) == TC_IC_BOOT_FAIL) {
			if (chip->prev_aod != chip->enable_aod) {
				if (chip->lcd_mode == mode)
					if (chip->boot_fail_cnt >= BOOT_FAIL_RECOVERY_MAX)
						chip->boot_fail_cnt = 0;
			}
		}
	...
	if (run_init) {
		...
		/* transition for error situation */
		chip->prev_aod = chip->enable_aod;
		chip->prev_knock = chip->enable_knock;
		chip->prev_prox = chip->enable_prox;
		chip->prev_sleep = chip->enable_sleep;

		lxs_hal_reset(ts, reset_ctrl, !!(ts->state_sleep == TS_IC_SLEEP));
	} else {
		lxs_ts_release_all_event(ts);
	}

out:
	if (!chip->reset_is_on_going && (ts->state_sleep != TS_IC_SLEEP))
		lxs_ts_irq_control(ts, 1);
...
int lxs_hal_touch_mode(struct lxs_ts *ts, int state_mode)
{
	...
	if (work_busy(&ts->upgrade_work.work) & WORK_BUSY_RUNNING)
		lxs_hal_enable_pending(ts, 1, "mode");
	...
	lxs_hal_enable_pending(ts, 0, "mode");

	if (state_mode == TS_MODE_SUSPEND)
		t_dev_dbg_base(ts->dev, "%s suspend\n", __func__);

	if (chip->enable_aod)
		t_dev_dbg_base(ts->dev, "%s aod\n", __func__);

	if ((state_mode == TS_MODE_SUSPEND) || chip->enable_aod)
		lxs_ts_watchdog_stop(ts);
	...
	case TS_MODE_RESUME:
		...
		/* Normal or AOD */
		chk_mode = (chip->enable_aod) ? chip->lcd_mode : chip->driving_mode;
	...
	case TS_MODE_SUSPEND:
		/* Off or Off(EWU) */
		if (chip->enable_sleep && lxs_ts_is_power_off(ts)) {
			if (!chip->enable_knock || chip->enable_prox) {
				keep_sleep = 1;
				do_reset = 0;
				break;
		...
		mode = (touch_mode_allowed(ts, LCD_MODE_U0)) ? LCD_MODE_U0 : LCD_MODE_STOP;
				...
				u32 status, u32 ic_status, int irq)
{
	...
	while (1) {
		...
		if (check_mask && detected) {
			...
			if (type_fault && (fault_val >= 0)) {
				len += lxs_gen_snprintf(log, log_max, len,
	...
	if (ic_abnormal || ic_error || ic_disp_err) {
		...
		len = lxs_gen_snprintf(log, log_max, 0, "[%d] ", irq);
		...
		for (i = 0; i < ARRAY_SIZE(err_val) ; i++) {
			...
			len += lxs_gen_snprintf(log, log_max, len, "%s", err_str[i]);
			err_pre |= err_val[i];
		}
...
int lxs_hal_irq_lpwg(struct lxs_ts *ts)
{
	...
	ret = (ops->lpwg_data) ? ops->lpwg_data(ts) : __irq_lpwg_default(ts);
	if (ret < 0)
...
int lxs_hal_chipset_option_log(struct lxs_ts *ts, char *buf)
{
	struct lxs_ts_chip *chip = &ts->chip;
	...
	size += lxs_dev_snprintf(ts->dev, buf, size, "[opt summary]\n");
	while (options->fmt != NULL) {
		if (options->value != options->chk)
			size += lxs_dev_snprintf(ts->dev, buf, size,
						options->fmt, options->value);
...
static void lxs_hal_chipset_option(struct lxs_ts *ts)
{
	...
	lxs_hal_chipset_option_log(ts, NULL);
}
...
void lxs_hal_rinfo_channel(struct lxs_ts *ts)
{
	struct lxs_ts_chip *chip = &ts->chip;
	struct lxs_hal_reg *reg = chip->reg;
	u32 channel;
...
void lxs_hal_rinfo_resolution(struct lxs_ts *ts)
{
	struct lxs_ts_chip *chip = &ts->chip;
	struct lxs_hal_reg *reg = chip->reg;
	u32 resolution;
...
void lxs_hal_rinfo_base(struct lxs_ts *ts)
{
	struct lxs_ts_chip *chip = &ts->chip;
	struct lxs_hal_reg *reg = chip->reg;
	u32 info_data;
...
void lxs_hal_rinfo(struct lxs_ts *ts)
{
...
void lxs_hal_activate(struct lxs_ts *ts)
{
	...
	if (ts->fquirks->activate)
		ts->fquirks->activate(ts);
}

void lxs_hal_deactivate(struct lxs_ts *ts)
{
	if (ts->fquirks->deactivate)
		ts->fquirks->deactivate(ts);
...
int lxs_hal_probe(struct lxs_ts *ts)
{
	...
	lxs_ts_power_connect(ts);
	lxs_ts_delay(ts->hw_reset_delay);

	ret = lxs_hal_hw_init(ts);
	if (ret) {
		lxs_ts_power_disconnect(ts);
...
static int lxs_hal_watchdog_chk_id(struct lxs_ts *ts)
{
	...
	t_dev_dbg_trace(ts->dev, "%s: ok - 0x%08X\n", __func__, chip_id);
...
static int lxs_hal_watchdog_chk_status(struct lxs_ts *ts)
{
	...
	t_dev_dbg_trace(ts->dev, "%s: ok - 0x%08X, 0x%08X\n", __func__, ic_status, status);
