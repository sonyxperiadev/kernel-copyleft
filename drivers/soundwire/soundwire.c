...
/**
 * swr_remove_group(struct swr_device *dev, u8 dev_num)
 */
int swr_remove_group(struct swr_device *dev, u8 dev_num)
{
	struct swr_master *master;
	...
	if (master->remove_group)
		is_removed = master->remove_group(master);
...
EXPORT_SYMBOL(swr_remove_group);
