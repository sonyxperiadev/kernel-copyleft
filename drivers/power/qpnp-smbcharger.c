...
static int get_prop_batt_status(struct smbchg_chip *chip)
{
	...
	if (gpio_is_valid(chip->stat1_gpio)) {
		...
		pr_smb(PR_STATUS, "stat1_gpio = 0x%02X ^ 0x%02X = 0x%02X\n",
			val, chip->stat1_active_low, (val ^ chip->stat1_active_low));
	...
	if (gpio_is_valid(chip->stat2_gpio)) {
		...
		pr_smb(PR_STATUS, "stat2_gpio = 0x%02X ^ 0x%02X = 0x%02X\n",
			val, chip->stat2_active_low, (val ^ chip->stat2_active_low));
...
static int smb_parse_dt(struct smbchg_chip *chip)
{
	...
	if (!gpio_is_valid(chip->stat1_gpio)) {
		dev_err(chip->dev, "stat1(%d) is missing\n", chip->stat1_gpio);
		chip->stat1_gpio = -1;
	}
	...
	if (!gpio_is_valid(chip->stat2_gpio)) {
		dev_err(chip->dev, "stat2(%d) is missing\n", chip->stat2_gpio);
		chip->stat2_gpio = -1;
	}
