...
#else
		if (!chip->skip_usb_notification && chip->typec_current_ma) {
#endif
...
static void smbchg_external_power_changed(struct power_supply *psy)
{
	...
	rc = chip->usb_psy->get_property(chip->usb_psy,
				POWER_SUPPLY_PROP_SUB_TYPE, &prop);
	if (rc == 0 && POWER_SUPPLY_SUB_TYPE_INVALID == prop.intval) {
		pr_smb(PR_SOMC, "detect INVALID CHARGER !!\n");
		somc_chg_invalid_set_state(&chip->somc_params, 1);
	}
#endif
...
static void smbchg_hvdcp_det_work(struct work_struct *work)
{
	...
	} else if (chip->typec_psy && is_usb_present(chip)) {
