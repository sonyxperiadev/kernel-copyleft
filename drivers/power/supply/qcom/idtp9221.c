...
struct idtp9221 {
	...
	bool			vout_limit_by_high_temp;
	bool			vout_limit_by_charge_full;
	int			vout_limit_thresh;
	bool			vout_limit_charge_full_en;
...
static int idtp9221_debug_mask = PR_MAJOR | PR_MONITOR;
...
static void idtp9221_dump_reg(struct idtp9221 *chip)
{
	...
	if (time_diff_ms < idtp9221_debug_dump * 1000)
		return;
...
static int idtp9221_vtherm_calc_find_msb(long antilog)
{
	int msb = 0;
...
{
	long ln, log2, log2_integer, log2_real;
	int msb;
...
static int idtp9221_handle_vout_set(struct idtp9221 *chip)
{
	...
	if (chip->status == IDTP9221_STATUS_PWR_POWER_OK) {
		if (chip->op_mode == IDTP9221_OP_MODE_QI_EPP) {
			if (chip->vout_limit_by_high_temp ||
			    chip->vout_limit_by_charge_full)
				vout_mv = chip->bpp_vout_mv;
			else if (chip->thermal_max_voltage > 0 &&
...
static void idtp9221_vout_change_work(struct work_struct *work)
{
	...
	if (current_v_reg + VOUT_STEP < target_v_reg) {
		...
		val = target_v_reg;
		idtp9221_dbg(chip, PR_MAJOR, "Step Vout(Goal): 0x%02x\n", val);
		rc = idtp9221_i2c_write_byte(chip, IDTP9221_REG_VOUT_SET, val);
		if (rc < 0) {
			dev_err(chip->dev, "write error VOUT_SET %d\n", rc);
			goto error;
...
static int idtp9221_set_ilim_set(struct idtp9221 *chip, int val)
{
	...
	if (current_ilim_reg != target_ilim_reg) {
		idtp9221_dbg(chip, PR_MAJOR, "set ilim_set to %dmA (from %d)\n",
...
static int idtp9221_set_fod(struct idtp9221 *chip, int reg_offset, u8 val)
{
	...
	idtp9221_dbg(chip, PR_MAJOR, "set FOD reg:0x%02x, val:0x%02x\n",
								reg_addr, val);
...
static int idtp9221_set_ept(struct idtp9221 *chip, int mode)
{
	...
	if (chip->status != IDTP9221_STATUS_PWR_OFF) {
		...
		idtp9221_dbg(chip, PR_MAJOR, "Sent EPT command. mode:%d\n",
									mode);
	}
	return rc;
}
...
static int idtp9221_read_regs_oneshot_in_driver(struct idtp9221 *chip)
{
	...
	idtp9221_dbg(chip, PR_MAJOR, "FW Source : %d\n", chip->fw_src);
	...
	idtp9221_dbg(chip, PR_MAJOR, "OTP FW Rev: %02x %02x %02x %02x\n",
	...
	idtp9221_dbg(chip, PR_MAJOR, "EEPROM FW Rev: %02x %02x %02x %02x\n",
			chip->fw_rev_sram_major_l, chip->fw_rev_sram_major_h,
			chip->fw_rev_sram_minor_l, chip->fw_rev_sram_minor_h);

	idtp9221_dbg(chip, PR_MAJOR, "Wireless charger is activated\n");
...
#define VTHERM_JUMP_THRESHOLD 240
static int idtp9221_read_regs_each_cycle(struct idtp9221 *chip)
{
	...
	/*
	 * Workaround for Vtherm jump. Update Vtherm only if current Vtherm is
	 * roughly the same as last one. When attached at first from boot or
	 * in a long time, it might not be updated only once. It doesn't matter.
	 */
	if (chip->last_vtherm_adc &&
	    abs(vtherm_adc - chip->last_vtherm_adc) > VTHERM_JUMP_THRESHOLD) {
		idtp9221_dbg(chip, PR_MAJOR, "Vtherm jumped: 0x%04x->0x%04x\n",
					chip->last_vtherm_adc, vtherm_adc);
	} else {
		chip->vtherm_uv = IDTP9221_ADC_TO_VTHERM_UV(vtherm_adc);
		chip->vtherm_temp_dc =
			idtp9221_conv_vtherm_to_celsius(chip->vtherm_uv);
...
static void idtp9221_handle_halt_proc(struct idtp9221 *chip)
{
	...
	if (halt) {
		if (chip->is_worker_active) {
			...
			idtp9221_dbg(chip, PR_MAJOR,
					"wait for worker's end point\n");
			chip->halt_retry = true;
			return;
		}
		idtp9221_dbg(chip, PR_MAJOR, "Disable WLC_EN\n");
...
static ssize_t idtp9221_param_show(struct device *dev,
				struct device_attribute *attr,
				char *buf);

static ssize_t idtp9221_param_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size);

static struct device_attribute wireless_attrs[] = {
	__ATTR(interval_power_ok_ms,	0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(interval_negotiation_ms,	0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(monitor_for_debug,	0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(wlc_en,			0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(halt_for_debug,		0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(disable_halt,		0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(vout_set_mv,		0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(ilim_set_ma,		0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(fod_0_a,			0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(fod_0_b,			0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(fod_1_a,			0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(fod_1_b,			0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(fod_2_a,			0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(fod_2_b,			0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(fod_3_a,			0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(fod_3_b,			0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(fod_4_a,			0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(fod_4_b,			0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(fod_5_a,			0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(fod_5_b,			0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(fod_expansion_a,		0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(fod_expansion_b,		0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(fod_epp_mode_1_a,	0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(fod_epp_mode_1_b,	0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(fod_adjust_en,		0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(send_ept,		0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(cmd_reg_addr,		0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(cmd_reg_data,		0644, idtp9221_param_show,
						idtp9221_param_store),
	__ATTR(fw_rev,			0444, idtp9221_param_show, NULL),
	__ATTR(vrect_mv,		0444, idtp9221_param_show, NULL),
	__ATTR(vout_mv,			0444, idtp9221_param_show, NULL),
	__ATTR(iout_ma,			0444, idtp9221_param_show, NULL),
	__ATTR(die_temp_c,		0444, idtp9221_param_show, NULL),
	__ATTR(vtherm_uv,		0444, idtp9221_param_show, NULL),
	__ATTR(op_freq_khz,		0444, idtp9221_param_show, NULL),
	__ATTR(reg_ept,			0444, idtp9221_param_show, NULL),
	__ATTR(vtherm_temp_dc,		0444, idtp9221_param_show, NULL),
	__ATTR(sigstr_pc,		0444, idtp9221_param_show, NULL),
	__ATTR(rpp_mw,			0444, idtp9221_param_show, NULL),
	__ATTR(txid,			0444, idtp9221_param_show, NULL),
	__ATTR(ping_freq_khz,		0444, idtp9221_param_show, NULL),
};
				...
				const char *buf, size_t size)
{
	...
	case ATTR_HALT_FOR_DEBUG:
		...
		if (ret < 0)
			return ret;
		ret = idtp9221_handle_halt(chip, IDTP9221_ENABLE_CLIENT_DEBUG,
									val);
		if (ret < 0)
			return ret;
	...
	case ATTR_SEND_EPT:
		...
		if (ret < 0)
			return ret;
		idtp9221_dbg(chip, PR_MAJOR, "Send EPT Packet. mode:%d\n", val);
...
static int idtp9221_parse_dt(struct idtp9221 *chip)
{
	...
	rc = of_property_read_u32(node, "somc,vout-limit-batt-therm",
						&chip->vout_limit_thresh);

	chip->vout_limit_charge_full_en = of_property_read_bool(node,
...
static void idtp9221_detect_work(struct work_struct *work)
{
	...
	if (chip->status == IDTP9221_STATUS_PWR_OFF) {
		...
		chip->last_vtherm_adc = 0;
		idtp9221_dbg(chip, PR_MAJOR, "skip monitoring due to POFF\n");
		goto reg_done;
	}
	...
	if (chip->status == IDTP9221_STATUS_PWR_POWER_OK &&
		...
		if (chip->wireless_auth && chip->vout_limit_thresh &&
							chip->batt_psy) {
			...
			if (rc < 0)
				pr_err("Error in getting battery temp, rc=%d\n",
									rc);
			if (pval.intval > chip->vout_limit_thresh) {
				idtp9221_dbg(chip, PR_MISC,
					"Force Vout limit due to high temp\n");
				chip->vout_limit_by_high_temp = true;
			} else {
				chip->vout_limit_by_high_temp = false;
			}
		} else {
			chip->vout_limit_by_high_temp = false;
		}
	...
	if (chip->vout_limit_charge_full_en && chip->batt_psy) {
		rc = power_supply_get_property(chip->batt_psy,
					POWER_SUPPLY_PROP_STATUS, &pval);
		if (rc < 0)
			pr_err("Error in getting charge status, rc=%d\n", rc);
		if (pval.intval == POWER_SUPPLY_STATUS_FULL &&
					!chip->vout_limit_by_charge_full) {
			idtp9221_dbg(chip, PR_MISC,
					"Force Vout limit due to FULL\n");
			chip->vout_limit_by_charge_full = true;
		}

		if (pval.intval != POWER_SUPPLY_STATUS_FULL &&
					chip->vout_limit_by_charge_full)
			chip->vout_limit_by_charge_full = false;
	} else {
		chip->vout_limit_by_charge_full = false;
	}
...
static irqreturn_t idtp9221_irq_handler(int irq, void *data)
{
	...
	idtp9221_dbg(chip, PR_MAJOR, "irq_status=%d\n", stat);
