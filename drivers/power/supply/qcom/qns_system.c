...
static struct class_attribute qns_attrs[] = {
	__ATTR(charging_state, 0444, qns_param_show, NULL),
	__ATTR(current_now, 0444, qns_param_show, NULL),
	__ATTR(voltage, 0444, qns_param_show, NULL),
	__ATTR(temp, 0444, qns_param_show, NULL),
	__ATTR(fcc, 0444, qns_param_show, NULL),
	__ATTR(design, 0444, qns_param_show, NULL),
	__ATTR(soc, 0444, qns_param_show, NULL),
#ifdef DEBUG	
	__ATTR(battery_type, 0644, qns_param_show, qns_param_store),
#else
	__ATTR(battery_type, 0444, qns_param_show, NULL),
#endif	
	__ATTR(charge_current, 0200, NULL, qns_param_store),
	__ATTR(charge_voltage, 0200, NULL, qns_param_store),
	__ATTR(alarm, 0644, qns_param_show, qns_param_store),
	__ATTR(options, 0644, qns_param_show, qns_param_store),
	__ATTR_NULL,
};
        ...
        size_t count)
{
	...
	case BATTERY_TYPE:
		strcpy(battid , buf);
		pr_info("QNS: received '%s'\n", battid);
		val = strlen(battid);
		if (val > 0 && val < 32)
			battid[val-1] = 0;
		pr_info("QNS: new id '%s'\n", battid);
		break;
#endif		
	}
	return count;
