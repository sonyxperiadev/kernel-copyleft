...
static int somc_fg_aging_setting(struct fg_chip *chip, u8 threshold, bool mode)
{
	int rc = 0;
	...
	rc = fg_set_resume_soc(chip, threshold);
	if (rc) {
...
#define RESUME_SOC_AGING_VAL	0xE3
				...
				int nom_cap_uah, int *thresh)
{
	...
	if (params->aging_data.batt_aging ||
		...
		if (rc) {
			...
			*thresh = RESUME_SOC_AGING_VAL;
		}
	} else {
		if (!params->aging_data.aging_mode)
			return;
		pr_fg_ext(PR_INFO, "stop aging mode\n");
		*thresh = params->aging_data.resume_soc_nomal_val;
		rc = somc_fg_aging_setting(chip, *thresh, false);
		if (rc)
			pr_err("failed aging setting rc=%d\n", rc);
		else
...
static int somc_fg_register(struct fg_chip *chip)
{
	...
	params->aging_data.resume_soc_nomal_val =
