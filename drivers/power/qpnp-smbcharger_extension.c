...
static int somc_chg_get_fv_cmp_cfg(struct smbchg_chip *chip)
{
	...
	ret = smbchg_read(chip, &reg, chip->chgr_base + VFLOAT_CMP_CFG_REG, 1);
	if (ret) {
...
static int somc_chg_therm_get_therm_mitigation(struct smbchg_chip *chip)
{
	...
	if (somc_chg_is_charging_hvdcp(chip)) {
		/* 9V */
		therm_ma =
		  (int)params->thermal.usb_9v[therm_lvl];
		pr_smb_ext(PR_THERM,
			"USB 9V[level:%d]: Input current value=%d\n",
			therm_lvl, therm_ma);
	} else {
		/* 5V */
		therm_ma =
		...
		pr_smb_ext(PR_THERM,
			"USB 5V[level:%d]: Input current value=%d\n",
			therm_lvl, therm_ma);
	}
	return therm_ma;
}
...
static void somc_chg_apsd_rerun(struct smbchg_chip *chip)
{
	...
	cancel_delayed_work_sync(&params->apsd.rerun_w);
...
static int somc_chg_invalid_set_state(struct chg_somc_params *params,
								int status)
{
	pr_info("%d\n", !!status);
			...
			struct device_node *node)
{
	...
	if (params->thermal.usb_9v_current_max)
