...
int somc_chg_apsd_rerun(struct chg_somc_params *params, int rerun)
{
	int ret = 0;
	dev_dbg(params->dev, "APSD rerun=%d\n", rerun);
	if (rerun)
		ret = somc_chg_apsd_wait_rerun(params->dev,
						*params->misc_base,
						*params->usb_chgpth_base);
	return ret;
}
