...
int dsi_panel_driver_gpio_request(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(spec_pdata->vci_en_gpio)) {
		rc = gpio_request(spec_pdata->vci_en_gpio,
							"vci_en_gpio");
		if (rc) {
			pr_err("request for vci_en gpio failed, rc=%d\n", rc);
			goto error_release_vci_en;
		}
	}

	if (gpio_is_valid(spec_pdata->touch_vddio_en_n_gpio)) {
		rc = gpio_request(spec_pdata->touch_vddio_en_n_gpio,
							"touch_vddio_en_n_gpio");
		if (rc) {
			pr_err("request for touch vddio_en gpio failed, rc=%d\n", rc);
			goto error_release_touch_vddio_en_n;
		}
	}
	...
	if (gpio_is_valid(spec_pdata->touch_vddh_en_gpio)) {
		rc = gpio_request(spec_pdata->touch_vddh_en_gpio,
							"touch_vddh_en_gpio");
		if (rc) {
			pr_err("request for touch vddh_en gpio failed, rc=%d\n", rc);
...
error_release_touch_vddio_en_n:
	if (gpio_is_valid(spec_pdata->touch_vddio_en_n_gpio))
		gpio_free(spec_pdata->touch_vddio_en_n_gpio);

error_release_vci_en:
	if (gpio_is_valid(spec_pdata->vci_en_gpio))
		gpio_free(spec_pdata->vci_en_gpio);
...
int dsi_panel_driver_post_power_off(struct dsi_panel *panel)
{
	...
	if (rc)
		pr_err("unable to set dir for touch vddh_en gpio rc=%d\n", rc);
	if (gpio_is_valid(spec_pdata->touch_vddh_en_gpio)) {
		gpio_set_value(spec_pdata->touch_vddh_en_gpio, 0);
		usleep_range(spec_pdata->touch_vddh_off * 1000,
				spec_pdata->touch_vddh_off * 1000 + 100);
	}

	rc = gpio_direction_output(spec_pdata->touch_vddio_en_n_gpio, 0);
	if (rc)
		pr_err("unable to set dir for touch vddio_en gpio rc=%d\n", rc);
	if (gpio_is_valid(spec_pdata->touch_vddio_en_n_gpio))
		gpio_set_value(spec_pdata->touch_vddio_en_n_gpio, 1);

	rc = gpio_direction_output(spec_pdata->vci_en_gpio, 1);
	if (rc)
		pr_err("unable to set dir for vci_en gpio rc=%d\n", rc);
	if (gpio_is_valid(spec_pdata->vci_en_gpio))
		gpio_set_value(spec_pdata->vci_en_gpio, 0);
...
int dsi_panel_driver_pre_power_on(struct dsi_panel *panel)
{
	...
	rc = gpio_direction_output(spec_pdata->vci_en_gpio, 0);
	if (rc)
		pr_err("unable to set dir for vci_en gpio rc=%d\n", rc);
	if (gpio_is_valid(spec_pdata->vci_en_gpio))
		gpio_set_value(spec_pdata->vci_en_gpio, 1);

	rc = gpio_direction_output(spec_pdata->touch_vddio_en_n_gpio, 1);
	if (rc)
		pr_err("unable to set dir for touch vddio_en gpio rc=%d\n", rc);
	if (gpio_is_valid(spec_pdata->touch_vddio_en_n_gpio))
		gpio_set_value(spec_pdata->touch_vddio_en_n_gpio, 0);

	rc = gpio_direction_output(spec_pdata->touch_vddh_en_gpio, 0);
	if (rc)
		pr_err("unable to set dir for touch vddh_en gpio rc=%d\n", rc);
	if (gpio_is_valid(spec_pdata->touch_vddh_en_gpio)) {
		gpio_set_value(spec_pdata->touch_vddh_en_gpio, 1);
		usleep_range(spec_pdata->touch_vddh * 1000,
			spec_pdata->touch_vddh * 1000 + 100);
	}
					...
					struct device_node *np)
{
	...
	spec_pdata->touch_vddh = !rc ? tmp : 0;
...
int dsi_panel_driver_parse_gpios(struct dsi_panel *panel)
{
	...
	spec_pdata->vci_en_gpio = of_get_named_gpio(panel->panel_of_node,
					      "somc,panel_vci",
					      0);
	if (!gpio_is_valid(spec_pdata->vci_en_gpio)) {
	...
	spec_pdata->touch_vddio_en_n_gpio = of_get_named_gpio(panel->panel_of_node,
					      "somc,ts_vddio",
					      0);
	if (!gpio_is_valid(spec_pdata->touch_vddio_en_n_gpio)) {
		pr_err("%s: failed get touch vddio enable gpio\n", __func__);
	}

	spec_pdata->touch_vddh_en_gpio = of_get_named_gpio(panel->panel_of_node,
					      "somc,ts_vddh",
					      0);
	if (!gpio_is_valid(spec_pdata->touch_vddh_en_gpio)) {
		pr_err("%s: failed get touch vddh enable gpio\n", __func__);
	}
