...
int dsi_panel_driver_pre_power_on(struct dsi_panel *panel)
{
	...
	spec_pdata = panel->spec_pdata;
	if (spec_pdata->pre_sod_mode && (incell->state & POWER_ON_MASK)) {
		pr_notice("%s: Panel reset\n", __func__);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (spec_pdata->sod_mode != mode) {
		pr_info("%s: current %d request %d\n", __func__, spec_pdata->sod_mode, mode);
		if (mode == SOD_POWER_ON && spec_pdata->display_onoff_state) {
			...
			spec_pdata->sod_mode = mode;
			display_sod_mode = spec_pdata->sod_mode;
		} else if (mode == SOD_POWER_OFF && spec_pdata->display_onoff_state && spec_pdata->aod_mode) {
			pr_info("%s: power off\n", __func__);
			spec_pdata->sod_mode = mode;
			display_sod_mode = spec_pdata->sod_mode;
			dsi_panel_driver_notify_suspend(panel);
		} else if (mode == SOD_POWER_OFF && !spec_pdata->display_onoff_state) {
			pr_info("%s: power off\n", __func__);
			spec_pdata->sod_mode = mode;
			display_sod_mode = spec_pdata->sod_mode;
			dsi_panel_driver_notify_suspend(panel);
