...
int dsi_panel_driver_power_off(struct dsi_panel *panel)
{
	...
	spec_pdata = panel->spec_pdata;
	if (spec_pdata->sod_mode == SOD_POWER_OFF_SKIP) {
...
int dsi_panel_driver_post_power_off(struct dsi_panel *panel)
{
	...
	if (spec_pdata->oled_disp) {
		rc = dsi_panel_driver_vreg_ctrl(&spec_pdata->touch_power_info, "touch-avdd",
										false);
		if (rc)
...
int dsi_panel_driver_pre_power_on(struct dsi_panel *panel)
{
	...
	if (spec_pdata->oled_disp) {
		rc = dsi_panel_driver_vreg_ctrl(&spec_pdata->touch_power_info,
						"touch-avdd", true);
		if (rc) {
					...
					struct device_node *np)
{
	...
	rc = of_property_read_u32(np,
			"somc,pw-wait-after-off-lp11", &tmp);
	...
	if (spec_pdata->area_count_table_size > 0) {
		...
		spec_pdata->area_count = kzalloc(sizeof(unsigned long) * spec_pdata->area_count_table_size, GFP_KERNEL);
...
static void dsi_panel_driver_notify_sod_resume(struct dsi_panel *panel)
{
	struct drm_ext_event event;
	int blank = DRM_BLANK_UNBLANK;
	event.data = &blank;
								...
								bool enable)
{
	...
	if (panel->spec_pdata->oled_disp) {
		if (enable)
			...
			dsi_panel_driver_oled_short_det_disable(
							panel->spec_pdata);
	}
}
	...
	if (aod_first_skipped) {
		if (mode) {
			...
			for (retry = 0; retry < 10; retry++) {
				gpio_set_value(spec_pdata->reset_touch_gpio, 1);
				if (gpio_get_value(spec_pdata->reset_touch_gpio) != 1)
					msleep(5);
				else
					break;
			}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((mode == SOD_POWER_OFF && !spec_pdata->sod_mode && !spec_pdata->aod_mode)) {
		pr_info("%s: not support request sod %d "
			"current sod %d current aod %d\n", __func__,
			mode, spec_pdata->sod_mode, spec_pdata->aod_mode);
		return count;
	}

	if (spec_pdata->sod_mode != mode) {
		pr_info("%s: requested sod mode %d "
			"current sod %d current aod %d incell->state 0x%02x\n", __func__,
			mode, spec_pdata->sod_mode, spec_pdata->aod_mode, incell->state);
		if (mode == SOD_POWER_OFF_SKIP && spec_pdata->sod_mode == SOD_POWER_OFF && !spec_pdata->aod_mode) {
			if (incell->state & SYSTEM_ON_MASK) {
				pr_info("%s: no power off \n", __func__);
				dsi_panel_driver_notify_sod_resume(panel);
			}
		} else if (mode == SOD_POWER_OFF_SKIP && spec_pdata->sod_mode == SOD_POWER_OFF && spec_pdata->aod_mode) {
			pr_info("%s: power off\n", __func__);
		...
		struct device_attribute *attr, char *buf)
{
	...
	memset(spec_pdata->area_count, 0, sizeof(unsigned long) * spec_pdata->area_count_table_size);
	mutex_unlock(&display->display_lock);
...
void dsi_panel_driver_low_touch_vddio(struct dsi_panel *panel)
{
	struct panel_specific_pdata *spec_pdata = NULL;
	...
	spec_pdata = panel->spec_pdata;
	gpio_set_value(spec_pdata->touch_vddio_en_gpio, 0);
}
