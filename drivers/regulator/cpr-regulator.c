			       ...
			       struct cpr_regulator *cpr_vreg)
{
	...
	{
		//bump up APC voltage by 37.5 mV for testing upon per corner ceiling and floor voltages.
		cpr_vreg->pvs_corner_v[i] += 37500;
				     ...
				     struct cpr_regulator *cpr_vreg)
{
	...
	for (i = CPR_FUSE_CORNER_MIN; i <= cpr_vreg->num_fuse_corners; i++) {
		...
		// Bump up the quotient by 37.5mV
		cpr_vreg->cpr_fuse_target_quot[i] += 78;
		cpr_info(cpr_vreg,
