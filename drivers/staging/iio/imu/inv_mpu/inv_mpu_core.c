...
static int autosuspend_delay_ms = 500;
	...
	u16 i2c_addr, u8 reg, u8 data)
{
	...
	if (reg < st->hw->num_reg) {
		st->reg_shadow[reg] = data;
		st->write_back[reg] = 1;
	} else {
		dev_err(&st->client->dev, "addr 0x%x out of range\n", reg);
	}
	return 0;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	case ATTR_SELF_TEST:
		if (st->chip_config.enable)
			return -EBUSY;
		mutex_lock(&indio_dev->mlock);
		/* MPU3050 does not support hw test
		Just return OK for gyro and accel if it is connected */
		if (INV_MPU3050 == st->chip_type) {
...
static int inv_set_chip_type(struct inv_mpu_iio_s *st,
	...
	const struct i2c_device_id *id)
{
	...
	result = inv_set_chip_type(st, id);
	if (result) {
		dev_err(&client->dev, "%s: Chip type not supported\n",
				__func__);
		result = -EINVAL;
		goto out_free_iio;
	}

	st->reg_shadow = kzalloc(st->hw->num_reg  * 2, GFP_KERNEL);
	if (!st->reg_shadow) {
		dev_err(&client->dev, "%s: No memory\n", __func__);
		result = -ENOMEM;
		goto out_free_iio;
	}
	st->write_back = st->reg_shadow + st->hw->num_reg;
...
static void inv_mpu_restore_regs(struct inv_mpu_iio_s *st)
{
	int ii;
	dev_dbg(&st->client->dev, "%s %d registers\n",
			__func__, st->hw->num_reg);
	for (ii = 0; ii < st->hw->num_reg; ii++) {
		if (ii != st->reg.fifo_r_w && st->write_back[ii]) {
			st->write_back[ii] = 0;
			inv_i2c_single_write(st, ii, st->reg_shadow[ii]);
		}
	}
}

static int inv_mpu_power_supply(struct inv_mpu_iio_s *st, bool power_on)
{
	int result = 0;

	dev_dbg(&st->client->dev, "%s: %d\n", __func__, power_on);

	if (power_on && !st->power_supply_on) {
		result = !st->plat_data.power_supply ? 0 :
			st->plat_data.power_supply(&st->client->dev, 1);
		if (result)
			return result;
		result = !st->plat_data.secondary_power_supply ? 0 :
			st->plat_data.secondary_power_supply(&st->client->dev,
					1);
		if (result)
			return result;
...
static int inv_mpu_runtime_suspend(struct device *dev)
{
	...
	int rc = inv_mpu_power_supply(st, 0);
	dev_dbg(dev, "%s\n", __func__);
	return rc ? -EAGAIN : 0;
}
...
static int inv_mpu_runtime_resume(struct device *dev)
{
	struct inv_mpu_iio_s *st =
			iio_priv(i2c_get_clientdata(to_i2c_client(dev)));
	int rc = inv_mpu_power_supply(st, 1);
	dev_dbg(dev, "%s\n", __func__);
	return rc ? -EAGAIN : 0;
}
#endif
...
static int inv_mpu_suspend(struct device *dev)
{
	...
	if (!pm_runtime_suspended(dev)) {
		...
		dev_info(dev, "%s: runtime-suspended.\n", __func__);
		result = 0;
	}
	return result;
}
...
static const struct dev_pm_ops inv_mpu_pmops = {
	...
	SET_RUNTIME_PM_OPS(inv_mpu_runtime_suspend,
		inv_mpu_runtime_resume, NULL)
};
