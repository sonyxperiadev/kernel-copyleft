		...
		int *gyro_result, int *accl_result)
{
	...
	if (INV_MPU3050 == st->chip_type)
		result = inv_i2c_single_write(st, reg->lpf,
				(INV_FILTER_98HZ | DEF_SELFTEST_GYRO_FS));
	else
	...
	if (INV_MPU3050 == st->chip_type) {
		if (st->mpu_slave != NULL) {
			...
			if (result)
				return result;
			/* Bandwidth: same as MPU  */
			result = st->mpu_slave->set_lpf(st,
					1000 / (DEF_SELFTEST_SAMPLE_RATE + 1));
			if (result)
				return result;
		}
	}
	...
	if (INV_MPU3050 == st->chip_type)
		result = inv_i2c_single_write(st, reg->user_ctrl,
				(BIT_FIFO_EN | BIT_3050_AUX_IF_EN));
	else
	...
	if (has_accl) {
		if (INV_MPU3050 == st->chip_type)
			d = BITS_GYRO_OUT | BITS_3050_ACCL_OUT;
		else
	...
	while (s < st->self_test.samples) {
		...
		while ((i < packet_count) && (s < st->self_test.samples)) {
			...
			if (INV_MPU3050 == st->chip_type) {
				...
				pr_debug(
				"%s self_test gyro data - %d %+d %+d %+d",
				 st->hw->name, s, vals[0], vals[1], vals[2]);

				if (has_accl) {
					st->mpu_slave->combine_data(&data[ind],
						vals);
					for (j = 0; j < THREE_AXIS; j++)
						accl_result[j] += vals[j];
					pr_debug(
						"%s self_test accel data" \
						" - %d %+d %+d %+d",
						st->hw->name, s, vals[0],
						vals[1], vals[2]);
				}
				...
				pr_debug("%s self_test gyro data" \
					" - %d %+d %+d %+d",
					 st->hw->name, s, vals[0],
					 vals[1], vals[2]);
			}
...
void inv_recover_setting(struct inv_mpu_iio_s *st)
{
	...
	if (INV_MPU3050 == st->chip_type)
		inv_i2c_single_write(st, reg->lpf,
				(st->chip_config.fsr << GYRO_CONFIG_FSR_SHIFT) |
				st->chip_config.lpf);
	else
	...
	if (INV_MPU3050 == st->chip_type) {
		if (st->mpu_slave != NULL) {
			st->mpu_slave->set_lpf(st,
					st->chip_config.new_fifo_rate);
			st->mpu_slave->set_fs(st, st->chip_config.accl_fs);
		}
	}
