	...
	const struct i2c_device_id *id)
{
	...
	if (st->plat_data.power_supply)
		st->plat_data.power_supply(&client->dev, 1);
	...
	if (st->plat_data.power_supply)
		st->plat_data.power_supply(&client->dev, 0);
...
static int ak89xx_runtime_suspend(struct device *dev)
{
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
	struct inv_ak89xx_state_s *st = iio_priv(indio_dev);
	int rc = ak89xx_power_supply(st, 0);
	dev_dbg(dev, "%s\n", __func__);
	return rc ? -EAGAIN : 0;
}

static int ak89xx_runtime_resume(struct device *dev)
{
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
	struct inv_ak89xx_state_s *st = iio_priv(indio_dev);
	int rc = ak89xx_power_supply(st, 1);
	dev_dbg(dev, "%s\n", __func__);
	return rc ? -EAGAIN : 0;
}
#endif
...
static int ak89xx_suspend(struct device *dev)
{
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
	struct inv_ak89xx_state_s *st = iio_priv(indio_dev);
	int rc;
