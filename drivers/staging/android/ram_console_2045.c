	...
	char *dest)
{
	...
	memcpy_fromio(ram_console_old_log,
	       &buffer->data[buffer->start], buffer->size - buffer->start);
	memcpy_fromio(ram_console_old_log + buffer->size - buffer->start,
...
//CORE-KH-DebugToolPorting-01-d		entry->proc_fops = &ram_console_file_ops;
