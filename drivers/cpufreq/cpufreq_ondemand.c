...
#ifdef CONFIG_CPU_FREQ_GOV_OND_OPT
...
static unsigned int sampling_rate_before_boosted;
#endif
...
static struct dbs_tuners {
	...
	unsigned int freq_1st_step;
	unsigned int freq_2nd_step;
#endif
} dbs_tuners_ins = {
...
show_one(freq_1st_step, freq_1st_step);
show_one(freq_2nd_step, freq_2nd_step);
#endif
...
#ifdef CONFIG_CPU_FREQ_GOV_OND_OPT
	sampling_rate_before_boosted = dbs_tuners_ins.sampling_rate;
#endif
	return count;
}
...
static ssize_t store_freq_1st_step(struct kobject *a, struct attribute *b,
				   const char *buf, size_t count)
{
	unsigned int input;
	int ret;
	...
	if (ret != 1)
		return -EINVAL;
	dbs_tuners_ins.freq_1st_step = input;
	return count;
}

static ssize_t store_freq_2nd_step(struct kobject *a, struct attribute *b,
				   const char *buf, size_t count)
{
	unsigned int input;
	int ret;
	...
	if (ret != 1)
		return -EINVAL;
	dbs_tuners_ins.freq_2nd_step = input;
	return count;
}
#endif
...
define_one_global_rw(freq_1st_step);
define_one_global_rw(freq_2nd_step);
#endif
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	if (max_load_freq > dbs_tuners_ins.up_threshold * policy->cur) {
		...
		freq_target = policy->cur
			+ (dbs_tuners_ins.freq_step * policy->max) / 100;

		if (policy->cur < dbs_tuners_ins.freq_1st_step)
			freq_target = dbs_tuners_ins.freq_1st_step;
		else if (policy->cur < dbs_tuners_ins.freq_2nd_step
				&& freq_target > dbs_tuners_ins.freq_2nd_step)
			freq_target = dbs_tuners_ins.freq_2nd_step;
...
{
#ifndef CONFIG_CPU_FREQ_GOV_OND_OPT
	int i;
#endif
...
#ifdef CONFIG_CPU_FREQ_GOV_OND_OPT
	/*
	 *	Shorten sampling rate and not call dbs_refresh_callback
	 *	for quick response
	 */
	if (!boost_flag) {
		boost_end_time = ktime_to_us(ktime_get())
						+ DEF_BOOSTED_DURATION;
		dbs_tuners_ins.sampling_rate = DEF_BOOSTED_SAMPLING_RATE;
		boost_flag = 1;
	}
#else
	for_each_online_cpu(i)
