...
static void dbs_reset_sample(struct cpu_dbs_info_s *p_dbs_info)
{
	...
	queue_delayed_work_on(p_dbs_info->cur_policy->cpu, dbs_wq,
...
#endif
		/*
		 * Maybe some CPUs use the same cpufreq_policy,
		 * just need to refresh once for these CPUs.
		 */
		if (per_cpu(od_cpu_dbs_info, i).cur_policy->cpu == i)
			queue_work_on(i, dbs_wq,
