...
#define DMSG(msg, ...) pr_info(msg, ##__VA_ARGS__)
#else
#define DMSG(msg, ...) pr_info_ratelimited(msg, ##__VA_ARGS__)
#endif
...
void wcnss_pronto_log_debug_regs(void)
{
	...
	reg = readl_relaxed(reg_addr);
	DMSG("%s:  PRONTO_PMU_COM_GDSCR %08x\n",
						__func__, reg);
	reg >>= 31;
	...
	DMSG("%s:  Read data FIFO testbus %08x\n",
					__func__, reg);
	...
	writel_relaxed(reg, tst_ctrl_addr);
	reg = readl_relaxed(tst_addr);
	DMSG("%s:  Command FIFO testbus %08x\n",
					__func__, reg);
	...
	writel_relaxed(reg, tst_ctrl_addr);
	reg = readl_relaxed(tst_addr);
	DMSG("%s:  Rrite data FIFO testbus %08x\n",
					__func__, reg);
	...
	writel_relaxed(reg, tst_ctrl_addr);
	reg = readl_relaxed(tst_addr);
	DMSG("%s:  AXIM SEL CFG0 testbus %08x\n",
					__func__, reg);
	...
	writel_relaxed(reg, tst_ctrl_addr);
	reg = readl_relaxed(tst_addr);
	DMSG("%s:  AXIM SEL CFG1 testbus %08x\n",
					__func__, reg);
	...
	writel_relaxed(reg, tst_ctrl_addr);
	reg = readl_relaxed(tst_addr);
	DMSG("%s:  CTRL SEL CFG0 testbus %08x\n",
					__func__, reg);
	...
	writel_relaxed(reg, tst_ctrl_addr);
	reg = readl_relaxed(tst_addr);
	DMSG("%s:  CTRL SEL CFG1 testbus %08x\n", __func__, reg);
	...
	if ((reg & PRONTO_PMU_WLAN_BCR_BLK_ARES) ||
		...
		DMSG("%s:  Cannot log, wlan domain is power collapsed\n",
				__func__);
		return;
	}
	...
	DMSG("%s: WLAN_BRDG_ERR_SOURCE %08x\n", __func__, reg);

	reg = readl_relaxed(penv->wlan_tx_status);
	DMSG("%s: WLAN_TX_STATUS %08x\n", __func__, reg);
