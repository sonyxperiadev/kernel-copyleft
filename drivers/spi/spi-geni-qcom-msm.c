...
struct geni_spi_desc {
	...
	int (*geni_spi_pwr_rsc_init)(struct device *dev);
	int (*geni_spi_set_rate)(struct device *dev, unsigned long clk_freq);
	int (*geni_spi_switch_power_state)(struct device *dev, bool state);
};
...
static int geni_spi_set_clock_and_bw(struct device *dev,
...
static int geni_spi_resources_on(struct spi_geni_master *mas)
{
	int ret;
...
static int geni_spi_resources_state(struct device *dev, bool power_on)
{
	struct spi_controller *spi = dev_get_drvdata(dev);
	struct spi_geni_master *mas = spi_controller_get_devdata(spi);

	return power_on ? geni_spi_resources_on(mas) : geni_spi_resources_off(mas);
}

static int geni_spi_pwr_init(struct device *dev)
{
	struct spi_controller *spi = dev_get_drvdata(dev);
	struct spi_geni_master *mas = spi_controller_get_devdata(spi);
	int ret;

	ret = dev_pm_domain_attach_list(mas->se.dev,
					&mas->dev_data->pd_data, &mas->pd_list);
	if (ret <= 0)
		return -EPROBE_DEFER;
...
static int geni_spi_resource_init(struct device *dev)
{
	struct spi_controller *spi = dev_get_drvdata(dev);
	struct spi_geni_master *mas = spi_controller_get_devdata(spi);
	int ret;
...
static int __maybe_unused spi_geni_runtime_suspend(struct device *dev)
{
	...
	return mas->dev_data->geni_spi_switch_power_state ?
	       mas->dev_data->geni_spi_switch_power_state(dev, false) : 0;
}
...
static int __maybe_unused spi_geni_runtime_resume(struct device *dev)
{
	...
	return mas->dev_data->geni_spi_switch_power_state ?
	       mas->dev_data->geni_spi_switch_power_state(dev, true) : 0;
}
...
static const struct geni_spi_desc geni_spi = {
	.geni_spi_pwr_rsc_init = geni_spi_resource_init,
	.geni_spi_set_rate = geni_spi_set_clock_and_bw,
	.geni_spi_switch_power_state = geni_spi_resources_state,
};
