				...
				struct spi_transfer *xfer)
{
	...
	/*
	 * T1 guard time for between CS and Clock signal,
	 * the guard time should not be lower than 1 clock cycle
	 */
	usleep_range(10, 15);
	...
	/*
	 * When data transfer is success,
	 * T3 guard time is required to let the CS HIGH before pulling the
	 * the CHIP Select line to LOW again,
	 * the guard time should not be lower than 1ms
	 */
...
static int geni_spi_ese_cs_and_pwr_parse_dt(struct platform_device *pdev,
				struct spi_geni_master *geni_mas)
{
	int ret = 0;

	geni_mas->ese_pwr_gpio = of_get_named_gpio_flags(pdev->dev.of_node,
			"nxp,ese-pwr", 0, NULL);
	if ((!gpio_is_valid(geni_mas->ese_pwr_gpio))) {
		ret = -EINVAL;
		dev_err(&pdev->dev, "Cannot retrieve eSE PWR gpio\n");
		goto err_exit;
	}

	geni_mas->ese_cs_gpio = of_get_named_gpio_flags(pdev->dev.of_node,
			"nxp,ese-cs", 0, NULL);
	if ((!gpio_is_valid(geni_mas->ese_cs_gpio))) {
		ret = -EINVAL;
		dev_err(&pdev->dev, "Cannot retrieve eSE CS gpio\n");
		goto err_exit;
	}

	ret = gpio_direction_output(geni_mas->ese_pwr_gpio, 0);
	if (ret < 0) {
		dev_err(&pdev->dev, "Not able to set eSE PWR gpio as output\n");
		goto err_exit;
	}
	ret = gpio_direction_output(geni_mas->ese_cs_gpio, 0);
	if (ret < 0) {
		dev_err(&pdev->dev, "Not able to set eSE CS gpio as output\n");
		goto err_cs_pwr;
	}
...
err_cs_pwr:
	gpio_free(geni_mas->ese_pwr_gpio);
err_exit:
	return ret;
}
...
static int spi_geni_probe(struct platform_device *pdev)
{
	...
	ret = devm_request_irq(&pdev->dev, geni_mas->irq, geni_spi_irq,
			       IRQF_TRIGGER_HIGH, "spi_geni_cloned", geni_mas);
	if (ret) {
	...
	ret = geni_spi_ese_cs_and_pwr_parse_dt(pdev, geni_mas);
	if (ret) {
		dev_err(&pdev->dev, "Err parsing NXP dt: err:%d\n", ret);
		goto spi_geni_probe_unmap;
	}
...
static int spi_geni_remove(struct platform_device *pdev)
{
	...
	gpio_free(geni_mas->ese_pwr_gpio);
	gpio_free(geni_mas->ese_cs_gpio);
...
static int spi_geni_runtime_suspend(struct device *dev)
{
	...
	gpio_set_value(geni_mas->ese_cs_gpio, 0);
	gpio_set_value(geni_mas->ese_pwr_gpio, 0);
...
MODULE_ALIAS("platform:spi_geni_cloned");
