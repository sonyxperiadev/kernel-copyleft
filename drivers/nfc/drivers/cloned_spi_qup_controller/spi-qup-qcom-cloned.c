				...
				struct spi_transfer *xfer)
{
	...
	gpio_set_value(dd->ese_cs_gpio, 0);
	/*
	...
	gpio_set_value(dd->ese_cs_gpio, 1);
	/*
...
static int qup_spi_ese_cs_and_pwr_parse_dt(struct platform_device *pdev,
				struct msm_spi *dd)
{
	int ret = 0;

	dd->ese_pwr_gpio = of_get_named_gpio_flags(pdev->dev.of_node,
			"nxp,ese-pwr", 0, NULL);
	if ((!gpio_is_valid(dd->ese_pwr_gpio))) {
		ret = -EINVAL;
		dev_err(&pdev->dev, "Cannot retrieve eSE PWR gpio\n");
		goto err_exit;
	}

	dd->ese_cs_gpio = of_get_named_gpio_flags(pdev->dev.of_node,
			"nxp,ese-cs", 0, NULL);
	if ((!gpio_is_valid(dd->ese_cs_gpio))) {
		ret = -EINVAL;
		dev_err(&pdev->dev, "Cannot retrieve eSE CS gpio\n");
		goto err_exit;
	}

	ret = gpio_direction_output(dd->ese_pwr_gpio, 0);
	if (ret < 0) {
		dev_err(&pdev->dev, "Not able to set eSE PWR gpio as output\n");
		goto err_exit;
	}
	ret = gpio_direction_output(dd->ese_cs_gpio, 0);
	if (ret < 0) {
...
static int msm_spi_probe(struct platform_device *pdev)
{
	...
	rc = qup_spi_ese_cs_and_pwr_parse_dt(pdev, dd);
	if (rc) {
...
static int msm_spi_pm_suspend_runtime(struct device *device)
{
	...
	gpio_set_value(dd->ese_cs_gpio, 0);
	gpio_set_value(dd->ese_pwr_gpio, 0);
...
static int msm_spi_pm_resume_runtime(struct device *device)
{
	...
	gpio_set_value(dd->ese_pwr_gpio, 1);
