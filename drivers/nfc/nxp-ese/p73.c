...
/* NFC reserved controller */
#define NFCC_RESERVED_R00 "nxp,nfcc-reserved-r00"
#define NFCC_RESERVED_R01 "nxp,nfcc-reserved-r01"
#define NFCC_RESERVED_R02 "nxp,nfcc-reserved-r02"
#define NFCC_RESERVED_R03 "nxp,nfcc-reserved-r03"
...
struct p61_dev {
	...
	const char *nfcc_name_reserved_r00;
        const char *nfcc_name_reserved_r01;
        const char *nfcc_name_reserved_r02;
        const char *nfcc_name_reserved_r03;
};
...
*/
	if (!nfc_dev) {
		    P61_ERR_MSG("%s: cannot find NFC controller '%s'\n", __func__, p61_dev->nfcc_name);
                    nfc_dev = bus_find_device_by_name(&i2c_bus_type, NULL, p61_dev->nfcc_name_reserved_r00);
		if (!nfc_dev) {
		    P61_ERR_MSG("%s: cannot find NFC controller '%s'\n", __func__, p61_dev->nfcc_name_reserved_r00);
                    nfc_dev = bus_find_device_by_name(&i2c_bus_type, NULL, p61_dev->nfcc_name_reserved_r01);
                        if (!nfc_dev) {
			    P61_ERR_MSG("%s: cannot find NFC controller '%s'\n", __func__, p61_dev->nfcc_name_reserved_r01);
                            nfc_dev = bus_find_device_by_name(&i2c_bus_type, NULL, p61_dev->nfcc_name_reserved_r02);
                                if (!nfc_dev) {
			            P61_ERR_MSG("%s: cannot find NFC controller '%s'\n", __func__, p61_dev->nfcc_name_reserved_r02);
                                    nfc_dev = bus_find_device_by_name(&i2c_bus_type, NULL, p61_dev->nfcc_name_reserved_r03);
                                    if (!nfc_dev) {
                                        P61_ERR_MSG("%s: cannot find NFC controller '%s'\n", __func__, p61_dev->nfcc_name_reserved_r03);
                                        return -ENODEV;
                                    } else {
                                        P61_ERR_MSG("%s: NFC controller found, nfcc_name_reserved_r03\n", __func__);
                                        memcpy(&p61_dev->nfcc_name, &p61_dev->nfcc_name_reserved_r03, sizeof(p61_dev->nfcc_name_reserved_r03));
                                    }
                                } else {
                                    P61_ERR_MSG("%s: NFC controller found, nfcc_name_reserved_r02\n", __func__);
                                    memcpy(&p61_dev->nfcc_name, &p61_dev->nfcc_name_reserved_r02, sizeof(p61_dev->nfcc_name_reserved_r02));
                                }
                        } else {
                            P61_ERR_MSG("%s: NFC controller found, nfcc_name_reserved_r01\n", __func__);
                            memcpy(&p61_dev->nfcc_name, &p61_dev->nfcc_name_reserved_r01, sizeof(p61_dev->nfcc_name_reserved_r01));
                        } 
                }else {
                            P61_ERR_MSG("%s: NFC controller found, nfcc_name_reserved_r01\n", __func__);
                            memcpy(&p61_dev->nfcc_name, &p61_dev->nfcc_name_reserved_r00, sizeof(p61_dev->nfcc_name_reserved_r00));
                      }	 
        }

                P61_ERR_MSG("%s: NFC controller found, nfcc_name='%s'\n", __func__, p61_dev->nfcc_name);
			     ...
			     loff_t * offset)
{
	...
	unsigned char *tx_buffer = kzalloc(MAX_BUFFER_SIZE, GFP_KERNEL|GFP_DMA);

	if(!tx_buffer) {
		printk("kmalloc tx_buffer error");
		return 0;
	}
	...
	if (copy_from_user((void *)tx_buffer, &buf[0], count)) {
	...
	/* Write data */
	ret = spi_write(p61_dev->spi, (void *)tx_buffer, count);
	if (ret < 0) {
		ret = -EIO;
	} else {
		ret = count;
			    ...
			    loff_t * offset)
{
	...
	unsigned char *rx_buffer = kzalloc(MAX_BUFFER_SIZE, GFP_KERNEL|GFP_DMA);

	if(!rx_buffer) {
		printk("kmalloc rx_buffer error");
		return 0;
	}
	...
	if (copy_to_user(buf, (void *)rx_buffer, count)) {
...
static int p61_probe(struct spi_device *spi)
{
	...
	unsigned int irq_flags;
        ...
        pr_err("%s (1745) chip select : %d , bus number = %d \n", __func__,
	...
	of_property_read_string(np, NFCC_RESERVED_R00, &p61_dev->nfcc_name_reserved_r00);
        of_property_read_string(np, NFCC_RESERVED_R01, &p61_dev->nfcc_name_reserved_r01);
        of_property_read_string(np, NFCC_RESERVED_R02, &p61_dev->nfcc_name_reserved_r02);
        of_property_read_string(np, NFCC_RESERVED_R03, &p61_dev->nfcc_name_reserved_r03);
        P61_ERR_MSG("%s: eSE power controller nfcc='%s' ,r00='%s',r01='%s' ,r02='%s' ,r03='%s'\n", __func__,
		    p61_dev->nfcc_name, p61_dev->nfcc_name_reserved_r00,p61_dev->nfcc_name_reserved_r01, p61_dev->nfcc_name_reserved_r02, p61_dev->nfcc_name_reserved_r03);
