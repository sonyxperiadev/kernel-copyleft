...
#define FUNCTION MSM_PIN_FUNCTION
...
static const struct pinfunction yupik_functions[] = {
	FUNCTION(ibi_i3c),
	FUNCTION(gpio),
	FUNCTION(qup00),
	FUNCTION(qup07),
...
static const struct msm_pinctrl_soc_data yupik_tlmm = {
	.pins = yupik_pins,
...
static void yupik_tlmm_config_mpm_wake_disable_gpios(void)
{
	unsigned int i;
...
static int yupik_tlmm_probe(struct platform_device *pdev)
{
	...
	yupik_tlmm_config_mpm_wake_disable_gpios();

	return 0;
}

static const struct of_device_id yupik_tlmm_of_match[] = {
	{ .compatible = "qcom,yupik-tlmm", .data = &yupik_tlmm},
	{ },
};

static struct platform_driver yupik_tlmm_driver = {
	.driver = {
		.name = "yupik-pinctrl",
		.of_match_table = yupik_tlmm_of_match,
	},
	.probe = yupik_tlmm_probe,
	.remove = msm_pinctrl_remove,
};

static int __init yupik_tlmm_init(void)
{
	return platform_driver_register(&yupik_tlmm_driver);
}
arch_initcall(yupik_tlmm_init);

static void __exit yupik_tlmm_exit(void)
{
	platform_driver_unregister(&yupik_tlmm_driver);
}
module_exit(yupik_tlmm_exit);

MODULE_DESCRIPTION("QTI yupik TLMM driver");
MODULE_LICENSE("GPL");
MODULE_DEVICE_TABLE(of, yupik_tlmm_of_match);
