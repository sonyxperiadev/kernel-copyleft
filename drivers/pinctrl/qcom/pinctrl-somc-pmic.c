...
static int somc_pinctrl_pmic_probe(struct platform_device *pdev)
{
	struct pinctrl *pin;
...
static const struct of_device_id somc_pinctrl_pmic_dt_match[] = {
	{ .compatible = "somc-pinctrl-pmic", },
	{ },
};

static struct platform_driver somc_pinctrl_pmic_drv = {
	.probe		= somc_pinctrl_pmic_probe,
	.driver = {
		.name	= "somc-pinctrl-pmic",
		.owner	= THIS_MODULE,
		.of_match_table = somc_pinctrl_pmic_dt_match,
	},
};

static int __init somc_pinctrl_pmic_drv_register(void)
{
	return platform_driver_register(&somc_pinctrl_pmic_drv);
}
arch_initcall(somc_pinctrl_pmic_drv_register);

static void __exit somc_pinctrl_pmic_drv_unregister(void)
{
	platform_driver_unregister(&somc_pinctrl_pmic_drv);
}
module_exit(somc_pinctrl_pmic_drv_unregister);
