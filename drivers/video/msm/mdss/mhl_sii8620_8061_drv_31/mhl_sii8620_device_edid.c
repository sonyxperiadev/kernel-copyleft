...
/* 1.5s is enough to clear edid done */
#define MHL_CRL_EDID_DONE_TIME 1500
...
void mhl_device_start_edid_read(void)
{
	...
	*/
	...
	mhl_sii8620_device_edid_read_request(0);

	ret = mhl_lib_timer_start(edid_context.edid_done_timer,
					MHL_CRL_EDID_DONE_TIME);
	if (ret != 0)
...
static void mhl_device_clear_edid_done_timer(void *callback_param)
{
	...
	if (edid_context.is_request_queued) {
...
pr_err("%s:SWWA_BZ30759\n", __func__);
	...
	*/
	...
	#define BIT_080C_HDCP1X_LB_BIST 0x01
		...
		mhl_pf_write_reg(REG_PAGE_0_DDC_MANUAL,BIT_PAGE_0_DDC_MANUAL_MAN_DDC);
		mhl_pf_write_reg(REG_PAGE_2_INTR9,0xFF);
		/* Disable EDID interrupt */
		mhl_pf_write_reg(REG_PAGE_2_INTR9, 0);
		mhl_pf_write_reg(REG_PAGE_2_TPI_CBUS_START,BIT_PAGE_2_TPI_CBUS_START_GET_EDID_START_0);
		do {
			result = mhl_pf_read_reg(REG_PAGE_2_INTR9);
		} while (!result);
		mhl_pf_write_reg(REG_PAGE_2_INTR9,0xFF);
		...
		if (BIT_PAGE_0_DDC_STATUS_DDC_BUS_LOW & ddc_status) {
			pr_warn("%s: Clearing DDC ack status\n", __func__);
			mhl_pf_write_reg(REG_PAGE_0_DDC_STATUS,
	...
	/* turn on heartbeat */
	mhl_pf_modify_reg(REG_PAGE_5_DISC_CTRL1,
	...
	/* those have been already set. need to init them again? */
	mhl_pf_write_reg(REG_PAGE_2_PKT_FILTER_0, 0xFF);
	mhl_pf_write_reg(REG_PAGE_2_PKT_FILTER_1, 0xFF);
...
int  mhl_8620_dev_int_edid_isr(uint8_t int_edid_devcap_status)
{
	...
	if (BIT_INTR9_EDID_DONE & int_edid_devcap_status) {
		...
		if (num_extensions < 0) {
			...
			if (MHL_SUCCESS != mhl_sii8620_device_edid_read_request(get_current_edid_request_block()))
				goto retry_edid_read;
