...
/* Others */
#define BLOCK_DDC_VIA_UPSTREAM_HPD_LOW BIT_PAGE_0_SYS_CTRL1_OTP_AUTO_LOAD_EN
#define BIT_PAGE_3_M3_P0CTRL_MHL3_P0_UNLIMIT_EN_PP         0x08
#define BIT_PAGE_3_M3_P0CTRL_MHL3_P0_UNLIMIT_EN_NORM       0x00

#define BIT_PAGE_0_SYS_CTRL1_DVI_HDMI_TPI		BIT_PAGE_0_SYS_CTRL1_EDGE
...
static uint16_t total_bytes_to_ack;
...
static void clear_auto_zone_for_mhl_3(void)
{
	...
	/* Claer auto zone mode 2'b00 0.5x mode, hdmi_clk_freq = 0.5*pxl_clk_freq */
	mhl_pf_modify_reg(REG_PAGE_3_MHL_PLL_CTL0,
			MSK_PAGE_3_MHL_PLL_CTL0_HDMI_CLK_RATIO, 0x00);
}
...
static void set_auto_zone_for_mhl_1_2(void)
{
	...
	/* Set auto zone mode 2'b10 2x mode, hdmi_clk_freq = 2*pxl_clk_freq */
	mhl_pf_modify_reg(REG_PAGE_3_MHL_PLL_CTL0,
			MSK_PAGE_3_MHL_PLL_CTL0_HDMI_CLK_RATIO,
			BIT_PAGE_3_MHL_PLL_CTL0_HDMI_CLK_RATIO);
}
	...
	if ((BIT_CBUS_MHL12_DISCON_INT & int_disc_status) ||
		...
		if (0x02 == (disc_stat2 & 0x03)) {
			...
			mhl_pf_write_reg(REG_PAGE_5_DISC_CTRL4, 0x90);
				...
				mhl_pf_write_reg(REG_PAGE_3_MHL_PLL_CTL0,0x07);		/* 0x0337 */
				mhl_pf_write_reg(REG_PAGE_3_MHL_DP_CTL0 ,0xF0);		/* 0x0331 */
				mhl_pf_write_reg(REG_PAGE_3_MHL_DP_CTL1 ,0xA2);		/* 0x0332 */
				mhl_pf_write_reg(REG_PAGE_3_MHL_DP_CTL3 ,0x35);		/* 0x0334 */
				mhl_pf_write_reg(REG_PAGE_3_MHL_DP_CTL5 ,0x01);		/* 0x0336 */
				mhl_pf_write_reg(REG_PAGE_3_MHL_DP_CTL2 ,0x03);		/* 0x0333 */
				mhl_pf_write_reg(REG_PAGE_3_MHL_DP_CTL6 ,0x02);		/* 0x0350 */
				mhl_pf_write_reg(REG_PAGE_3_MHL_DP_CTL7 ,0x03);		/* 0x0351 */
...
static int mhl_cbus_isr(uint8_t cbus_int)
{
	...
	*/
	...
	if (cbus_int & ~BIT_PAGE_5_CBUS_INT_0_CBUS_INT_0_STAT2) {
		/* bugzilla 27396
...
static int coc_isr(uint8_t coc_int_status)
{
	...
	if (!IS_MODE_MHL3) {
		...
		if (BIT_COC_CALIBRATION_DONE & coc_int_status) {
			...
			if (retry) {
				uint8_t coc_ctl0;
				pr_debug("%s: Retry calibration\n",
					__func__);
				coc_ctl0 = mhl_pf_read_reg(REG_PAGE_7_COC_CTL0);
				mhl_pf_write_reg(REG_PAGE_7_COC_CTL0,
					coc_ctl0 & ~0x04);
				mhl_pf_write_reg(REG_PAGE_7_COC_CTL14,
					0x00);
				mhl_pf_write_reg(REG_PAGE_7_COC_CTL15,
					0x80);
				msleep(20);
				mhl_pf_write_reg(REG_PAGE_7_COC_CTL0,
					coc_ctl0);
...
static void regs_init_for_ready_to_mhl_connection(void)
{
	...
	// mhl_pf_write_reg(REG_PAGE_3_M3_CTRL, 0x0A);
	...
	mhl_pf_write_reg(REG_PAGE_3_MHL_COC_CTL1, 0x87);
	...
	mhl_pf_write_reg(REG_PAGE_3_MHL_PLL_CTL0, 0x07);
	...
	mhl_pf_write_reg(REG_PAGE_3_MHL_DP_CTL7, 0x06);
...
{
	if (IS_MODE_MHL3) {
			/* on Titan 1.1 and following, even in MHL3 mode, TPI:1A[0] controls DVI vs. HDMI */
			mhl_pf_write_reg(REG_PAGE_0_SYS_CTRL1,
					 BLOCK_DDC_VIA_UPSTREAM_HPD_LOW);
	} else {
		...
		mhl_pf_write_reg(REG_PAGE_0_INTR1, 0xFF);
			...
			mhl_pf_write_reg(REG_PAGE_0_SYS_CTRL1,
					 BLOCK_DDC_VIA_UPSTREAM_HPD_LOW
					 |BIT_PAGE_0_SYS_CTRL1_DVI_HDMI_TPI);
		}
	}
...
int mhl_device_switch_cbus_mode(CBUS_MODE_TYPE mode_sel)
{
	...
	case CBUS_eCBUS_S:
		...
		mhl_pf_write_reg(REG_PAGE_3_MHL_COC_CTL1, 0xBD);
...
static char *mhl_dev_print_cbusp(MHL_DEV_CBUS_P cbusp)
{
	...
	case DEV_RESERVE_P_5:
		return "DEV_RESERVE_P5";
...
#ifndef UNIT_TEST
static DEVICE_ATTR(aksv, 0660, mhl_device_sysfs_rda_aksv, NULL);
static DEVICE_ATTR(tmds, 0660, mhl_device_sysfs_rda_tmds, NULL);
...
int stop_video(void)
{
	...
	if (IS_MODE_MHL3) {
		...
		do {
			...
			ddc_status = (mhl_pf_read_reg(REG_PAGE_0_DDC_STATUS) & 0x5F);
		...
		pr_debug("%s: HDCP2 Off; Last DDC Status %02X; Count = %d\n",
			 __func__, ddc_status, count);
...
static int set_hdmi_params()
{
	...
	uint8_t					link_speed = MHL_XDC_TMDS_000;
...
#ifdef ROGUE_WORKAROUND
	if (IS_MODE_MHL3) {
		pr_debug("%s: MHL: send TTR\n", __func__);
		set_cbus_command(MHL_MSC_MSG, 0xF0,
	...
	if (is_compress_mode) {
		if (IS_MODE_MHL3 && !mhl_cbus_16bpp_available()) {
			...
			set_link_mode(MHL_STATUS_PATH_ENABLED | MHL_STATUS_CLK_MODE_PACKED_PIXEL);
		...
		set_link_mode(MHL_STATUS_PATH_ENABLED | MHL_STATUS_CLK_MODE_NORMAL);
	...
	if (IS_MODE_MHL3){
		...
		if (bpp_on_wb){
			...
			p_buffer->descriptors[1].att_code=0;
			p_buffer->descriptors[1].view_pixel_format = 0;
			p_buffer->descriptors[2].att_code=0;
			p_buffer->descriptors[2].view_pixel_format = 0;
			p_buffer->descriptors[3].att_code=0;
			p_buffer->descriptors[3].view_pixel_format = 0;
			p_buffer->descriptors[4].att_code=0;
			p_buffer->descriptors[4].view_pixel_format = 0;
	 	}else{
			/* only one descriptor to send */
			xfer_size =sizeof (p_buffer->header)
		...
		if (p_buffer){
			...
			p_buffer->descriptors[0].att_code =0;
			switch(dev_context->link_mode & MHL_STATUS_CLK_MODE_MASK){
			case MHL_STATUS_CLK_MODE_PACKED_PIXEL:
				p_buffer->descriptors[0].view_pixel_format = VIEW_PIX_FMT_16BPP;
				break;
			case MHL_STATUS_CLK_MODE_NORMAL:
				p_buffer->descriptors[0].view_pixel_format = VIEW_PIX_FMT_24BPP;
				break;
			}
		...
		*/
		...
		case MHL_STATUS_CLK_MODE_PACKED_PIXEL:
			link_speed = mhl_cbus_set_lowest_tmds_link_speed(
		...
		mhl_pf_write_reg(REG_PAGE_3_MHL3_TX_ZONE_CTL, link_speed);
		switch (link_speed) {
