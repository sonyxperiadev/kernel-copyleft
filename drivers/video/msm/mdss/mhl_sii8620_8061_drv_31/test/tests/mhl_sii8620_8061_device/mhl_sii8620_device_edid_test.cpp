...
/* 
128 byte readしたら、FIFOの中のポインターは次のBlockのアドレスをさす
   (SIMG QA)  
*/

/* 
128 byte readしなかったら、FIFOの中のポインターは次のBlockのアドレスをささない？
   (SIMG QA)  
*/

/* 
128 byte 以上 readしたら、FIFOの中のポインターはどのBlockをさすの？
   (SIMG QA)  
*/
...
#if 0
/* edid_block_numberをn⇒0に戻す。初期化して最初から読み込みなおす */
/* edid dataをstaticでなく、動的にメモリを確保する  */

/* si_mhl_sii8620_device_get_num_cea_861_extensions */

/* １回目のEDID Request終了あとに、interruptで、ext blockがまだ読み込みきれてなかったら、 そのBlockの読み出しRequestを実施する */
...
//現在のextention blockに残があれば、そのBlockのread 要求を出す。

//intにはいったときに、HPDがHIGHではなかったら、Errorを返す

//extention blockが0より小さかく、HPDがHIGHだったら、再度0をrequestする。

//

//DDC ack checkで、応答がなかったら、再度edidを0から読み込みはじめる。
...
//DCAPの割り込みはEDIDのINTでは見ない

//edid 読み込み中にHPDおちたら、edid読み込みプロセス止める
