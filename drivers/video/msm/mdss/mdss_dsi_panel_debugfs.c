...
void mipi_dsi_panel_create_debugfs(struct msm_fb_data_type *mfd)
{
	...
	dbg_dir = debugfs_create_dir("mdss_dsi_panel", 0);

	if (!dbg_dir) {
		dev_err(dev, "%s: dbgfs create dir failed\n", __func__);
	} else {
		if (!debugfs_create_file("read", S_IWUSR, dbg_dir, mfd,
								&read_fops)) {
			dev_err(dev, "%s: failed to create dbgfs read file\n",
								__func__);
			return;
		}
		if (!debugfs_create_file("write", S_IWUSR, dbg_dir, mfd,
								&write_fops)) {
			dev_err(dev, "%s: failed to create dbgfs write file\n",
								__func__);
			return;
		}
		if (!debugfs_create_file("result", S_IRUGO, dbg_dir, mfd,
								&result_fops)) {
