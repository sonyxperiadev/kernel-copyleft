		...
		int *u_data, int *v_data)
{
	...
	if ((vendor_pdata->driver_ic == PANEL_DRIVER_IC_NOVATEK) ||
		(vendor_pdata->driver_ic == PANEL_DRIVER_IC_NONE))
		len = CLR_NUM_REG_DATA_NOV;
	else
		len = CLR_NUM_REG_DATA_REN;
...
int mdss_dsi_panel_pcc_setup(struct msm_fb_data_type *mfd)
{
	...
	if (u_data == 0 && v_data == 0)
		goto exit;
	if (vendor_pdata->driver_ic == PANEL_DRIVER_IC_NONE)
		if (u_data == 0 && v_data == 0x24)
			goto exit;
