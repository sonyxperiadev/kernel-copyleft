		...
		struct device_attribute *attr, char *buf)
{
	...
	struct mdss_panel_common_pdata *panel_data
		= dev_get_platdata(&ctrl_pdata->panel_data.panel_pdev->dev);
	struct mdss_pcc_data *pcc_data = &panel_data->pcc_data;
	u32 r, g, b;
		...
		int *u_data, int *v_data)
{
	...
	int param_type = vendor_pdata->pcc_data.param_type;
...
int mdss_dsi_panel_pcc_setup(struct msm_fb_data_type *mfd)
{
	...
	pcc_data = &vendor_pdata->pcc_data;
	if (!pcc_data->color_tbl)
		goto exit;
	ctrl_pdata = container_of(pdata, struct mdss_dsi_ctrl_pdata,
				panel_data);
	...
	if (vendor_pdata->uv_read_cmds.cmds)
		get_uv_data(ctrl_pdata, vendor_pdata, &pcc_data->u_data,
			&pcc_data->v_data);
	if (pcc_data->u_data == 0 && pcc_data->v_data == 0)
		goto exit;
	if (pcc_data->param_type == CLR_DATA_UV_PARAM_TYPE_SY35590_DEFAULT)
			...
			int driver_ic, char *id_data)
{
	...
	for_each_child_of_node(np, next) {
		...
		rc = of_property_read_u32(next,
			"somc,panel-uv-param-type", &tmp);
		panel_data->pcc_data.param_type =
			(!rc ? tmp : CLR_DATA_UV_PARAM_TYPE_NONE);

		rc = of_property_read_u32(next,
			"somc,panel-pcc-table-size", &tmp);
		panel_data->pcc_data.tbl_size =
			(!rc ? tmp : 0);

		if (of_property_read_bool(next, "somc,panel-pcc-table")) {
			panel_data->pcc_data.color_tbl =
				kzalloc(panel_data->pcc_data.tbl_size *
					sizeof(struct mdss_pcc_color_tbl),
					GFP_KERNEL);
			if (!panel_data->pcc_data.color_tbl) {
			...
			rc = of_property_read_u32_array(next,
				...
				(u32 *)panel_data->pcc_data.color_tbl,
				panel_data->pcc_data.tbl_size *
				sizeof(struct mdss_pcc_color_tbl) /
				sizeof(u32));
			if (rc) {
...
static int __devinit mdss_dsi_panel_probe(struct platform_device *pdev)
{
	...
	if (!vendor_pdata.driver_ic) {
		...
		pr_err("%s: gpio=%d\n", __func__, vendor_pdata.driver_ic);
...
static int __devexit mdss_dsi_panel_remove(struct platform_device *pdev)
{
	...
	kzfree(vendor_pdata->pcc_data.color_tbl);
	vendor_pdata->pcc_data.color_tbl = NULL;
