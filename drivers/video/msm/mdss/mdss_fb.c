			 ...
			 unsigned long arg)
{
	...
	switch (cmd) {
	case MSMFB_CURSOR:
		pr_debug("%s: MSMFB_CURSOR\n",__func__);
		ret = mdss_fb_cursor(info, argp);
		break;

	case MSMFB_SET_LUT:
		pr_debug("%s: MSMFB_SET_LUT\n",__func__);
		ret = mdss_fb_set_lut(info, argp);
		break;

	case MSMFB_GET_PAGE_PROTECTION:
		pr_debug("%s: MSMFB_GET_PAGE_PROTECTION\n",__func__);
	...
	case MSMFB_DISPLAY_COMMIT:
		pr_debug("%s: MSMFB_DISPLAY_COMMIT\n",__func__);
		ret = mdss_fb_display_commit(info, argp);
		break;

	case MSMFB_LPM_ENABLE:
		pr_debug("%s: MSMFB_LPM_ENABLE\n",__func__);
