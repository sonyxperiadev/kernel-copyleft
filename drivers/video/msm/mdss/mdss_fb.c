...
static int mdss_fb_power_setting_idle(struct msm_fb_data_type *mfd)
{
	int ret = 0;
...
int mdss_fb_wait_for_fence(struct msm_fb_data_type *mfd)
{
	int i, ret = 0;
	/* buf sync */
...
int mdss_fb_signal_timeline(struct msm_fb_data_type *mfd)
{
...
static void mdss_fb_commit_wq_handler(struct work_struct *work)
{
	...
	if (fb_backup->disp_commit.flags &
		...
		mdss_fb_pan_display_sub(var, info);
	}
	mutex_lock(&mfd->sync_mutex);
	mfd->is_committing = 0;
	complete_all(&mfd->commit_comp);
