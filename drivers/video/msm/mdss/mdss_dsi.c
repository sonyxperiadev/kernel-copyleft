...
static void mdss_dsi_dba_work(struct work_struct *work)
{
	...
	utils_init_data.chip_name = "adv7535";
	...
	if (!IS_ERR_OR_NULL(pinfo->dba_data)) {
		...
		mdss_dba_utils_power_on(pinfo->dba_data, pinfo, true);
				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_PANEL_OFF:
		...
		pr_debug("%s:Skip mdss dsi off ctrl\n", __func__);
		break;
#elif defined(CONFIG_MACH_SONY_BLANC_WINDY)
		pr_debug("%s:Skip mdss dsi off ctrl\n", __func__);
		break;
#endif
		ctrl_pdata->ctrl_state &= ~CTRL_STATE_MDP_ACTIVE;
		...
		struct platform_device *pdev)
{
	...
	if (!dsi_pan_node)
		...
		pr_debug("%s:dsi_pan_node->name = %s\n", __func__,dsi_pan_node->name);
	return dsi_pan_node;
}
...
static int mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	pr_err("%s: Dsi Ctrl->%d initialized\n", __func__, index);
