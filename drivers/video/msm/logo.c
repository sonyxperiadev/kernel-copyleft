...
int load_565rle_image(char *filename)
{
	...
	if (!info->screen_base) {
		pr_err("Screen base not initialized\n");
		return -EINVAL;
	}
...
static void __init draw_logo(void)
{
	...
	if (fb_info && fb_info->fbops->fb_open) {
		pr_info("Drawing logo.\n");
		if (fb_info->fbops->fb_open(fb_info, 0))
			pr_info("%s: fb_open failed\n", __func__);
		if (fb_info->fbops->fb_pan_display(&fb_info->var, fb_info))
			pr_info("%s: fb_pan_display failed\n", __func__);
	}
}
