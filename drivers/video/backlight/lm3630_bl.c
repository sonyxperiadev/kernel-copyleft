...
#if 0
static struct lm3630_platform_data *lm3630_parse_config(struct device *dev)
{
	...
	if(!pdata){
		pr_err("Failed to alloc memory for pdata \n");
		return NULL;
	}

	if(of_property_read_u32(devnode, "max_brt_led1", &pdata->max_brt_led1)){
		pr_err("Failed to get property: max_brt_led1");
		return NULL;
	}

	if(of_property_read_u32(devnode, "max_brt_led2", &pdata->max_brt_led2)){
		pr_err("Failed to get property: max_brt_led2");
		return NULL;
	}

	if(of_property_read_u32(devnode, "init_brt_led1", &pdata->init_brt_led1)){
		pr_err("Failed to get property: init_brt_led1");
		return NULL;
	}

	if(of_property_read_u32(devnode, "init_brt_led2", &pdata->init_brt_led2)){
		pr_err("Failed to get property: init_brt_led2");
		return NULL;
	}

	if(of_property_read_u32(devnode, "pwm_ctrl", &pdata->pwm_ctrl)){
		pr_err("Failed to get property: pwm_ctrl");
		return NULL;
	}

	if(of_property_read_u32(devnode, "pwm_active", &pdata->pwm_active)){
		pr_err("Failed to get property: pwm_active");
		return NULL;
	}

	if(of_property_read_u32(devnode, "bank_a_ctrl", &pdata->bank_a_ctrl)){
		pr_err("Failed to get property: bank_a_ctrl");
		return NULL;
	}

	if(of_property_read_u32(devnode, "bank_b_ctrl", &pdata->bank_b_ctrl)){
		pr_err("Failed to get property: bank_b_ctrl");
		return NULL;
	}
