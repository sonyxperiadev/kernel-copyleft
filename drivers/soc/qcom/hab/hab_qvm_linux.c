...
void  hab_shmem_remove(struct platform_device *pdev)
{
}
...
struct hab_driver_ops shmem_ops = {
	.habhyp_commdev_alloc = shmem_habhyp_commdev_alloc,
	.habhyp_commdev_dealloc = shmem_habhyp_commdev_dealloc,
	.physical_channel_send = shmem_physical_channel_send,
	.physical_channel_read = shmem_physical_channel_read,
	.physical_channel_rx_dispatch = shmem_physical_channel_rx_dispatch,
	.dump_hab_wq = shmem_dump_hab_wq,
...
static int hab_shmem_probe(struct platform_device *pdev)
{
	...
	pr_debug("pchan idx %d, hab irq=%d shmem_base=%pK, mem %pK, mem->start %llu\n",
			 qvm_priv_info.probe_cnt, irq, shmem_base, mem, mem->start);

	qvm_priv_info.probe_cnt++;
	if (qvm_priv_info.probe_cnt == pchan_cnt) {
		hab_ops_register(&shmem_ops);

		/*
		 * this function is called at the end of last probe call
		 * so shmem probes are done before char driver is created.
		 */
		ret = hab_driver_init();
		if (ret) {
...
void shmem_hab_pipe_read_dump(struct physical_channel *pchan)
{
	...
	for (i = 0; i < DBG_ITEM_SIZE; i++) {
		...
		snprintf(str, sizeof(str),
		"it %d: rd %d wr %d va %pK index 0x%X size %d ret %d\n",
...
int shmem_hab_stat_log(struct physical_channel **pchans, int pchan_cnt, char *dest,
			int dest_size)
{
	return 0;
};
