...
struct hab_driver_ops {
	...
	int (*physical_channel_send)(struct physical_channel *pchan,
				struct hab_header *header, void *payload, unsigned int flags);
	...
	int (*hab_stat_log)(struct physical_channel **pchans,
			int pchan_cnt,
			char *dest,
			int dest_siz);
...
static inline void hab_ops_register(struct hab_driver_ops *os_ops)
{
	hab_driver.ops = os_ops;
}

static inline int habhyp_commdev_alloc(void **commdev,
		int is_be,
		char *name,
		int vmid_remote,
		struct hab_device *mmid_device)
{
	return hab_driver.ops->habhyp_commdev_alloc(commdev, is_be, name,
		vmid_remote, mmid_device);
}

static inline int habhyp_commdev_dealloc(void *commdev)
{
	return hab_driver.ops->habhyp_commdev_dealloc(commdev);
}

static inline int physical_channel_read(struct physical_channel *pchan,
		void *payload,
		size_t read_size)
{
	return hab_driver.ops->physical_channel_read(pchan, payload, read_size);
}

static inline int physical_channel_send(struct physical_channel *pchan,
		struct hab_header *header,
		void *payload,
		unsigned int flags)
{
	return hab_driver.ops->physical_channel_send(pchan, header, payload, flags);
}

static inline void physical_channel_rx_dispatch(unsigned long physical_channel)
{
	hab_driver.ops->physical_channel_rx_dispatch(physical_channel);
}

static inline void dump_hab_wq(struct physical_channel *pchan)
{
	hab_driver.ops->dump_hab_wq(pchan);
}

static inline void hab_pipe_read_dump(struct physical_channel *pchan)
{
	hab_driver.ops->hab_pipe_read_dump(pchan);
}

static inline int hab_stat_log(struct physical_channel **pchans, int pchan_cnt, char *dest,
			int dest_size)
{
...
{
	return hab_driver.ops->habhyp_virq_tx_register(dbl, dbl_label);
}

static inline int habhyp_virq_rx_register(struct hvirq_dbl *dbl, int dbl_label)
{
	return hab_driver.ops->habhyp_virq_rx_register(dbl, dbl_label);
}

static inline int habhyp_virq_send(struct hvirq_dbl *dbl)
{
	return hab_driver.ops->habhyp_virq_send(dbl);
}

static inline int habhyp_virq_tx_unregister(struct hvirq_dbl *dbl)
{
	return hab_driver.ops->habhyp_virq_tx_unregister(dbl);
}

static inline int habhyp_virq_rx_unregister(struct hvirq_dbl *dbl)
{
	return hab_driver.ops->habhyp_virq_rx_unregister(dbl);
}

static inline int habhyp_get_virq_num_id(void **virqdev, int label)
{
	return hab_driver.ops->habhyp_get_virq_num_id(virqdev, label);
}
...
void hab_ops_register(struct hab_driver_ops *os_ops);
