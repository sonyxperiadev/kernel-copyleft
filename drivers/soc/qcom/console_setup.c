...
static void disable_serial_gpio(void)
{
	...
	np_tx = of_find_node_by_path(
			"/soc/pinctrl@1000000/msm_gpio_20");
	if (!np_tx) {
		pr_err("couldn't find msm_gpio_20 node\n");
		return;
	}

	np_rx = of_find_node_by_path(
			"/soc/pinctrl@1000000/msm_gpio_21");
	if (!np_rx) {
		pr_err("couldn't find msm_gpio_21 node\n");
		goto err0;
	}
	...
	if (!np_tx_def) {
		pr_err("couldn't find msm_gpio_20_def node\n");
		goto err1;
	}

	np_rx_def = of_find_node_by_name(np_rx, "default");
	if (!np_rx_def) {
		pr_err("couldn't find msm_gpio_21_def node\n");
		goto err2;
	}
...
static int __init init_console_setup(void)
{
	if (need_serial_console) {
		...
		np = of_find_node_by_path("/soc/serial@7af0000");
		if (!np) {
			pr_err("couldn't find /soc/serial@7af0000 node\n");
			return -EINVAL;
		}
