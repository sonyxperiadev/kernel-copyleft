...
static int need_serial_console = 0;
...
static void disable_serial_gpio(void)
{
	...
	np_tx = of_find_node_by_path(
			"/soc/pinctrl@03400000/msm_gpio_4");
	if (!np_tx) {
	...
	np_rx = of_find_node_by_path(
			"/soc/pinctrl@03400000/msm_gpio_5");
	if (!np_rx) {
...
static int __init init_console_setup(void)
{
	if (need_serial_console) {
		...
		np = of_find_node_by_path("/soc/serial@0c170000");
		if (!np) {
			pr_err("couldn't find /soc/serial@0c170000 node\n");
			return -EINVAL;
		}
