...
struct	control_node_data
{
	...
	ssize_t ( *info_read )( void *data, struct device_attribute*, char* );
...
ssize_t info_file_node_read( struct device *dev, struct device_attribute *attr, char *buf )
{

	struct control_node_data	*internal_data	= dev_get_drvdata( dev );

	return	internal_data->info_read( internal_data->data, attr, buf );

}

static DEVICE_ATTR( control, S_IRUGO | S_IWUSR, control_file_node_read, control_file_node_write );
static DEVICE_ATTR( info, S_IRUGO | S_IWUSR, info_file_node_read, NULL );
...
struct control_class_device*	control_file_node_register( struct control_node_load *load )
{
	...
	file_node_data->info_read		= load->info_read ? load->info_read : empty_info_read;
	...
	if( device_create_file( return_data->Device, &dev_attr_info ) )
		printk( KERN_ERR "DEV_T : Class %s, Device %s, device_create_file( info ) failed\n", load->class_name, load->device_name );
...
void control_file_node_unregister( struct control_class_device *unload )
{
	...
	device_remove_file( unload->Device, &dev_attr_info );
