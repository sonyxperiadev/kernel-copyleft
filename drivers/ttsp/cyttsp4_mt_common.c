...
static void cyttsp4_get_mt_touches(struct cyttsp4_mt_data *md, int num_cur_tch)
{
	...
	struct cyttsp4_core_platform_data *core_pdata =
				dev_get_platdata(&md->ttsp->core->dev);
	...
	for (i = 0; i < num_cur_tch; i++) {
		...
		if (core_pdata->use_configure_sensitivity) {
			if (tch.abs[CY_TCH_O] == CY_OBJ_GLOVE) {
				tch.abs[CY_TCH_P] += si->si_ofs.max_p + 1;
				dev_dbg(dev, "%s: t=%d z=%d force zero\n",
					__func__, t, tch.abs[CY_TCH_P]);
			}
		} else {
...
#ifdef CONFIG_FB
static void cyttsp4_mt_notify_resume(struct work_struct *work)
{
	struct cyttsp4_mt_data *md = container_of(work,
			struct cyttsp4_mt_data, notify_resume);
	struct device *dev = &md->ttsp->dev;
...
static void cyttsp4_mt_notify_suspend(struct work_struct *work)
{
	struct cyttsp4_mt_data *md = container_of(work,
			struct cyttsp4_mt_data, notify_suspend);
	struct device *dev = &md->ttsp->dev;
	dev_dbg(dev, "%s: usage_count=%d\n", __func__,
		atomic_read(&dev->power.usage_count));
				...
				unsigned long event, void *data)
{
	...
	static bool is_unblank;

	if (evdata && evdata->data && event == FB_EVENT_BLANK && md &&
			md->ttsp) {
		blank = evdata->data;
		if ((*blank == FB_BLANK_UNBLANK)  && (!is_unblank)) {
			is_unblank = true;
			cancel_work_sync(&md->notify_suspend);
			cancel_work_sync(&md->notify_resume);
			schedule_work(&md->notify_resume);
		} else if ((*blank == FB_BLANK_POWERDOWN)  && (is_unblank)) {
			is_unblank = false;
			cancel_work_sync(&md->notify_resume);
			cancel_work_sync(&md->notify_suspend);
			schedule_work(&md->notify_suspend);
		}
	}
...
#endif
#ifdef CONFIG_FB
	md->fb_notif.notifier_call = cyttsp4_fb_notifier_callback;
	rc = fb_register_client(&md->fb_notif);
	if (rc) {
		dev_err(dev, "%s: Unable to register fb_notifier r=%d\n",
				__func__, rc);
	} else {
		INIT_WORK(&md->notify_resume, cyttsp4_mt_notify_resume);
		INIT_WORK(&md->notify_suspend, cyttsp4_mt_notify_suspend);
	}
#endif
	dev_dbg(dev, "%s: OK\n", __func__);
	return 0;
