						...
						uint16_t len, uint8_t *wd)
{
	...
	struct spi_transfer xfer;
	uint8_t buf[3];
	uint8_t scmd[SPI_PRG_MAX_NUM + 4 + 4];
	uint8_t stadu = 0x00;
	uint8_t stadl = 0x00;
	...
	memcpy(scmd, break_code, sizeof(break_code));
	...
	do {
		...
		memset(scmd+8, 0x00, sizeof(unsigned char) * (write_length));
		memset(&xfer, 0, sizeof(struct spi_transfer));

		spi_message_init(&msg);

		scmd[4 + 0] = 0x02;				/* command */
		scmd[4 + 1] = dl_flg[bank];		/* 0: PSEQ / 1: TNCTL */
		/* size(upper) */
		scmd[4 + 2] = (write_length >> 8) & 0x00FF;
		scmd[4 + 3] = write_length & 0x00FF;	/* size(lower) */
		/* write data */
