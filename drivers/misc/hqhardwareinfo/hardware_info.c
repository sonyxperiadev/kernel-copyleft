...
#define HARDWARE_INFO_VERSION   "SM6350"
...
static BOARD_TYPE_TABLE pcba_type_table[] =
{
    ...
    { .type_value = 5,  .pcba_type_name = "QN1613E", },
    { .type_value = 6,  .pcba_type_name = "QN1613F", },
...
{
    {  .stage_value = 0,        .pcba_stage_name="DP", },
    {  .stage_value = 163,      .pcba_stage_name="V3", },
    {  .stage_value = 550,      .pcba_stage_name="SP", },
    {  .stage_value = 900,      .pcba_stage_name="SP2",},
    {  .stage_value = 1200,     .pcba_stage_name="AP", },
    {  .stage_value = 1800,     .pcba_stage_name="PP", },
};
...
#define UFS_VENDOR_NAME "/sys/bus/platform/drivers/ufshcd-qcom/1d84000.ufshc/string_descriptors/manufacturer_name"
...
#define STAGE_ADC_NAME "/sys/bus/iio/devices/iio:device1/in_voltage_get_stageid_adc_input"
char* get_stage_adc(void)
{
    ...
    if (IS_ERR(pfile)) {
        printk("[HWINFO]: open read_stage_adc file failed!\n");
        goto ERR_0;
    }
    ...
    if(ret <= 0) {
        printk("[HWINFO]: read read_stage_adc  file failed!\n");
        goto ERR_1;
    }

    tmp_adc_val = simple_strtoull(tmpbuf, NULL, 0);
    adc_val = (int)(tmp_adc_val / 1000);
    printk("[HWINFO] %s adc_val %d.\n", __func__, adc_val);
...
*/
        ...
        ret = gpio_request(type_gpio_pin0, "pcb_type_gpios2");
        if (ret) {
        ...
        ret = gpio_request(type_gpio_pin2, "pcb_type_gpios0");
        if (ret) {
                printk("[HWINFO] pcb_type_gpio not available (ret=%d)\n", ret);
                current_type_value = -1;
                goto err;
        ...
        current_type_value = (gpio_get_value(type_gpio_pin0) << 0) | (gpio_get_value(type_gpio_pin1) << 1) | (gpio_get_value(type_gpio_pin2) << 2) | (gpio_get_value(type_gpio_pin3) << 3) ;
        printk("[HWINFO] type_value 0x%x\n",current_type_value);

        slot_gpio = of_get_named_gpio(np, "gpio_94", 0);
    }
