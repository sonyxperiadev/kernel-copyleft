 ...
 * It also has chip structure prototype and the platform data prototype.
 *
...
/* To get the DAC step size to increment for the given CDC difference & cin */
#define GET_DAC_STEP_SIZE(x, cin)	((u32)(x) / ((1 << (cin)) * 650))
/* To get the POS AFE value */
...
/* Minimum DAC calibration target */
...
/* Milli sec to micro second conversion factor */
...
/* Event packing for STATUS */
#define PACK_FOR_STATUS_EVENT(status, prox_status) ((status << HEX_BASE) |\
						 (prox_status << 4) |\
						 (CONV_COMP_STATUS))
/* Event packing for BASELINE */
#define PACK_FOR_BASELINE_EVENT(baseline, stage_num) ((baseline << HEX_BASE) |\
						 (stage_num << 4) |\
						 (CONV_COMP_BASELINE))
...
struct adux1050_platform_data {
	...
	/* This holds the irq gpio number for the device. */
	bool is_set_irq_gpio_no;
	unsigned short irq_gpio_no;
	/* This holds the force calib variables for the device. */
	bool proxy_enable;
	unsigned short allowed_proxy_time;
	unsigned short max_proxy_count;
	/* This holds user error threshold bits for each stage. */
	unsigned char intr_err;
	/* This holds device name. */
	char device_name[DEVICE_NAME_MAX];
	/* This holds mask of CDC interrupt. */
	bool mask_cdc_interrupt;
};
...
struct adux1050_chip {
	...
	u8 proxy_enable;
	/* Low threshold interrupt control */
	u8	low_thresh_enable;
	/* High threshold interrupt control */
