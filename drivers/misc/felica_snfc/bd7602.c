/* drivers/misc/felica_snfc/bd7602.c
 *
...
static ssize_t bd7602_rst_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	u8 data;
	...
	value = data ? 0 : 1;
	gpio_set_value_cansleep(bd7602_dev->rst_gpio, value);
...
static ssize_t bd7602_rst_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	u8 data;
	int ret, size;
	...
	ret = gpio_get_value_cansleep(bd7602_dev->rst_gpio);
	if (ret < 0) {
...
static struct device_attribute bd7602_sysfs_attrs[] = {
	...
	__ATTR(rst, S_IRUSR | S_IWUSR, bd7602_rst_show, bd7602_rst_store),
};
				...
				const struct i2c_device_id *id)
{
	...
	bd7602_dev->rst_gpio = of_get_named_gpio(client->dev.of_node,
						"bd7602,rst_gpio", 0);
	if (!gpio_is_valid(bd7602_dev->rst_gpio)) {
		ret = -EINVAL;
		dev_err(&client->dev,
			"failed to of_get_named_gpio(bd7602,rst_gpio)\n");
		goto err_rst_gpio;
	}
	ret = gpio_request(bd7602_dev->rst_gpio, "bd7602_rst");
	if (ret) {
		dev_err(&client->dev, "failed to rst gpio_request\n");
