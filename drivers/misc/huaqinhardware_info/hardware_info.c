...
#define HARDWARE_INFO_VERSION   "SM8750"
#define HARDWARE_INFO_WCN       "WCN6750"
...
static UFS_VENDOR_TABLE vendor_table[] = {
	{.id = "300", .name = "Micron",},
	{.id = "408", .name = "Kioxia",},
	{.id = "429", .name = "Hynix",},
	{.id = "462", .name = "Samsung",},
...
static BOARD_TYPE_TABLE pcba_type_table[] = {
	{.type_value = 0, .pcba_type_name = "QN7289AAC", .pcba_modem_id = "AC"},
	{.type_value = 1, .pcba_type_name = "QN7289ANKSJ", .pcba_modem_id = "NKSJ"},
	{.type_value = 2, .pcba_type_name = "QN7289AACNKSJ-DVT2", .pcba_modem_id = "ACNKSJ-DVT2"},
	{.type_value = 4, .pcba_type_name = "QN7289AAC-A", .pcba_modem_id = "AC-A"},
	{.type_value = 5, .pcba_type_name = "QN7289ANKSJ-A", .pcba_modem_id = "NKSJ-A"},
	{.type_value = 6, .pcba_type_name = "QN7289ACNKSJ", .pcba_modem_id = "ACNKSJ"},
	{.type_value = 7, .pcba_type_name = "QN7289AEVB", .pcba_modem_id = "EVB"},
};

/*
static BOARD_STAGE_TABLE pcba_stage_tabe[] = {
	{.stage_value_min = 0,    .stage_value_max = 100,  .pcba_stage_name = "EVB",},
	{.stage_value_min = 150,  .stage_value_max = 175,  .pcba_stage_name = "EVT",},
	{.stage_value_min = 180,  .stage_value_max = 420,  .pcba_stage_name = "DVT1",},
	{.stage_value_min = 530,  .stage_value_max = 620,  .pcba_stage_name = "DVT2",},
	{.stage_value_min = 830,  .stage_value_max = 970,  .pcba_stage_name = "PVT",},
	{.stage_value_min = 1100, .stage_value_max = 1300, .pcba_stage_name = "NC",},
	{.stage_value_min = 1430, .stage_value_max = 1620, .pcba_stage_name = "NC",},
	{.stage_value_min = 1660, .stage_value_max = 1940, .pcba_stage_name = "NC",},
};
*/
...
void get_hardware_info_data(enum hardware_id id, const void *data)
{
	if (NULL == data) {
		...
		case HWID_ALSPS:
			...
			break;
		case HWID_ALS_REAR:
			hwinfo_data.als_rear = data;
			break;
		case HWID_PRESS:
			hwinfo_data.press = data;
			break;
		case HWID_GSENSOR:
			...
			char *buf)
{
	if ((NULL != hwinfo_data.ctp_driver) || (NULL != hwinfo_data.ctp_module)
	    || (strlen(hwinfo_data.ctp_fw_version) > 0)) {
...
static ssize_t store_fingerprint(struct device *dev, struct device_attribute *attr,
			   const char *buf, size_t count)
{
	int ret;
	static char name[100] = "Not found";
	ret = snprintf(name, 100, "%s", buf);
	hwinfo_data.fingerprint = name;
	if (ret) {
		hwinfo_debug("[HWINFO] %s success. %s\n", __func__, buf);
	} else {
		hwinfo_data.fingerprint = "Not found";
...
static int get_ufs_vendor_id(void)
{
	...
	if(of_chosen){
		...
		if(!bootargs){
			pr_err("[HWINFO]: failed to get bootargs!\n");
			return ret;
		}
        }else{
		pr_err("[HWINFO]: failed to get /chosen!\n");
		return ret;
	} 
	
	if(strstr(bootargs,"Ufs_id=300")){
		ufs_vendor_id = 0;
	}else if(strstr(bootargs,"Ufs_id=408")){
		ufs_vendor_id = 1;
	}else if(strstr(bootargs,"Ufs_id=429")){
		ufs_vendor_id = 2;
	}else if(strstr(bootargs,"Ufs_id=462")){
		ufs_vendor_id = 3;
        }else if(strstr(bootargs,"Ufs_id=00")){
		ufs_vendor_id = 4;
	}else{
          pr_err("[HWINFO]: failed to get UfsMfrId !\n");
          return ret;
	}

	return ufs_vendor_id;
}

 static unsigned int get_rom_size(void)
 {
 	unsigned int ufs_size = 32;
 	struct  device_node * of_chosen = NULL;
 	char *bootargs = NULL;
 	int ret = -1;
 	...
 	if (strstr(bootargs,"Ufs_size=1024")) {
 		ufs_size = 1024;
 	}else if (strstr(bootargs,"Ufs_size=512")) {
 		ufs_size = 512;
 	} else if (strstr(bootargs,"Ufs_size=256")) {
 		ufs_size = 256;
 	}else if (strstr(bootargs,"Ufs_size=128")) {
 		ufs_size = 128;
 	} else if (strstr(bootargs,"Ufs_size=64")) {
 		ufs_size = 64;
 	} else if (strstr(bootargs,"Ufs_size=32")) {
 		ufs_size = 32;
 	} else if (strstr(bootargs,"Ufs_size=16")) {
 		ufs_size = 16;
 	} else if (strstr(bootargs,"Ufs_size=8")) {
 		ufs_size = 8;
 	} else if (strstr(bootargs,"Ufs_size=6")) {
 		ufs_size = 6;
 	} else if (strstr(bootargs,"Ufs_size=4")) {
 		ufs_size = 4;
 	} else {
 		ufs_size = 0;
 	}
...
static unsigned int get_ram_size(void)
{
	...
	if (temp_size > 23552) {
		ram_size = 24;
	} else if (temp_size > 15000) {
		ram_size = 16;
	} else if (temp_size > 11000) {
		ram_size = 12;
...
#define STAGE_ADC_NAME_MB "/sys/bus/iio/devices/iio:device1/in_voltage_get_stageid_adc_mb_input"
#define STAGE_ADC_NAME_KB "/sys/bus/iio/devices/iio:device1/in_voltage_get_stageid_adc_kb_input"
/*
char *get_stage_adc_kb(struct device *dev)
{
	int adc_val = 0;
	int adc_val_high = 0;
	int adc_val_low = 0;
	...
	if (IS_ERR(stageid_adc)) {
		rc = PTR_ERR(stageid_adc);
		if (rc != -EPROBE_DEFER)
			pr_err("stageid_adc channel unavailable, rc=%d\n", rc);
		stageid_adc = NULL;
		return "Error";
	}

	hwinfo_debug("[HWINFO] stageid_adc iio_channel_get ok\n");
	...
	for (i = 0; i < ARRAY_SIZE(pcba_stage_tabe); i++) {
		adc_val_high =
		    pcba_stage_tabe[i].stage_value_max;
		adc_val_low =
		    pcba_stage_tabe[i].stage_value_min;
		hwinfo_debug
...
//EXPORT_SYMBOL_GPL(get_stage_adc_kb);
char *get_adc_kb(void)
{
	return get_stage_adc_kb(lcm_dev);
}
EXPORT_SYMBOL_GPL(get_adc_kb);
*/
...
//	return sprintf(buf, "KBoard:%s\n",get_stage_adc_kb(dev));
	return sprintf(buf, "MBoard:%s\n", "PVT");
}
			  ...
			  char *buf)
{
  int ufs_vendor_id = 0;
  int ufs_size = 0;
  ufs_size = get_rom_size();
  ufs_vendor_id = get_ufs_vendor_id();
  pr_err("[HWINFO]:ufs_size==%d,ufs_vendor_id=%d\n",ufs_size,ufs_vendor_id);
  if(ufs_vendor_id == -1){
     return sprintf(buf, "VendorName:%s DDRSize:%dGB UfsSize:%dGB DDRType:%s\n",
                        "failed", get_ram_size(),ufs_size,DDR_TYPE);
  }
     return sprintf(buf, "VendorName:%s DDRSize:%dGB UfsSize:%dGB DDRType:%s\n",
                        vendor_table[ufs_vendor_id].name,get_ram_size(),ufs_size,DDR_TYPE);
}
...
static ssize_t show_als_rear(struct device *dev, struct device_attribute *attr,
			  char *buf)
{
	SENSOR_INFO_SHOW(hwinfo_data.als_rear, buf);
}

static ssize_t store_als_rear(struct device *dev, struct device_attribute *attr,
			   const char *buf, size_t count)
{
	SENSOR_INFO_STORE(hwinfo_data.als_rear, buf);
	return count;
}

static ssize_t show_press(struct device *dev, struct device_attribute *attr,
			  char *buf)
{
	SENSOR_INFO_SHOW(hwinfo_data.press, buf);
}

static ssize_t store_press(struct device *dev, struct device_attribute *attr,
			   const char *buf, size_t count)
{
	SENSOR_INFO_STORE(hwinfo_data.press, buf);
	return count;
}
...
static ssize_t show_main_cam_sn(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	if (strlen(hwinfo_data.main_cam_sn) > 0)
...
static ssize_t show_main_cam_2_sn(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	if (strlen(hwinfo_data.main_cam_2_sn) > 0)
...
static ssize_t show_main_cam_3_sn(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	if (strlen(hwinfo_data.main_cam_3_sn) > 0)
...
static ssize_t show_sub_cam_sn(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	if (strlen(hwinfo_data.sub_cam_sn) > 0)
...
{
	hwinfo_debug("[HWINFO] %s success.\n", __func__);
	return sprintf(buf, "PCBA_%s_%s", get_type_name(),
		       "PVT");
}
...
static DEVICE_ATTR(als_rear, 0644, show_als_rear, store_als_rear);
static DEVICE_ATTR(press, 0644, show_press, store_press);
...
static DEVICE_ATTR(fingerprint, 0644, show_fingerprint, store_fingerprint);
