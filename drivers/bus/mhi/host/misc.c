			      ...
			      u64 local_time, u64 remote_time)
{
	...
	MHI_LOG(dev, "Time response: seq:%x local: %llu remote: %llu (ticks)\n",
			       ...
			       char *buf)
{
	...
	if (ret) {
		MHI_ERR(dev, "Failed to request time, seq:%x, ret:%d\n", seq, ret);
		return scnprintf(buf, PAGE_SIZE,
				 "Request failed or feature unsupported\n");
	}
...
int mhi_report_error(struct mhi_controller *mhi_cntrl)
{
	...
	if (sfr_info && sfr_info->buf_addr) {
		...
		MHI_ERR(dev, "mhi: %s sfr: %s\n", dev_name(dev), (char *) sfr_info->buf_addr);
	}
				   ...
				   u32 event_quota)
{
	...
	MHI_VERB(dev, "Received TSYNC event with seq: 0x%x time: 0x%llx\n",
	...
	if (WARN_ON(mhi_tsync->int_sequence != sequence)) {
		MHI_ERR(dev, "Unexpected response: 0x%x Expected: 0x%x\n",
					...
					u64 remote_time))
{
	...
	MHI_VERB(dev, "time DB request with seq:0x%x\n", mhi_tsync->int_sequence);
