		...
		u32 new_mask, u32 press_mask, u32 release_mask, int release_all)
{
	...
	for (i = 0; i < ts->max_finger; i++, tdata++) {
		if (new_mask & BIT(i)) {
			if (press_mask & BIT(i)) {
				t_dev_dbg_event(ts->dev,
					"tID:%d.%d finger press (%4d, %4d, %4d) tc:%d %s\n",
					i, (input->mt->trkid - 1) & TRKID_MAX,
					tdata->x, tdata->y, tdata->pressure, tcount, iname);
				continue;
			}

			t_dev_dbg_event_m(ts->dev,
				"tID:%d.%d finger move (%4d, %4d, %4d) tc:%d %s\n",
				i, (input->mt->trkid - 1) & TRKID_MAX,
				tdata->x, tdata->y, tdata->pressure, tcount, iname);
			continue;
		}
		...
		if (release_mask & BIT(i)) {
			...
			t_dev_dbg_event(ts->dev,
				"tID:%d finger %s (%4d, %4d, %4d) tc:%d %s\n",
				i, rstr, tdata->x, tdata->y, tdata->pressure, tcount, iname);
		}
	}
}
#endif	/* USE_EVENT_LOG_V2 */
...
static int lxs_ts_setup(struct lxs_ts *ts)
{
	...
	ts->d_frame_rate_lp = TS_DISP_FRAME_30HZ;
	ts->t_frame_rate_lp = TS_TOUCH_FRAME_60HZ;
