...
static int lxs_hal_hw_reset(struct lxs_ts *ts, int ctrl, int pwr_con)
{
	...
	lxs_hal_reinit(ts, skip_quirk, pwr_con, 1);
...
static int lxs_hal_tc_con_glove(struct lxs_ts *ts)
{
	...
	if (chip->opt.f_glove_en)
		ret = lxs_hal_tc_con_type_g(ts, addr, value, "glove_en");
...
static int lxs_hal_tc_con_grab(struct lxs_ts *ts)
{
	...
	if (chip->opt.f_grab_en)
		ret = lxs_hal_tc_con_type_g(ts, addr, value, "grab_en");
...
static int lxs_hal_disp_frame_rate(struct lxs_ts *ts, int mode)
{
	struct lxs_ts_chip *chip = &ts->chip;
	struct lxs_hal_reg *reg = chip->reg;
	int value = (mode == LCD_MODE_U3) ? ts->d_frame_rate_np : ts->d_frame_rate_lp;
	...
	if ((value < TS_DISP_FRAME_OFF) || (value > TS_DISP_FRAME_120HZ))
		return 0;
...
static int lxs_hal_touch_frame_rate(struct lxs_ts *ts, int mode)
{
	struct lxs_ts_chip *chip = &ts->chip;
	struct lxs_hal_reg *reg = chip->reg;
	int value = (mode == LCD_MODE_U3) ? ts->t_frame_rate_np : ts->t_frame_rate_lp;
	...
	if ((value < TS_TOUCH_FRAME_60HZ) || (value > TS_TOUCH_FRAME_240HZ))
		return 0;
...
static void lxs_hal_enable_work_func(struct work_struct *work)
{
	...
	ret = lxs_ts_pw_lock(ts, INCELL_DISPLAY_POWER_LOCK);
	if (ret) {
	...
	ret = lxs_ts_pw_lock(ts, INCELL_DISPLAY_POWER_UNLOCK);
	if (ret)
				...
				u32 status, u32 ic_status, int irq)
{
	...
	if (!status && !ic_status) {
		t_dev_err(ts->dev, "[%d] all low detected\n", irq);
		return -ERESTART;
	}

	if ((status == ~0) && (ic_status == ~0)) {
		t_dev_err(ts->dev, "[%d] all high detected\n", irq);
		return -ERESTART;
	}
	...
	t_dev_dbg_irq_sts(ts->dev, "[%d] h/w:%Xh, f/w:%Xh(%Xh)\n",
	...
	case CHIP_STATUS_TYPE_0:
		...
		lxs_hal_check_dbg_report(ts, status, 1);
		break;
	default:
