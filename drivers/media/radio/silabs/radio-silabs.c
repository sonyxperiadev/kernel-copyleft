...
static int initialize_recv(struct silabs_fm_device *radio)
{
	...
	retval = set_property(radio, FM_DEBUG_FEATURE_PROP, 0);
	if (retval < 0)	{
		FMDERR("%s: FM_DEBUG_FEATURE_PROP fail error %d\n",
				__func__, retval);
		goto set_prop_fail;
	}
...
static int silabs_fm_set_stereo_mono_mode(struct file *file, void *priv,
	...
	const struct v4l2_tuner *tuner)
{
	...
	retval = silabs_fm_set_stereo_mono_mode(file, priv, tuner);
