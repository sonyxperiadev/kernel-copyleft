...
/* MM-MC-FixFlashLedCanNotWork-00+{ */
int msm_flash_lm3642_check_flags(struct msm_led_flash_ctrl_t *fctrl)
{
    int rc = 0;
    int i;
    uint16_t read_val = 0;
    uint16_t reg_addr = 0x0B;
    uint16_t check_count = 2;
    ...
    for (i = 0; i < check_count; i++)
    {
    	...
    	if (rc < 0) {
    		pr_err("%s %d: Read reg_addr(0x%x) failed !\n", __func__, __LINE__, reg_addr);
            goto ERROR;
    	}

        if (read_val == 0)
        {
            CDBG("%s %d: Checking %d reg_addr(0x%x) = 0x%x, success. \n", __func__, __LINE__, i, reg_addr, read_val);
            break;
        }
        else
            CDBG("%s %d: Checking %d reg_addr(0x%x) = 0x%x ~\n", __func__, __LINE__, i, reg_addr, read_val);
    }

    if (read_val)
    {
        pr_err("%s %d: reg_addr(0x%x) = 0x%x, check flag failed !\n", __func__, __LINE__, reg_addr, read_val);
        rc = -EINVAL;
        goto ERROR;
    }
...
int msm_flash_lm3642_led_low(struct msm_led_flash_ctrl_t *fctrl)
{
    ...
    /* MM-MC-FixFlashLedCanNotWork-00+{ */
    rc = msm_flash_lm3642_check_flags(fctrl);
    if (rc < 0)
    {
        pr_err("%s:%d: msm_flash_lm3642_check_flags() failed !\n", __func__, __LINE__);
        return rc;
    }
    /* MM-MC-FixFlashLedCanNotWork-00+} */
