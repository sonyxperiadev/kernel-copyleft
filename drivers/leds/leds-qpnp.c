...
#ifdef CONFIG_LEDS_QPNP_RGB_SCALE
static int rgb_scale = 100;

static int scale_brightness(const struct qpnp_led_data *led)
{
	int brightness = led->cdev.brightness;

	if (rgb_scale != 100) {
		brightness = (brightness*rgb_scale + 50) / 100;
		brightness = clamp(brightness,
				1, (int)led->cdev.max_brightness);
	}
...
static void qpnp_rgb_set_duration(struct qpnp_led_data *led)
{
	...
	const unsigned max_pause = RGB_LED_MAX_PAUSE_REG;
	const unsigned max_pause_time = RGB_LED_MAX_PAUSE_TIME;

	if (led->rgb_cfg->pwm_cfg->lut_params.lut_pause_hi > max_pause_time)
		led->rgb_cfg->pwm_cfg->lut_params.lut_pause_hi
					= max_pause_time;
	if (led->rgb_cfg->pwm_cfg->lut_params.lut_pause_lo > max_pause_time)
		led->rgb_cfg->pwm_cfg->lut_params.lut_pause_lo
					= max_pause_time;

	long_pause = ((led->rgb_cfg->pwm_cfg->lut_params.lut_pause_hi
			>= led->rgb_cfg->pwm_cfg->lut_params.lut_pause_lo) ?
			led->rgb_cfg->pwm_cfg->lut_params.lut_pause_hi :
			led->rgb_cfg->pwm_cfg->lut_params.lut_pause_lo);
	duration = (long_pause + (max_pause - 1)) / max_pause;
	led->rgb_cfg->pwm_cfg->lut_params.lut_pause_hi
	= (led->rgb_cfg->pwm_cfg->lut_params.lut_pause_hi / duration) + 1;
	led->rgb_cfg->pwm_cfg->lut_params.lut_pause_lo
	= (led->rgb_cfg->pwm_cfg->lut_params.lut_pause_lo / duration) + 1;
	led->rgb_cfg->pwm_cfg->lut_params.ramp_step_ms = (duration - 1);
	led->rgb_cfg->pwm_cfg->lut_params.use_duration = false;
			...
			const char *buf, size_t count)
{
	...
	led->rgb_cfg->pwm_cfg->duty_cycles->start_idx
		...
		= (led->rgb_cfg->pwm_cfg->pwm_channel * INDEX_MAX_EACH_LED)
				+ RGB_LUT_INDEX_OFFSET;
	led->rgb_cfg->pwm_cfg->lut_params.idx_len = ret;

	dev_err(dev, "%s: start_idx=%d , idx_len=%d, [0]:%d, [1]:%d\n", __func__,
		led->rgb_cfg->pwm_cfg->duty_cycles->start_idx,
			...
			const char *buf, size_t count)
{
	...
	const unsigned lut_pause_max = RGB_LED_LUT_MAX_PAUSE;
	...
	if (lut_pause > lut_pause_max)
		led->rgb_cfg->pwm_cfg->lut_params.use_duration = true;
...
static struct device_attribute rgbled_attrs[] = {
	...
	__ATTR(max_single_brightness, S_IRUGO,
		rgbled_single_value_show, NULL),
	__ATTR(max_mix_brightness, S_IRUGO,
			...
			const char *buf, size_t count)
{
	...
	dev_err(dev, "%s: rgb_sync->sync_state from %d to %lu\n",
			__func__, rgb_sync->sync_state, state);
			...
			const char *buf, size_t count)
{
	...
	for (i = 0; i < LPG_CHANNEL_MAX; ++i) {
		...
		if (led->rgb_cfg->pwm_cfg->lut_params.use_duration)
...
static int qpnp_rgb_init(struct qpnp_led_data *led)
{
	...
	if (led->rgb_sync) {
		...
		rc = pwm_change_mode(led->rgb_cfg->pwm_cfg->pwm_dev,
			PM_PWM_MODE_PWM);
		if (rc < 0) {
			dev_err(&led->pdev->dev, "Failed to " \
				"configure pwm mode\n");
			return rc;
		}
	}
				...
				struct device_node *node)
{
	...
	if (rc < 0) {
		dev_err(&led->pdev->dev, "Unable to read color_variation_max_num\n");
		color_variation_max_num = RGB_CURR_DEFAULT_PATTERN;
	}
	dev_info(&led->pdev->dev, "color_variation_max_num[%d] rgb_current_index[%d]\n",
		color_variation_max_num, rgb_current_index);

	rgb_current_table = devm_kzalloc(&led->pdev->dev,
		sizeof(u32) * color_variation_max_num * RGB_CURR_UNIT_NUM,
		GFP_KERNEL);
	if (rgb_current_table) {
		rc = of_property_read_u32_array(node, "somc,max_current",
				rgb_current_table,
				color_variation_max_num * RGB_CURR_UNIT_NUM);
		if (rc < 0) {
			dev_err(&led->pdev->dev, "Unable to read max_mix_current\n");
			led->rgb_cfg->single_pwm_value = RGB_MAX_LEVEL - 1;
			led->rgb_cfg->mix_pwm_value = RGB_MAX_LEVEL - 1;
		} else {
			for (i = 0; i < color_variation_max_num; i++) {
				current_index = i * RGB_CURR_UNIT_NUM;
	...
	devm_kfree(&led->pdev->dev, rgb_current_table);

	led->max_current = (led->rgb_cfg->single_pwm_value >
				led->rgb_cfg->mix_pwm_value ?
				led->rgb_cfg->single_pwm_value :
				led->rgb_cfg->mix_pwm_value);
	dev_info(&led->pdev->dev, "single_pwm_value[%d] mix_pwm_value[%d]\n",
			led->rgb_cfg->single_pwm_value, led->rgb_cfg->mix_pwm_value);
	dev_info(&led->pdev->dev, "max_current[%d]\n", led->max_current);

	if (led->rgb_sync) {
		rc = of_property_read_u32(node, "somc,pwm-channel", &val);
		if (!rc)
		...
		led->rgb_cfg->pwm_cfg->pwm_dev = of_pwm_get(node, NULL);
		return 0;
	}
...
#ifdef CONFIG_LEDS_QPNP_RGB_SCALE
static int update_rgb_brightness(struct device *dev, void *arg)
{
	struct qpnp_led_data *led = dev_get_drvdata(dev);
	int i;

	for (i = 0; i < led->num_leds; i++) {
		if (led[i].id != QPNP_ID_RGB_RED &&
		    led[i].id != QPNP_ID_RGB_GREEN &&
		    led[i].id != QPNP_ID_RGB_BLUE) {
			continue;
		}

		dev_info(led[i].cdev.dev, "brightness scale %d%%\n", rgb_scale);

		qpnp_led_set(&led[i].cdev, led[i].cdev.brightness);
	}

	return 0;
}

int qpnp_led_set_rgb_scale(int scale)
{
	int ret = 0;
	static DEFINE_MUTEX(scale_lock);

	if (!qpnp_leds_driver.driver.bus)
		return -ENODEV;

	if (scale < 1 || scale > 100)
		return -EINVAL;

	mutex_lock(&scale_lock);

	if (scale != rgb_scale) {
		rgb_scale = scale;
		ret = driver_for_each_device(&qpnp_leds_driver.driver,
			NULL, NULL, update_rgb_brightness);
	}

	mutex_unlock(&scale_lock);
