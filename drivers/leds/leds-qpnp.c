				...
				struct device_node *node)
{
	...
	if (rc < 0) {
		dev_err(&led->spmi_dev->dev, "Unable to read color_variation_max_num\n");
		color_variation_max_num = RGB_CURR_DEFAULT_PATTERN;
	}
	dev_info(&led->spmi_dev->dev, "color_variation_max_num[%d] rgb_current_index[%d]\n",
		color_variation_max_num, rgb_current_index);

	rgb_current_table = devm_kzalloc(&led->spmi_dev->dev,
	...
	if (rgb_current_table) {
		...
		if (rc < 0) {
			dev_err(&led->spmi_dev->dev, "Unable to read max_mix_current\n");
	...
	devm_kfree(&led->spmi_dev->dev, rgb_current_table);

	led->max_current = (led->rgb_cfg->single_pwm_value >
				led->rgb_cfg->mix_pwm_value ?
				led->rgb_cfg->single_pwm_value :
				led->rgb_cfg->mix_pwm_value);
	dev_info(&led->spmi_dev->dev, "single_pwm_value[%d] mix_pwm_value[%d]\n",
			led->rgb_cfg->single_pwm_value, led->rgb_cfg->mix_pwm_value);
	dev_info(&led->spmi_dev->dev, "max_current[%d]\n", led->max_current);
