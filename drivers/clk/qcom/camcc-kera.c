...
static struct clk_vdd_class *cam_cc_kera_regulators[] = {
	&vdd_cx,
	&vdd_mx,
};
...
static const struct clk_parent_data cam_cc_parent_data_0_ao[] = {
	{ .fw_name = "bi_tcxo_ao" },
	{ .hw = &cam_cc_pll0.clkr.hw },
	{ .hw = &cam_cc_pll0_out_even.clkr.hw },
	{ .hw = &cam_cc_pll0_out_odd.clkr.hw },
...
static struct clk_rcg2 cam_cc_camnoc_rt_axi_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = cam_cc_kera_regulators,
		.num_vdd_classes = ARRAY_SIZE(cam_cc_kera_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
			[VDD_LOWER] = 300000000,
			[VDD_LOW] = 400000000},
...
static struct clk_rcg2 cam_cc_slow_ahb_clk_src = {
	...
	.clkr.hw.init = &(const struct clk_init_data) {
		...
		.parent_data = cam_cc_parent_data_0_ao,
		.num_parents = ARRAY_SIZE(cam_cc_parent_data_0_ao),
		.flags = CLK_SET_RATE_PARENT,
		.ops = &clk_rcg2_ops,
	},
};
...
static struct clk_regmap *cam_cc_kera_clocks[] = {
...
static const struct qcom_reset_map cam_cc_kera_resets[] = {
...
static const struct regmap_config cam_cc_kera_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static struct qcom_cc_desc cam_cc_kera_desc = {
	...
	.resets = cam_cc_kera_resets,
	.num_resets = ARRAY_SIZE(cam_cc_kera_resets),
	.clk_regulators = cam_cc_kera_regulators,
	.num_clk_regulators = ARRAY_SIZE(cam_cc_kera_regulators),
};

static const struct of_device_id cam_cc_kera_match_table[] = {
	{ .compatible = "qcom,kera-camcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, cam_cc_kera_match_table);

static int cam_cc_kera_probe(struct platform_device *pdev)
{
	...
	clk_lucid_ole_pll_configure(&cam_cc_pll2, regmap, &cam_cc_pll2_config);
	...
	ret = qcom_cc_really_probe(pdev, &cam_cc_kera_desc, regmap);
	if (ret) {
...
static void cam_cc_kera_sync_state(struct device *dev)
{
...
static int __init cam_cc_kera_init(void)
{
	return platform_driver_register(&cam_cc_kera_driver);
}
subsys_initcall(cam_cc_kera_init);

static void __exit cam_cc_kera_exit(void)
{
	platform_driver_unregister(&cam_cc_kera_driver);
}
module_exit(cam_cc_kera_exit);

MODULE_DESCRIPTION("QTI CAM_CC KERA Driver");
MODULE_LICENSE("GPL");
