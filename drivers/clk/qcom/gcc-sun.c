...
static struct clk_vdd_class *gcc_sun_regulators[] = {
	&vdd_cx,
	&vdd_mx,
};
...
static struct clk_rcg2 gcc_pcie_0_aux_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = gcc_sun_regulators,
		.num_vdd_classes = ARRAY_SIZE(gcc_sun_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
			[VDD_LOWER] = 19200000},
	},
...
static const struct regmap_config gcc_sun_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct qcom_cc_desc gcc_sun_desc = {
	.config = &gcc_sun_regmap_config,
	.clks = gcc_sun_clocks,
	.num_clks = ARRAY_SIZE(gcc_sun_clocks),
	.resets = gcc_sun_resets,
...
static const struct of_device_id gcc_sun_match_table[] = {
	{ .compatible = "qcom,sun-gcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, gcc_sun_match_table);

static int gcc_sun_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &gcc_sun_desc);
	if (IS_ERR(regmap))
	...
	ret = qcom_cc_really_probe(pdev, &gcc_sun_desc, regmap);
	if (ret) {
...
static void gcc_sun_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &gcc_sun_desc);
}

static struct platform_driver gcc_sun_driver = {
	.probe = gcc_sun_probe,
	.driver = {
		.name = "gcc-sun",
		.of_match_table = gcc_sun_match_table,
		.sync_state = gcc_sun_sync_state,
	},
};

static int __init gcc_sun_init(void)
{
	return platform_driver_register(&gcc_sun_driver);
}
subsys_initcall(gcc_sun_init);

static void __exit gcc_sun_exit(void)
{
	platform_driver_unregister(&gcc_sun_driver);
}
module_exit(gcc_sun_exit);

MODULE_DESCRIPTION("QTI GCC SUN Driver");
MODULE_LICENSE("GPL");
