...
static struct clk_vdd_class *gcc_kera_regulators[] = {
	&vdd_cx,
	&vdd_mx,
};
...
static struct clk_rcg2 gcc_pcie_0_aux_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = gcc_kera_regulators,
		.num_vdd_classes = ARRAY_SIZE(gcc_kera_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
			[VDD_LOWER] = 19200000},
	},
...
static struct clk_regmap *gcc_kera_clocks[] = {
...
static const struct qcom_reset_map gcc_kera_resets[] = {
...
static const struct regmap_config gcc_kera_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct qcom_cc_desc gcc_kera_desc = {
	.config = &gcc_kera_regmap_config,
	.clks = gcc_kera_clocks,
	.num_clks = ARRAY_SIZE(gcc_kera_clocks),
	.resets = gcc_kera_resets,
	.num_resets = ARRAY_SIZE(gcc_kera_resets),
	.clk_regulators = gcc_kera_regulators,
	.num_clk_regulators = ARRAY_SIZE(gcc_kera_regulators),
};

static const struct of_device_id gcc_kera_match_table[] = {
	{ .compatible = "qcom,kera-gcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, gcc_kera_match_table);

static int gcc_kera_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &gcc_kera_desc);
	if (IS_ERR(regmap))
	...
	ret = qcom_cc_really_probe(pdev, &gcc_kera_desc, regmap);
	if (ret) {
...
static void gcc_kera_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &gcc_kera_desc);
}

static struct platform_driver gcc_kera_driver = {
	.probe = gcc_kera_probe,
	.driver = {
		.name = "gcc-kera",
		.of_match_table = gcc_kera_match_table,
		.sync_state = gcc_kera_sync_state,
	},
};

static int __init gcc_kera_init(void)
{
	return platform_driver_register(&gcc_kera_driver);
}
subsys_initcall(gcc_kera_init);

static void __exit gcc_kera_exit(void)
{
	platform_driver_unregister(&gcc_kera_driver);
}
module_exit(gcc_kera_exit);

MODULE_DESCRIPTION("QTI GCC KERA Driver");
MODULE_LICENSE("GPL");
