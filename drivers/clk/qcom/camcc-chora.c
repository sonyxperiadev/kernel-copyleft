...
#include <dt-bindings/clock/qcom,chora-camcc.h>
...
static struct clk_vdd_class *cam_cc_chora_regulators[] = {
	&vdd_cx,
	&vdd_mx,
};
...
static struct clk_rcg2 cam_cc_bps_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = cam_cc_chora_regulators,
		.num_vdd_classes = ARRAY_SIZE(cam_cc_chora_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
			[VDD_LOWER] = 300000000,
			[VDD_LOW] = 410000000,
...
static const struct freq_tbl ftbl_cam_cc_ope_0_clk_src[] = {
	...
	F(530000000, P_CAM_CC_PLL3_OUT_EVEN, 1, 0, 0),
...
static const struct freq_tbl ftbl_cam_cc_tfe_0_clk_src[] = {
	...
	F(500000000, P_CAM_CC_PLL4_OUT_EVEN, 1, 0, 0),
	F(600000000, P_CAM_CC_PLL4_OUT_EVEN, 1, 0, 0),
	F(700000000, P_CAM_CC_PLL4_OUT_EVEN, 1, 0, 0),
...
static struct gdsc *cam_cc_chora_gdscs[] = {
	[CAM_CC_CAMSS_TOP_GDSC] = &cam_cc_camss_top_gdsc,
};

static const struct qcom_reset_map cam_cc_chora_resets[] = {
...
static const struct regmap_config cam_cc_chora_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct qcom_cc_desc cam_cc_chora_desc = {
	.config = &cam_cc_chora_regmap_config,
	.clks = cam_cc_chora_clocks,
	.num_clks = ARRAY_SIZE(cam_cc_chora_clocks),
	.resets = cam_cc_chora_resets,
	.num_resets = ARRAY_SIZE(cam_cc_chora_resets),
	.clk_regulators = cam_cc_chora_regulators,
	.num_clk_regulators = ARRAY_SIZE(cam_cc_chora_regulators),
	.gdscs = cam_cc_chora_gdscs,
	.num_gdscs = ARRAY_SIZE(cam_cc_chora_gdscs),
};

static const struct of_device_id cam_cc_chora_match_table[] = {
	{ .compatible = "qcom,chora-camcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, cam_cc_chora_match_table);

static int cam_cc_chora_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &cam_cc_chora_desc);
	if (IS_ERR(regmap))
	 ...
	 *	cam_cc_gdsc_clk
	 */
	regmap_update_bits(regmap, 0x14038, BIT(0), BIT(0));

	ret = qcom_cc_really_probe(&pdev->dev, &cam_cc_chora_desc, regmap);
	if (ret) {
...
static void cam_cc_chora_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &cam_cc_chora_desc);
}

static struct platform_driver cam_cc_chora_driver = {
	.probe = cam_cc_chora_probe,
	.driver = {
		.name = "camcc-chora",
		.of_match_table = cam_cc_chora_match_table,
		.sync_state = cam_cc_chora_sync_state,
	},
};

module_platform_driver(cam_cc_chora_driver);

MODULE_DESCRIPTION("QTI CAMCC CHORA Driver");
MODULE_LICENSE("GPL");
