...
{
	struct fusb301_chip *chip = container_of(self,
				struct fusb301_chip, fb_notif);
	...
	if (blanked == chip->lcd_blanked)
		return 0;

	chip->lcd_blanked = blanked;
	...
	if (gpio_is_valid(chip->pdata->cbl_sns_gpio)) {
		ret = gpio_direction_output(chip->pdata->cbl_sns_gpio,
					blanked ? 0 : 1);
		if (ret) {
			dev_err(cdev, "%s(): failure to enable cbl_sns_gpio %d\n",
					__func__, chip->pdata->cbl_sns_gpio);
			return -EINVAL;
		}
	}
			...
			const struct i2c_device_id *id)
{
	...
	if (fb_register_client(&chip->fb_notif))
		dev_err(cdev, "failed to register fb_notifier\n");
#endif
	...
	/* start to enable cable sense */
	if (gpio_is_valid(chip->pdata->cbl_sns_gpio)) {
		ret = gpio_direction_output(chip->pdata->cbl_sns_gpio, 1);
		if (ret) {
