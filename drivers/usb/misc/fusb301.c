...
#define FUSB301_NOT_USE_USB_PSY
...
#define DWORK_LOOP_MAX			5

#define DETACH_TO_SNK_BLIND_SECT	2000
...
static const char *fusb301_get_state_string(u8 state)
{
	const char *ret;

	switch (state) {
	case FUSB_STATE_DISABLED:
		ret = "FUSB_STATE_DISABLED";
		break;
	case FUSB_STATE_ERROR_RECOVERY:
		ret = "FUSB_STATE_ERROR_RECOVERY";
		break;
	case FUSB_STATE_UNATTACHED_SNK:
		ret = "FUSB_STATE_UNATTACHED_SNK";
		break;
	case FUSB_STATE_UNATTACHED_SRC:
		ret = "FUSB_STATE_UNATTACHED_SRC";
		break;
	case FUSB_STATE_ATTACHWAIT_SNK:
		ret = "FUSB_STATE_ATTACHWAIT_SNK";
		break;
	case FUSB_STATE_ATTACHWAIT_SRC:
		ret = "FUSB_STATE_ATTACHWAIT_SRC";
		break;
	case FUSB_STATE_ATTACHED_SNK:
		ret = "FUSB_STATE_ATTACHED_SNK";
		break;
	case FUSB_STATE_ATTACHED_SRC:
		ret = "FUSB_STATE_ATTACHED_SRC";
		break;
	case FUSB_STATE_AUDIO_ACCESSORY:
		ret = "FUSB_STATE_AUDIO_ACCESSORY";
		break;
	case FUSB_STATE_DEBUG_ACCESSORY:
		ret = "FUSB_STATE_DEBUG_ACCESSORY";
		break;
	case FUSB_STATE_TRY_SNK:
		ret = "FUSB_STATE_TRY_SNK";
		break;
	case FUSB_STATE_TRYWAIT_SRC:
		ret = "FUSB_STATE_TRYWAIT_SRC";
		break;
	case FUSB_STATE_TRY_SRC:
		ret = "FUSB_STATE_TRY_SRC";
		break;
	case FUSB_STATE_TRYWAIT_SNK:
		ret = "FUSB_STATE_TRYWAIT_SNK";
		break;
	default:
		ret = "UNKNOWN";
		break;
...
static int set_property_on_battery(struct fusb301_chip *chip,
...
static int fusb301_set_chip_state(struct fusb301_chip *chip, u8 state)
{
	...
	/* if wrote ERROR_REC, wait 1ms for mode transition */
	if (state == FUSB_STATE_ERROR_RECOVERY)
				...
				const char *buff, size_t size)
{
	...
	if (sscanf(buff, "%d", &mode) == 1) {
		...
		if (IS_ERR_VALUE(rc)) {
			...
			dev_err(cdev, "%s: chip is not respond, reset\n",
								__func__);
			mutex_unlock(&chip->mlock);
			return -EDEADLK;
		}
...
static int fusb301_change_charge_type(struct fusb301_chip *chip,
				enum power_supply_type type)
{
	...
	case POWER_SUPPLY_TYPE_USB_HVDCP_3:
		if (chip->state == FUSB_STATE_ATTACHED_SNK)
			val = FUSB301_INT_DISABLE;
		else
			return 0;
		break;
	case POWER_SUPPLY_TYPE_UNKNOWN:
		val = FUSB301_INT_ENABLE;
		break;
	default:
		return 0;
	}

	if (chip->bclvl_masked == val)
		return 0;

	dev_dbg(cdev, "%s: set bc_lvl int mask %s\n", __func__,
			val == FUSB301_INT_ENABLE ? "ENABLE" : "DISABLE");
						...
						int icurrent)
{
	...
	return set_property_on_battery(chip,
...
static void fusb301_bclvl_changed(struct fusb301_chip *chip)
{
	...
	if (type == FUSB301_TYPE_SRC ||
		...
		limit = (chip->bc_lvl == FUSB301_SNK_3000MA ? 3000 :
			(chip->bc_lvl == FUSB301_SNK_1500MA ? 1500 : 500));
...
static void fusb301_snk_detected(struct fusb301_chip *chip)
{
	...
	if ((!chip->triedsnk) &&
		...
		if (IS_ERR_VALUE(fusb301_set_mode(chip, FUSB301_SNK)) ||
			...
			chip->pdata->ttry_timeout = 0x000001ff & rnd;
			dev_dbg(cdev, "ttry=%ums\n", chip->pdata->ttry_timeout);
...
static void fusb301_timer_try_expired(struct fusb301_chip *chip)
{
	...
	if (IS_ERR_VALUE(fusb301_set_mode(chip, FUSB301_SRC)) ||
		...
		get_random_bytes((void *)&rnd, 4);
		chip->pdata->ccdebounce_timeout = (0x000000ff & rnd) + 200;
		dev_dbg(cdev, "ccdebounce=%ums\n",
					chip->pdata->ccdebounce_timeout);
...
static void fusb301_detach(struct fusb301_chip *chip)
{
	...
	fusb301_change_charge_type(chip, POWER_SUPPLY_TYPE_UNKNOWN);
	...
	if (chip->pdata->ext_typec_ctrl_enabled &&
				chip->ext_typec_ctrl == POWER_SUPPLY_TYPE_UFP) {
		dev_info(cdev, "%s: changed to SNK by ext, start blind section.\n",
								 __func__);
		chip->is_bs = true;
		wake_lock_timeout(&chip->wlock,
			msecs_to_jiffies(DETACH_TO_SNK_BLIND_SECT + 100));
		queue_delayed_work(chip->cc_wq, &chip->bwork,
				msecs_to_jiffies(DETACH_TO_SNK_BLIND_SECT));
	}
}
...
static void fusb301_attach(struct fusb301_chip *chip)
{
	...
	if (chip->pdata->ext_typec_ctrl_enabled) {
		dev_dbg(cdev, "%s: something connected, chang to DRP.\n",
								__func__);
...
static int fusb301_init_gpio(struct fusb301_chip *chip)
{
	...
	if (gpio_is_valid(chip->pdata->cbl_det_gpio)) {
		rc = gpio_request_one(chip->pdata->cbl_det_gpio,
				GPIOF_DIR_IN, "fusb301_cbl_det_gpio");
		if (rc) {
			dev_err(cdev, "unable to request cbl_det_gpio %d\n",
					chip->pdata->cbl_det_gpio);
			return rc;
		}
	}
	...
	if (gpio_is_valid(chip->pdata->cbl_sns_gpio)) {
		...
		if (rc)
			dev_err(cdev, "unable to request cbl_sns_gpio %d\n",
...
{
	struct fusb301_chip *chip =
		container_of(work, struct fusb301_chip, cbl_det_work);
	struct device *cdev = &chip->client->dev;
	unsigned int val;

	val = gpio_get_value(chip->pdata->cbl_det_gpio);
	dev_info(cdev, "%s : cable det %d, state=%s\n", __func__,
				val, fusb301_get_state_string(chip->state));
	mutex_lock(&chip->mlock);
	chip->cbl_det = val ? false : true;
	if (!chip->cbl_det)
		fusb301_set_mode(chip, FUSB301_SNK);
	else
		fusb301_set_mode(chip, FUSB301_DRP);
...
{
	struct fusb301_chip *chip = (struct fusb301_chip *)handle;
	struct device *cdev = &chip->client->dev;
	...
	if (!chip->pdata->cbl_det_enabled) {
		dev_dbg(cdev, "%s : cable detection was disabled.\n", __func__);
		return IRQ_HANDLED;
	}

	schedule_work(&chip->cbl_det_work);
	return IRQ_HANDLED;
}
...
static int fusb301_parse_dt(struct fusb301_chip *chip)
{
	...
	data->cbl_det_enabled = of_property_read_bool(dev_node,
						"fusb301,cbl_det-enabled");
	...
	data->ext_typec_ctrl_enabled = of_property_read_bool(dev_node,
...
static enum power_supply_property fusb301_properties[] = {
	POWER_SUPPLY_PROP_CURRENT_CAPABILITY,
	POWER_SUPPLY_PROP_TYPE,
};

static int fusb301_property_is_writeable(struct power_supply *psy,
				enum power_supply_property prop)
{
	switch (prop) {
...
static void fusb301_change_charger_type_work(struct work_struct *work)
{
	struct fusb301_chip *chip =
			container_of(work, struct fusb301_chip, cwork);
	struct device *cdev = &chip->client->dev;

	mutex_lock(&chip->mlock);
	dev_dbg(cdev, "%s: change_charger_type=%d\n", __func__,
							chip->charger_type);
	fusb301_change_charge_type(chip, chip->charger_type);
	mutex_unlock(&chip->mlock);
}

static void fusb301_change_ext_typec_ctrl_work(struct work_struct *work)
{
	struct fusb301_chip *chip =
			container_of(work, struct fusb301_chip, ework);
	struct device *cdev = &chip->client->dev;
	...
	if (chip->ext_typec_ctrl == POWER_SUPPLY_TYPE_TYPEC)
		...
		dev_dbg(cdev, "changed to SNK by ext, but something connected now.\n");
	mutex_unlock(&chip->mlock);
}

static void fusb301_typec_ctrl_bs_work(struct work_struct *work)
{
	struct fusb301_chip *chip =
			container_of(work, struct fusb301_chip, bwork.work);
	struct device *cdev = &chip->client->dev;

	mutex_lock(&chip->mlock);
	if (chip->state == FUSB_STATE_ERROR_RECOVERY &&
				chip->ext_typec_ctrl == POWER_SUPPLY_TYPE_UFP) {
		dev_info(cdev, "%s: expired blind section, set SNK.\n",
								 __func__);
		fusb301_set_mode(chip, FUSB301_SNK);
	}
	chip->is_bs = false;
				...
				const union power_supply_propval *val)
{
	...
	switch (prop) {
	case POWER_SUPPLY_PROP_TYPE:
		cancel_work_sync(&chip->cwork);
		chip->charger_type = val->intval;
		queue_work(chip->cc_wq, &chip->cwork);
		break;
	case POWER_SUPPLY_PROP_TYPEC_MODE:
		...
		if (chip->pdata->ext_typec_ctrl_enabled) {
			wake_lock_timeout(&chip->wlock,
				msecs_to_jiffies(FUSB301_WAKE_LOCK_TIMEOUT));
			queue_work(chip->cc_wq, &chip->ework);
		}
		break;
	default:
		return -EINVAL;
...
static int fusb301_get_property(struct power_supply *psy,
				enum power_supply_property prop,
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_CAPABILITY:
		...
		break;
	case POWER_SUPPLY_PROP_TYPEC_MODE:
		val->intval = chip->ext_typec_ctrl;
		break;
	default:
		return -EINVAL;
	}
			...
			const struct i2c_device_id *id)
{
	...
	chip->cc_wq = create_singlethread_workqueue("fusb301-wq");
	if (!chip->cc_wq) {
	...
	INIT_DELAYED_WORK(&chip->bwork, fusb301_typec_ctrl_bs_work);
	...
	if (chip->pdata->cbl_det_enabled) {
		dev_info(cdev, "cbl_det_enabled\n");

		fusb301_set_mode(chip, FUSB301_SNK);
		INIT_WORK(&chip->cbl_det_work, fusb301_cbl_det_work);
	}

	chip->irq_cbl_det = gpio_to_irq(chip->pdata->cbl_det_gpio);
	...
	ret = devm_request_irq(cdev, chip->irq_cbl_det,
				fusb301_irq_cbl_det,
				IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
				"fusb301_cbl_det_irq", chip);
	if (ret) {
		dev_err(cdev, "failed to request IRQ\n");
		goto err5;
	}

	ret = enable_irq_wake(chip->irq_cbl_det);
	if (ret < 0) {
		dev_err(cdev, "failed to enable wakeup src %d\n", ret);
		goto err6;
	}

	INIT_WORK(&chip->ework, fusb301_change_ext_typec_ctrl_work);
	INIT_WORK(&chip->cwork, fusb301_change_charger_type_work);

	if (chip->pdata->ext_typec_ctrl_enabled) {
		dev_info(cdev, "ext_typec_ctrl_enabled\n");
		fusb301_set_mode(chip, FUSB301_SNK);
	}
	...
	if (chip->pdata->cbl_sns_enabled) {
		dev_info(cdev, "cbl_sns_enabled\n");

		/* start to enable cable sense */
		if (gpio_is_valid(chip->pdata->cbl_sns_gpio)) {
			ret = gpio_direction_output(chip->pdata->cbl_sns_gpio,
									1);
			if (ret) {
