...
static void wdet_check_water_work(struct work_struct *w)
{
	...
	if (wdet->cable_timer == 0) {
		...
		if (!or) {
			...
			wake_lock_timeout(&wdet->check_lock,
				msecs_to_jiffies(WDET_WAKELOCK_TIMEOUT));
...
struct wdet *wdet_create(struct device *parent)
{
	...
	wake_lock_init(&wdet->check_lock, WAKE_LOCK_SUSPEND, "wdet_wakelock");
