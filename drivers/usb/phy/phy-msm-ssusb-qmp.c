...
unsigned int ssphy_debug_log_enable;
...
module_param(ssphy_debug_log_enable, uint, 0644);
...
MODULE_PARM_DESC(ssphy_debug_log_enable, " Enable/Disable debug log");
...
static void msm_ssphy_param_output(struct usb_phy *uphy)
{
	...
	if (ssphy_debug_log_enable) {
		/* USB3_TXA */
		dev_info(uphy->dev, "USB3:TXA_TX_DRV_LVL      :0x%02x\n",
			readb_relaxed(phy->base + USB3_DP_QSERDES_TXA_TX_DRV_LVL));
		dev_info(uphy->dev, "USB3:TXA_PRE_EMPH        :0x%02x\n",
			readb_relaxed(phy->base + USB3_DP_QSERDES_TXA_PRE_EMPH));
		dev_info(uphy->dev, "USB3:TXA_TX_EMP_POST1_LVL:0x%02x\n",
			readb_relaxed(phy->base +
						USB3_DP_QSERDES_TXA_TX_EMP_POST1_LVL));
		dev_info(uphy->dev, "USB3:RXA_RX_EQU_ADAPTOR_CNTRL2:0x%02x\n",
			readb_relaxed(phy->base +
					USB3_DP_QSERDES_RXA_RX_EQU_ADAPTOR_CNTRL2));
		dev_info(uphy->dev, "USB3:RXA_RX_EQU_ADAPTOR_CNTRL3:0x%02x\n",
			readb_relaxed(phy->base +
					USB3_DP_QSERDES_RXA_RX_EQU_ADAPTOR_CNTRL3));
		dev_info(uphy->dev, "USB3:RXA_RX_EQU_ADAPTOR_CNTRL4:0x%02x\n",
			readb_relaxed(phy->base +
					USB3_DP_QSERDES_RXA_RX_EQU_ADAPTOR_CNTRL4));

		/* USB3_TXB */
		dev_info(uphy->dev, "USB3:TXB_TX_DRV_LVL      :0x%02x\n",
			readb_relaxed(phy->base + USB3_DP_QSERDES_TXB_TX_DRV_LVL));
		dev_info(uphy->dev, "USB3:TXB_PRE_EMPH        :0x%02x\n",
			readb_relaxed(phy->base + USB3_DP_QSERDES_TXB_PRE_EMPH));
		dev_info(uphy->dev, "USB3:TXB_TX_EMP_POST1_LVL:0x%02x\n",
			readb_relaxed(phy->base +
					USB3_DP_QSERDES_TXB_TX_EMP_POST1_LVL));
		dev_info(uphy->dev, "USB3:RXB_RX_EQU_ADAPTOR_CNTRL2:0x%02x\n",
			readb_relaxed(phy->base +
					USB3_DP_QSERDES_RXB_RX_EQU_ADAPTOR_CNTRL2));
		dev_info(uphy->dev, "USB3:RXB_RX_EQU_ADAPTOR_CNTRL3:0x%02x\n",
			readb_relaxed(phy->base +
					USB3_DP_QSERDES_RXB_RX_EQU_ADAPTOR_CNTRL3));
		dev_info(uphy->dev, "USB3:RXB_RX_EQU_ADAPTOR_CNTRL4:0x%02x\n",
			readb_relaxed(phy->base +
					USB3_DP_QSERDES_RXB_RX_EQU_ADAPTOR_CNTRL4));

		/* USB3_RXA */
		dev_info(uphy->dev, "USB3:RXA_RX_MODE_01_HIGH4     :0x%02x\n",
			readb_relaxed(phy->base
					+ USB3_DP_QSERDES_RXA_RX_MODE_01_HIGH4));

		/* USB3_RXB */
		dev_info(uphy->dev, "USB3:RXB_RX_MODE_01_HIGH4     :0x%02x\n",
			readb_relaxed(phy->base
					+ USB3_DP_QSERDES_RXB_RX_MODE_01_HIGH4));
	}
