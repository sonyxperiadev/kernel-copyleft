...
static void ulpi_init(struct msm_otg *motg, int *seq)
{
	if (!seq)
		return;
...
static int msm_otg_reset(struct usb_phy *phy)
{
	...
	ulpi_init(motg, pdata->phy_init_seq);
...
static void msm_otg_notify_charger(struct msm_otg *motg, unsigned mA)
{
	...
	case USB_ACA_A_CHARGER:
		if (mA && motg->pdata->chg_drawable_ida)
			mA = motg->pdata->chg_drawable_ida;
		/* fall through */
...
static void msm_hsusb_vbus_power(struct msm_otg *motg, bool on)
{
	...
	if (motg->pdata->vbus_power) {
		...
		if (!ret) {
			if (!on && test_bit(ID_A, &motg->inputs))
				msleep(50);
			vbus_is_on = on;
		}
		return;
...
static void msm_otg_start_recheck_chgtype(struct usb_phy *phy,
					unsigned long delay)
{
	struct msm_otg *motg = container_of(phy, struct msm_otg, phy);
...
static void msm_chg_detect_work(struct work_struct *w)
{
	...
	case USB_CHG_STATE_RECHECK:
		dev_dbg(phy->dev, "Chger type recheck retry count = %d\n",
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	enum usb_otg_state tmp_state = otg->phy->state;

	pm_runtime_resume(otg->phy->dev);
	dev_info(otg->phy->dev, "%s work, inputs=0x%08lx\n",
				otg_state_string(otg->phy->state),
				motg->inputs);
	...
	case OTG_STATE_B_PERIPHERAL:
		if (test_bit(MHL, &motg->inputs)) {
			...
			msm_otg_notify_charger(motg, IDEV_ACA_CHG_MAX);
		} else if ((motg->chg_state == USB_CHG_STATE_DETECTED) &&
				(motg->chg_type == USB_DCP_CHARGER)) {
			msm_otg_start_peripheral(otg, 0);
			otg->phy->state = OTG_STATE_B_IDLE;
			work = 1;
	...
	case OTG_STATE_MHL_CONNECTED:
		if (!test_bit(MHL, &motg->inputs)
				&& !test_bit(B_SESS_VLD, &motg->inputs)) {
			dev_info(otg->phy->dev, "detected the MHL dongle removed\n");
			set_bit(ID, &motg->inputs);
			msm_otg_notify_charger(motg, 0);
			otg->phy->state = OTG_STATE_B_IDLE;
			work = 1;
	...
	if (tmp_state != otg->phy->state)
		dev_info(otg->phy->dev, "changed to %s, from %s\n",
				otg_state_string(otg->phy->state),
				otg_state_string(tmp_state));
	if (work)
		queue_work(motg->wq, &motg->sm_work);
}
...
static void msm_otg_teardown_workqueue(struct msm_otg *motg)
{
	if (motg && motg->wq) {
		flush_workqueue(motg->wq);
...
static irqreturn_t msm_pmic_id_irq(int irq, void *data)
{
	...
	pr_debug("%s: PMIC ID IRQ handling\n", __func__);
	/*schedule delayed work for 5msec for ID line state to settle*/
	queue_delayed_work(motg->wq, &motg->pmic_id_status_work,
...
static int __init msm_otg_probe(struct platform_device *pdev)
{
	...
	motg->wq = create_singlethread_workqueue("otg_work");
	if (!motg->wq) {
		dev_err(&pdev->dev, "cannot create workqueue otg_work\n");
		ret = -ENOMEM;
		goto destroy_wlock;
	}
	...
	phy->start_recheck_chgtype = msm_otg_start_recheck_chgtype;
	phy->stop_recheck_chgtype = msm_otg_stop_recheck_chgtype;
...
teardown_wq:
	msm_otg_teardown_workqueue(motg);
destroy_wlock:
