...
#include "f_serial.c"
//#include "f_acm.c"/*MTD-CONN-JY-USBPORTING-00-*/
#include "f_adb.c"
#include "f_ccid.c"
#include "f_mtp.c"
...
#define F_CDROM  1/*CONN-JY-PCCOMPANION-00+*/
...
static void android_work(struct work_struct *data)
{
	...
	unsigned long flags;
	int pm_qos_vote = -1;
	/*CONN-JY-mtpRandump-00+{*/
	int testcase=0;

	printk(KERN_INFO"usb:%s: in state : %d config %p\n", __func__,next_state,cdev->config);
	printk(KERN_INFO"usb:%s: sw_suspend : %d sw_connect : %d \n", __func__,dev->sw_suspended,dev->sw_connected);
	printk(KERN_INFO"usb:%s:      suspend : %d      connect : %d \n", __func__,dev->suspended,dev->connected);
	if (strncmp(dev->pm_qos, "low", 3))printk(KERN_INFO"usb:%s: pm_qos high\n", __func__);
	/*CONN-JY-mtpRandump-00+}*/
	...
	if (dev->suspended != dev->sw_suspended && cdev->config) {
		...
		testcase=1;/*CONN-JY-mtpRandump-00+*/
	} else if (cdev->config) {
		uevent_envp = configured;
		next_state = USB_CONFIGURED;
		testcase=2;/*CONN-JY-mtpRandump-00+*/
		...
		if (dev->connected && strncmp(dev->pm_qos, "low", 3))
			...
			pm_qos_vote = 0;
		testcase=3;/*CONN-JY-mtpRandump-00+*/
	}
	...
	/*CONN-JY-mtpRandump-00+{*/
	printk(KERN_INFO"usb:%s: out case %d next_state : %d \n", __func__,testcase,next_state);
	if (pm_qos_vote != -1){
		android_pm_qos_update_latency(dev, pm_qos_vote);
		printk(KERN_INFO"usb:%s:  pm_qos_vote : %d \n", __func__,pm_qos_vote);
		}
	/*CONN-JY-mtpRandump-00+}*/
...
/*-------------------------------------------------------------------------*/
/* Supported functions initialization */
#if 0/*MTD-CONN-JY-USBPORTING-00-{*/
/* ACM */
...
#endif/*MTD-CONN-JY-USBPORTING-00-}*/
/* RMNET_SMD */
...
static struct android_usb_function *supported_functions[] = {
	...
	&adb_function,
	&ccid_function,
//	&acm_function,  /*MTD-CONN-JY-USBPORTING-00-*/
	&mtp_function,
	&ptp_function,
	&rndis_function,
	&rndis_qc_function,
...
android_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *c)
{
	...
	unsigned long flags;
	int testcase=0;/*CONN-JY-mtpRandump-00+*/
	...
	spin_unlock_irqrestore(&cdev->lock, flags);
	printk(KERN_INFO"usb:%s case= %d\n", __func__,testcase);/*CONN-JY-mtpRandump-00+*/
...
static void android_disconnect(struct usb_gadget *gadget)
{
	...
	printk(KERN_INFO"usb:%s\n", __func__);/*MTD-CONN-JY-USBPORTING-00+*/
