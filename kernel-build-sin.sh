...
if [ ! -f "$OUT/RPM.bin" ]; then
    die "Missing RPM.bin in $OUT"
fi

echo "Using $OUT/RPM.bin as RPM image..."
...
echo "console=ttyHSL0,115200,n8 androidboot.hardware=sony user_debug=31 androidboot.baseband=msm qdss.etm_boot_enable=1" > cmdline.txt
