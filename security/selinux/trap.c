...
static int cmp_scontext(char *rule, struct common_audit_data *ad)
{
	...
	rc = security_sid_to_context(ad->selinux_audit_data->state,ssid, &scontext, &scontext_len);
	if (rc) {
...
static int cmp_tcontext(char *rule, struct common_audit_data *ad)
{
	...
	rc = security_sid_to_context(ad->selinux_audit_data->state, tsid, &tcontext, &tcontext_len);
	if (rc) {
