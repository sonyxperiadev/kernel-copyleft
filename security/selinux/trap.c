...
bool selinux_trap_security_state;
...
static const char copy_table[]={
	...
	STRING_LSM_AUDIT_DATA_OPRES,	/* op_res ppid tgid tgcomm */
	STRING_LSM_AUDIT_DATA_TERM		/* TERM */
};

static int selinux_trap_setup(char *s)
{
        unsigned long res = memparse(s, NULL);
        selinux_trap_security_state = !!res;
        return 0;
}
early_param("oemandroidboot.security", selinux_trap_setup);
...
static int cmp_action(char *rule, struct common_audit_data *ad)
{
	...
	int i, perm;
	u16 tclass = ad->selinux_audit_data->slad->tclass;
	u32 av = ad->selinux_audit_data->slad->audited;
	u32 av_rule = 0;
	char *temp;
	char *token;
...
static void dump_common_audit_data_part(struct common_audit_data *ad, char type, char string[], int len, int maxlen)
{
	...
	case STRING_LSM_AUDIT_DATA_OPRES:
		...
		if (tsk && tsk->pid) {
			...
			if (tsk->group_leader->pid != tsk->pid) {
				snprintf(&string_work[strlen(string_work)], STRING_PART_LEN_MAX-strlen(string_work), " tgid=%d tgcomm=\"%s\"", tsk->group_leader->pid, tsk->group_leader->comm);
			} else if (tsk->parent->group_leader->pid) {
				snprintf(&string_work[strlen(string_work)], STRING_PART_LEN_MAX-strlen(string_work), " tgid=%d tgcomm=\"%s\"", tsk->parent->group_leader->pid, tsk->parent->group_leader->comm);
			}
		}
		break;
	} /* switch (ad->type) */
	...
	if (strlen(string_work) < maxlen - len) {
		/* parent buff copy */
		strncpy(&string[len], string_work, maxlen - len);
	}
}
...
void trap_selinux_error(struct common_audit_data *ad)
{
	...
	/* In case of production phone, Trap Module shall be disabled. */
	...
	if (ad->tsk && ad->tsk != current) {
		pr_err("SELinux: trap: ad->tsk is not current\n");
		BUG();
		return;
	}
