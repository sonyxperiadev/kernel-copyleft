...
static int sel_make_trap_files(struct dentry *dir,unsigned long *ino)
{
	int i;
	...
	trapped_dentry = sel_make_dir(dir, "trapped", ino);
...
#ifdef CONFIG_SECURITY_SELINUX_TRAP
	dentry = sel_make_dir(sb->s_root, "trap", &fsi->last_ino);
	if (IS_ERR(dentry)) {
		ret = PTR_ERR(dentry);
		goto err;
	}

	ret = sel_make_trap_files(dentry,&fsi->last_ino);
	if (ret)
		goto err;
#endif
