...
static struct gpiomux_setting fchg_i2c = {
	.func = GPIOMUX_FUNC_3,
	.drv  = GPIOMUX_DRV_2MA,
	.pull = GPIOMUX_PULL_NONE,
};
...
static struct msm_gpiomux_config rhine_all_configs[] __initdata = {
	...
	{ /* FCHG_I2C_SDA */
		.gpio = 47,
		.settings = {
			[GPIOMUX_ACTIVE]    = &fchg_i2c,
			[GPIOMUX_SUSPENDED] = &fchg_i2c,
		},
	},
	{ /* FCHG_I2C_SCL */
		.gpio = 48,
	...
	{ /* ANT_TUNE_SW_SEL0 [PK_HASH_INDEX_SRC] */
		.gpio = 122,
		.settings = {
			[GPIOMUX_ACTIVE]    = &grfc,
			[GPIOMUX_SUSPENDED] = &grfc,
	...
	{ /* ANT_TUNE_SW_SEL1 [ALL_USE_SERIAL_NUMBER] */
		.gpio = 123,
		.settings = {
			[GPIOMUX_ACTIVE]    = &grfc,
			[GPIOMUX_SUSPENDED] = &grfc,
	...
	{ /* GEMINI_SW_SEL0 [MSA_PK_HASH_IN_FUSE] */
		.gpio = 126,
		.settings = {
			[GPIOMUX_ACTIVE]    = &grfc,
			[GPIOMUX_SUSPENDED] = &grfc,
	...
	{ /* GRFC26(GSM_TX_PHASE_D2/RF_SW_SEL0) */
		.gpio = 137,
		.settings = {
			[GPIOMUX_ACTIVE]    = &gpio_2ma_follow_qct,
			[GPIOMUX_SUSPENDED] = &gpio_2ma_follow_qct,
...
{
	struct gpiomux_setting *pset = rhine_all_configs[idx].settings[which];
	unsigned set_slot = rhine_all_configs[idx].gpio
	...
	if ((pset->func == GPIOMUX_FOLLOW_QCT) &&
		...
		rhine_all_configs[idx].settings[which] = qct_sets[set_slot];
	} else if ((pset->func == GPIOMUX_FOLLOW_QCT) ||
		(pset->drv == GPIOMUX_FOLLOW_QCT) ||
		(pset->pull == GPIOMUX_FOLLOW_QCT) ||
		(pset->dir == GPIOMUX_FOLLOW_QCT)) {
		if (!qct_sets[set_slot]) {
			rhine_all_configs[idx].settings[which] = NULL;
			return;
		}
		...
		*merge_set[set_slot] = *pset;
		...
		rhine_all_configs[idx].settings[which] = merge_set[set_slot];
	}
}
...
{
#if defined(CONFIG_KS8851) || defined(CONFIG_KS8851_MODULE)
	gpiomux_set_qct_configs(msm_eth_configs, ARRAY_SIZE(msm_eth_configs));
#endif
...
void __init msm_8974_init_gpiomux(void)
{
	...
	gpiomux_remake_configs(ARRAY_SIZE(rhine_all_configs));

	msm_gpiomux_install(rhine_all_configs,
