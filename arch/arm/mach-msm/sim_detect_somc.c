...
#define SIM2_GPIO_NUM 52
#define SIM2_GPIO_DESC "sim2-detection"
...
struct sim_detect_drvdata {
	...
	struct mutex lock;
	unsigned long detection_in_progress;
...
static struct sim_detect_drvdata *pglobal_drvdata;
...
/* can not be used in atomic context */
int simstatus_store(long status)
{
	pr_info("%s called, sim status %ld\n", __func__, status);

	if (!pglobal_drvdata || 1 < status || status < 0)
		goto err_exit;

	sim_detect_report_switch_event(pglobal_drvdata);
	return 0;

err_exit:
	pr_err("%s called, error\n", __func__);
	return -EAGAIN;
}
				  ...
				  struct sim_detect_platform_data *pdata)
{
	...
	events[i].index = i;
	events[i].gpio = SIM2_GPIO_NUM;
	events[i].active_low = 0;
	events[i].desc = SIM2_GPIO_DESC;
...
static irqreturn_t sim_detect_isr(int irq, void *data)
{
	...
	set_bit(event->index, &ddata->detection_in_progress);
...
static void sim_detect_det_work(struct work_struct *work)
{
	...
	clear_bit(event->index, &ddata->detection_in_progress);
}
...
static int sim_detect_set_switch_device(struct sim_detect_drvdata *ddata)
{
	...
	ddata->sim_detect.state = SWITCH_INITIALIZED;
...
static int sim_detect_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (ddata->detection_in_progress) {
