...
static int ssr_monitor_read(struct file *file, char __user *buf, size_t count,loff_t *ppos)
{
	...
	wait_event_interruptible(ssr_md->ssr_monitor_wait_q, ssr_md->data_ready);
...
void ssr_monitor_store_crashreason(char* reason)
{
	...
	strncpy(ssr_monitor_crash_reason,reason,strlen(reason)+1);
}
