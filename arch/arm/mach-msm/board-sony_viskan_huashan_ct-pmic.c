...
static struct pm8xxx_gpio_init pm8921_gpios[] __initdata = {
	...
	/* UIM1_RST_CONN (GPIO_27),
	   ...
	   UIM2_RST   (GPIO_28) is Module-side UICC 2 reset signal.
	   UIM2_M_CLK (GPIO_31) is MSM-side UICC 2 clock signal.
	   UIM2_CLK   (GPIO_32) is Module-side UICC 2 clock signal.
	   Note: These Pin must need set by UIM2 driver in AMSS */
	...
	PM8XXX_GPIO_INPUT(37,   PM_GPIO_PULL_UP_30),	/* SIM2_DET_N */
...
#define GPIO_SW_SIM1_DETECTION		36
#define GPIO_SW_SIM2_DETECTION		37


static struct gpio_event_direct_entry sim1_sw_gpio_map[] = {
	{PM8921_GPIO_PM_TO_SYS(GPIO_SW_SIM1_DETECTION),
	SW_JACK_PHYSICAL_INSERT},
};

static struct gpio_event_direct_entry sim2_sw_gpio_map[] = {
	{PM8921_GPIO_PM_TO_SYS(GPIO_SW_SIM2_DETECTION),
	SW_JACK_PHYSICAL_INSERT},
};

static struct gpio_event_input_info sim1_sw_gpio_info = {
	.info.func = gpio_event_input_func,
	.info.no_suspend = 1,
	.flags = 0,
	.type = EV_SW,
	.keymap = sim1_sw_gpio_map,
	.keymap_size = ARRAY_SIZE(sim1_sw_gpio_map),
};

static struct gpio_event_input_info sim2_sw_gpio_info = {
	.info.func = gpio_event_input_func,
	.info.no_suspend = 1,
	.flags = 0,
	.type = EV_SW,
	.keymap = sim2_sw_gpio_map,
	.keymap_size = ARRAY_SIZE(sim2_sw_gpio_map),
};

static struct gpio_event_info *sim1_gpio_event_info[] = {
	&sim1_sw_gpio_info.info,
};

static struct gpio_event_info *sim2_gpio_event_info[] = {
	&sim2_sw_gpio_info.info,
};


struct gpio_event_platform_data sim1_detection_data = {
	.name       = "sim1-detection",
	.info       = sim1_gpio_event_info,
	.info_count = ARRAY_SIZE(sim1_gpio_event_info),
};

struct gpio_event_platform_data sim2_detection_data = {
	.name       = "sim2-detection",
	.info       = sim2_gpio_event_info,
	.info_count = ARRAY_SIZE(sim2_gpio_event_info),
};


static struct platform_device sim1_detection_device = {
	.name	= GPIO_EVENT_DEV_NAME,
	.id	= 0,
	.dev	= {.platform_data = &sim1_detection_data},
};

static struct platform_device sim2_detection_device = {
	.name	= GPIO_EVENT_DEV_NAME,
	.id	= 1,
	.dev	= {.platform_data = &sim2_detection_data},
};
...
static int __init input_devices_init(void)
{
	...
	platform_device_register(&sim1_detection_device);
	platform_device_register(&sim2_detection_device);
	return 0;
}
...
static void __exit input_devices_exit(void)
{
	...
	platform_device_unregister(&sim1_detection_device);
	platform_device_unregister(&sim2_detection_device);
}
