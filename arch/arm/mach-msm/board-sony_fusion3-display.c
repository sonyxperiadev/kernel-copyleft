...
/* prim = 1920 x 1080 x 4(bpp) x 3(pages) */
#define MSM_FB_PRIM_BUF_SIZE roundup(1920 * 1088 * 4 * 3, 0x10000)
#else
/* prim = 1920 x 1080 x 4(bpp) x 2(pages) */
...
static struct msm_bus_vectors mdp_vga_vectors[] = {
	/* VGA and less video */
	{
		...
		.ib = ((1920 * 1080 * 4 * 60) + (480 * 640 * 60 * 1.5)) * 2
			* 1.25,
	},
};
...
static struct msm_bus_vectors mdp_720p_vectors[] = {
	/* 720p and less video */
	{
		...
		.ib = ((1920 * 1080 * 4 * 60) + (1280 * 720 * 60 * 1.5)) * 2
			* 1.25,
	},
};
...
static struct msm_bus_vectors mdp_1080p_vectors[] = {
	/* 1080p and less video */
	{
		...
		.ib = ((1920 * 1080 * 4 * 60) + (1920 * 1080 * 60 * 1.5)) * 2
			* 1.25,
	},
};
...
static int mipi_dsi_panel_plf_init(void)
{
	...
	if (!dsi_power_on) {
		...
		if (rc) {
			pr_err("%s: set_voltage l2 dsi1_pll_vdda failed, "
						"rc=%d\n", __func__, rc);
			goto put_l29_l2;
		}
		...
		if (rc) {
			pr_err("%s: request LCD_DCDC_EN (gpio %d) failed,"
		...
		if (rc) {
			pr_err("%s:request MLCD_RESET_N (gpio %d) failed,"
...
static const struct panel *panels[] = {
	...
	&sharp_ls050t3sx02_r63311_black_panel_id,
	&sharp_ls050t3sx02_r63311_white_panel_id,
	&sharp_ls050t3sx02_r63311_12624797,
...
static int fusion3_dsi_power_save(int on)
{
...
static struct mipi_dsi_platform_data mipi_dsi_pdata = {
	.dsi_power_save = fusion3_dsi_power_save,
};
