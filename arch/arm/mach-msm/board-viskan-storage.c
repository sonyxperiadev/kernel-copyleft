...
void __init msm8960_init_mmc(void)
{
	...
	/*
	 * Enable the control pin by default, leave the sdcc3 driver
	 * to control the power setting.
	 */
	rc = gpio_request(SD_PWR_EN, "PM8921_MPP04");
	if (rc) {
		pr_err("%s:gpio_request(%d, %s) failed %d\n",
			__func__, SD_PWR_EN, "PM8921_MPP04", rc);
	} else {
		pr_info("%s:gpio_set_value_cansleep(%d)\n",
			__func__, SD_PWR_EN);
