...
/* prim = 1920 x 1200 x 4(bpp) x 3(pages) */
#define MSM_FB_PRIM_BUF_SIZE roundup(1920 * 1200 * 4 * 3, 0x10000)
#else
/* prim = 1920 x 1200 x 4(bpp) x 2(pages) */
#define MSM_FB_PRIM_BUF_SIZE roundup(1920 * 1200 * 4 * 2, 0x10000)
#endif
...
static int mipi_dsi_panel_plf_power(int on)
{
	...
	if (on) {
		...
		msleep(251);				/* Spec says > 250 ms */
	} else {
		pr_debug("%s: Power Off\n", __func__);
		msleep(86);				/* Spec says > 85 ms */
							/* Disable VSP/VSN */
...
#ifdef CONFIG_FB_MSM_MIPI_NT71391_PANASONIC_VVX10F008B00
	&panasonic_vvx10f008b00_panel_id,
#endif /* CONFIG_FB_MSM_MIPI_NT71391_PANASONIC_VVX10F008B00 */
	NULL,
};
...
static void __init mipi_dsi_panel_add_device(void)
{
	...
	set_mdp_clocks_for_wuxga(); /* TODO: Review */
...
static int pollux_dsi_power_save(int on)
{
...
static struct mipi_dsi_platform_data mipi_dsi_pdata = {
	.dsi_power_save = pollux_dsi_power_save,
};
