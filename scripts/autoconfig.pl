...
my @supported_platforms = ("kitakami");
my $kernel_dir;
my $config_dir;
my @products = ();
	...
	foreach (readdir DH) {
		if (/([\w-]+)_defconfig/) {
			if (is_platform_supported($1) == 1) {
				print "\t$1:\n";
				opendir Dir, "$config_dir/$1" or exit 0;
				foreach (readdir Dir) {
					if (/([\w-]+)_diffconfig/) {
...
sub perform_task_diffconfig($$$$)
{
	...
	my ($platform,) =  split(/_/, $defconfig);
	opendir Dir, "$config_dir/$platform" or exit 0;
	foreach (readdir Dir) {
		if (/([\w-]+)_diffconfig/) {
			if ($action eq "s") {
				print "Syncing $_ ...\n";
			...
			if ((is_product_excluded($1) == 0)) {
				...
				system "make ARCH=arm64 `basename $defconfig` > /dev/null 2>&1";
				if ($action ne "s") {
					apply_modification($key, $value, $defconfig);
					system "make ARCH=arm64 `basename olddefconfig` > /dev/null 2>&1";
				}
				$ENV{'KBUILD_DIFFCONFIG'} = $_;
				system "make ARCH=arm64 `basename savediffconfig` > /dev/null 2>&1";
				move("diffconfig", "$config_dir/$platform/$_")
...
sub perform_task_defconfig($$$$)
{
	...
	if (@products == 0) {
		#Updating the platform config
		my $option ="KCONFIG_NOTIMESTAMP=true";
		system "make ARCH=arm64 `basename $defconfig` > /dev/null 2>&1";
		if (($action eq "a") || ($action eq "d")) {
			apply_modification($key, $value, $defconfig);
		}
		system "make ARCH=arm64 savedefconfig $option > /dev/null 2>&1";
		move("defconfig", "$config_dir/$defconfig")
			or die "failed to copy file";
	}

	perform_task_diffconfig($action, $key, $value, $defconfig);
}
...
foreach $task (@tasks) {
	...
	foreach $defconfig (@defconfigs) {
		perform_task_defconfig($action, $key, $value, $defconfig);
	}
}
