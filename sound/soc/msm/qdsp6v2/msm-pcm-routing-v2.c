...
static int msm_routing_get_hearingprotection_control_(
				struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol) {
	struct hearingprotection_params user_param;
	void *param;
	int ret = 0;
	int port_id =
	((struct soc_multi_mixer_control *)kcontrol->private_value)->reg;
	...
	ret = sony_copp_effect_get(port_id, param,
			sizeof(struct hearingprotection_params),
			ADM_MODULE_ID_HP);
	if (ret) {
		pr_err("%s: Getting accumulation time failed %d\n",
			__func__, ret);
		return ret;
	}
	ucontrol->value.integer.value[1] = user_param.accum_time;

	pr_info("%s: accumulation time %d\n",
		__func__, user_param.accum_time);

	return ret;
}

static int msm_routing_get_hearingprotection_control(
				struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol) {
	int ret;

	pr_info("HearingProtection get control\n");
	mutex_lock(&routing_lock);
	ret = msm_routing_get_hearingprotection_control_(kcontrol, ucontrol);
	mutex_unlock(&routing_lock);
	return ret;
}

static int msm_routing_set_hearingprotection_control_(
				struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct hearingprotection_params user_param;
	...
	user_param.enable = value;
	user_param.accum_time = 0;
	pr_info("%s: send HP param(%d, %d) to q6adm\n",
		__func__, user_param.enable, user_param.accum_time);
	user_param.reserved = 0;
...
static int msm_routing_set_hearingprotection_control(
				struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol) {
	int ret;

	pr_debug("HearingProtection control\n");
	mutex_lock(&routing_lock);
	ret = msm_routing_set_hearingprotection_control_(kcontrol, ucontrol);
	mutex_unlock(&routing_lock);
	return ret;
}
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("%s: send ClearAudioVPT ClearStereo param(%d) to q6asm\n",
	...
	pr_info("%s: send ClearAudioVPT eq_coef(%d,%d,%d,%d,%d,%d) to q6asm\n",
	...
	pr_info("%s: send ClearAudioVPT VPT mode(%d) to q6asm\n",
...
static int msm_routing_set_clearaudio_vpt_control(
			struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol) {
	int ret;

	pr_debug("ClearAudio control\n");
	mutex_lock(&routing_lock);
	ret = msm_routing_set_clearaudio_vpt_control_(kcontrol, ucontrol);
	mutex_unlock(&routing_lock);
	return ret;
}
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = sony_popp_effect_set((void *)ac, param,
		sizeof(struct vpt_params), ASM_MODULE_ID_VPT51);
	return ret;
}
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = sony_popp_effect_set((void *)ac, param,
		sizeof(struct dynamicnormalizer_params), ASM_MODULE_ID_DN);
	return ret;
}
...
static const struct snd_kcontrol_new hearingprotection_controls[] = {
	SOC_SINGLE_MULTI_EXT("SLIMBUS_0_RX HearingProtection",
	SLIMBUS_0_RX, SND_SOC_NOPM, 255, 0, 2,
	msm_routing_get_hearingprotection_control,
	msm_routing_set_hearingprotection_control),
	SOC_SINGLE_MULTI_EXT("RT_PROXY_PORT_001_RX HearingProtection",
	RT_PROXY_PORT_001_RX, SND_SOC_NOPM, 255, 0, 2,
	msm_routing_get_hearingprotection_control,
	msm_routing_set_hearingprotection_control),
};

static const struct snd_kcontrol_new clearaudio_vpt_controls[] = {
	SOC_SINGLE_MULTI_EXT("MultiMedia1 ClearAudioVPT",
	MSM_FRONTEND_DAI_MULTIMEDIA1, SND_SOC_NOPM, 255, 0, 9,
	msm_routing_get_clearaudio_vpt_control,
	msm_routing_set_clearaudio_vpt_control),
	SOC_SINGLE_MULTI_EXT("MultiMedia3 ClearAudioVPT",
	MSM_FRONTEND_DAI_MULTIMEDIA3, SND_SOC_NOPM, 255, 0, 9,
	msm_routing_get_clearaudio_vpt_control,
	msm_routing_set_clearaudio_vpt_control),
	SOC_SINGLE_MULTI_EXT("MultiMedia4 ClearAudioVPT",
	MSM_FRONTEND_DAI_MULTIMEDIA4, SND_SOC_NOPM, 255, 0, 9,
	msm_routing_get_clearaudio_vpt_control,
	msm_routing_set_clearaudio_vpt_control),
	SOC_SINGLE_MULTI_EXT("MultiMedia6 ClearAudioVPT",
	MSM_FRONTEND_DAI_MULTIMEDIA6, SND_SOC_NOPM, 255, 0, 9,
	msm_routing_get_clearaudio_vpt_control,
	msm_routing_set_clearaudio_vpt_control),
	SOC_SINGLE_MULTI_EXT("MultiMedia7 ClearAudioVPT",
	MSM_FRONTEND_DAI_MULTIMEDIA7, SND_SOC_NOPM, 255, 0, 9,
	msm_routing_get_clearaudio_vpt_control,
	msm_routing_set_clearaudio_vpt_control),
	SOC_SINGLE_MULTI_EXT("MultiMedia8 ClearAudioVPT",
	MSM_FRONTEND_DAI_MULTIMEDIA8, SND_SOC_NOPM, 255, 0, 9,
	msm_routing_get_clearaudio_vpt_control,
	msm_routing_set_clearaudio_vpt_control),
};
...
static int msm_routing_probe(struct snd_soc_platform *platform)
{
	...
	snd_soc_add_platform_controls(platform,
				hearingprotection_controls,
			ARRAY_SIZE(hearingprotection_controls));

	snd_soc_add_platform_controls(platform,
				clearaudio_vpt_controls,
			ARRAY_SIZE(clearaudio_vpt_controls));
