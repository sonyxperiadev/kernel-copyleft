...
#define START_COEFF_STRUCT {.coeff = DEFAULT_COEFF, .num_shift_factor = 2, \
			.pan_setting = IIR_COEFF_CHANNEL_CENTER}
		...
		uint32_t param_id, uint8_t *param, uint32_t param_len)
{
	...
	param_value = kmalloc(pkt_len, GFP_KERNEL);
	if (!param_value) {
		pr_err("%s, params memory alloc failed\n", __func__);
		return -ENOMEM;
	}

	p_hdr = (struct param_hdr_v3 *)param_value;
	p_hdr->module_id = module_id;
	p_hdr->instance_id = INSTANCE_ID_0;
	p_hdr->param_id = param_id;
	p_hdr->param_size = param_len;
	p_hdr->reserved = 0;
	memcpy(param_value + sizeof(*p_hdr), param, param_len);

	rc = adm_ahc_send_params(port_id, ahc_copp_idx, param_value,
			pkt_len);
	if (rc) {
		...
		uint32_t *param_id, uint32_t *module_id)
{
	...
	case AHC_IIR_TF_REG_CONFIG_PARAMS:
		pr_debug("%s: AHC_IIR_TF_REG_CONFIG_PARAMS for band %d\n",
				__func__, band);
		*param_len = sizeof(struct iir_tf_5_config);
		...
		struct snd_ctl_elem_value *ucontrol, uint8_t *params_value)
{
	...
	case AHC_IIR_TF_REG_CONFIG_PARAMS: {
		struct iir_tf_5_config *param =
				(struct iir_tf_5_config *)params_value;
		int32_t *p_coeff;
		int i;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	/* Adjust size to 4-byte alignment. */
	param_len = (param_len + 3) & ~3;

	param_size = param_len + sizeof(struct param_hdr_v3);
	...
	rc = adm_get_pp_params(AHC_PORT_ID, 0, ADM_CLIENT_ID_DEFAULT,
	...
	rc = set_values_for_get_cmd(cmd, band, ucontrol, param_value);
