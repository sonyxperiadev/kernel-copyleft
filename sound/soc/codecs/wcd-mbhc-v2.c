...
static void wcd_enable_mbhc_supply(struct wcd_mbhc *mbhc, enum wcd_mbhc_plug_type plug_type)
{
	struct snd_soc_codec *codec = mbhc->codec;
	...
	if (det_extn_cable_en && mbhc->is_extn_cable &&
		...
		if (plug_type == MBHC_PLUG_TYPE_HEADPHONE || plug_type == MBHC_PLUG_TYPE_HEADSET)
			wcd_enable_curr_micbias(mbhc, WCD_MBHC_EN_MB);
	} else {
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	int cross_conn;
	int try = 0;
	bool skip_check_cross_conn = false;
	...
	if (mbhc->mbhc_cb->mbhc_common_micb_ctrl)
		mbhc->mbhc_cb->mbhc_common_micb_ctrl(codec, MBHC_COMMON_MICB_SET_18_VAL, true);
#endif
