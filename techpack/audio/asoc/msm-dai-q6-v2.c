...
struct msm_dai_q6_dai_data {
	...
	union afe_port_config port_config;
        struct afe_enc_bitrate enc_bitrate;
	u16 vi_feed_mono;
};
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (dai_data) {
		...
		case ENC_FMT_LDAC:
			memcpy(ucontrol->value.bytes.data + format_size,
				&dai_data->enc_config.data,
				sizeof(struct asm_custom_enc_cfg_ldac_t));
			break;
		default:
...
static int  msm_dai_q6_afe_quality_mode_info(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_BYTES;
	uinfo->count = sizeof(struct afe_enc_bitrate);
	return 0;
}


static int msm_dai_q6_afe_quality_mode_get(
			struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;
	...
	if (dai_data) {
		int format_size = sizeof(dai_data->enc_bitrate.format);
		memcpy(ucontrol->value.bytes.data,
			&dai_data->enc_bitrate.format,
			format_size);
		switch (dai_data->enc_bitrate.format) {
		case ENC_FMT_LDAC:
			memcpy(ucontrol->value.bytes.data + format_size,
				&dai_data->enc_bitrate.data,
				sizeof(union afe_enc_bitrate_data));
			pr_debug("%s: ldac qmode = %d\n", __func__,
				dai_data->enc_bitrate.data.ldac_qmode);
			break;
...
static int msm_dai_q6_afe_quality_mode_put(
			struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;
	...
	if (dai_data) {
		int format_size = sizeof(dai_data->enc_bitrate.format);
		memset(&dai_data->enc_bitrate, 0x0, sizeof(struct afe_enc_bitrate));
		memcpy(&dai_data->enc_bitrate.format,
			ucontrol->value.bytes.data,
			format_size);
		pr_debug("%s: enc_format is %x\n", __func__,
			dai_data->enc_bitrate.format);

		switch (dai_data->enc_bitrate.format) {
		case ENC_FMT_LDAC:
			memcpy(&dai_data->enc_bitrate.data,
				ucontrol->value.bytes.data + format_size,
				sizeof(union afe_enc_bitrate_data));
			pr_debug("%s: enc_format.data {link_quality:%x, q_mode:%x}\n",
			__func__, dai_data->enc_bitrate.data.ldac_quality_mode[0],
			dai_data->enc_bitrate.data.ldac_quality_mode[1]);
			ret = q6afe_send_enc_bitrate(SLIMBUS_7_RX,
				&dai_data->enc_bitrate.data,
				dai_data->enc_bitrate.format,
				dai_data->port_config,dai_data->afe_in_channels, 24);

			if(ret < 0) {
				pr_err("%s: AFE_encoder config for port 0x%x failed %d\n",
					__func__, SLIMBUS_7_RX, ret);
			}
			break;
...
static const struct snd_kcontrol_new afe_enc_bitrate_controls[] = {
	{
		.access = (SNDRV_CTL_ELEM_ACCESS_READWRITE |
			   SNDRV_CTL_ELEM_ACCESS_INACTIVE),
		.iface = SNDRV_CTL_ELEM_IFACE_PCM,
		.name = "AFE Set Quality Mode",
		.info = msm_dai_q6_afe_quality_mode_info,
		.get = msm_dai_q6_afe_quality_mode_get,
		.put = msm_dai_q6_afe_quality_mode_put,
	},
};
...
static int msm_dai_q6_dai_probe(struct snd_soc_dai *dai)
{
	...
	case SLIMBUS_7_RX:
		...
		rc = snd_ctl_add(dai->component->card->snd_card,
				 ...
				 dai_data));
		rc = snd_ctl_add(dai->component->card->snd_card,
				 snd_ctl_new1(&afe_enc_bitrate_controls[0],
				 dai_data));
		rc = snd_ctl_add(dai->component->card->snd_card,
