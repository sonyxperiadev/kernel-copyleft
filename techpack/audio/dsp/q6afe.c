...
int q6afe_send_enc_bitrate(u16 port_id,
				 union afe_enc_bitrate_data *cfg, u32 format,
				 ...
				 u16 afe_in_channels, u16 afe_in_bit_width)
{
	...
	config.pdata.param_id = AVS_ENCODER_PARAM_ID_ENC_BITRATE;
	config.param.payload_size = payload_size
					+ sizeof(config.port.enc_bitrate);
	pr_debug("%s: send AVS_ENCODER_PARAM_ID_ENC_BITRATE to DSP payload:%d\n",
				__func__, config.param.payload_size);
	config.pdata.param_size = sizeof(config.port.enc_bitrate);
	config.port.enc_bitrate = *cfg;
	pr_debug("%s: send AVS_ENCODER_PARAM_ID_ENC_BITRATE data:%x\n",
				__func__, config.port.enc_bitrate.ldac_qmode);

	ret = afe_apr_send_pkt(&config, &this_afe.wait[index]);
	if (ret) {
		pr_err("%s: AVS_ENCODER_PARAM_ID_ENC_BITRATE for port 0x%x failed %d\n",
			__func__, port_id, ret);
		goto exit;
	}
exit:
...
EXPORT_SYMBOL(q6afe_send_enc_bitrate);
