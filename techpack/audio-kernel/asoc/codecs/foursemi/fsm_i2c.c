...
int fsm_read_efsdata(struct fsm_calib_v2 *data)
{
	...
	result = get_re25_from_buf(data, buf, sizeof(buf));
	filp_close(fp, NULL);
	// set_fs(fs);
...
int fsm_write_efsdata(struct fsm_calib_v2 *data)
{
	...
	len = set_re25_to_buf(data, buf, sizeof(buf));
	if (len <= 0) {
		pr_err("re25 is invalid, len: %d", len);
		return -EINVAL;
	}
	// fs = get_fs();
	// set_fs(get_ds());
...
/*Date:20200915 Reason: for smart PA  compatible Author:sunjingtao Begin*/
int fsm_get_probe_status(void)
{
	return is_fsm_probed;
}
EXPORT_SYMBOL(fsm_get_probe_status);
/*Date:20200915 Reason: for smart PA  compatible Author:sunjingtao Begin*/
