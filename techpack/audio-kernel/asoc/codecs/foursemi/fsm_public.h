 ...
 * module: fsm_core
 */
 int fsm_get_probe_status(void);  //Date:20200915 Reason: for smart PA  compatible Author:sunjingtao
...
int fsm_hal_set_params(void *data, int size);
int fsm_hal_get_livedata(fsm_msg_t *data);
int fsm_hal_send_afe_cal(void *buf, int cmd_size, int buf_size);
int fsm_hal_save_re25(struct fsm_calib_v2 *data);
int fsm_hal_update_re25(int re25);
...
#ifdef CONFIG_FSM_FS1958
void fs1958_ops(fsm_dev_t *fsm_dev);
#else
#define fs1958_ops(...)
#endif

#ifdef CONFIG_FSM_FS1820
int fs1820_dev_detect(fsm_dev_t *fsm_dev, uint16_t *id);
#else
#define fs1820_dev_detect(...) (-EINVAL)
#endif
