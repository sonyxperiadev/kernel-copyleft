...
def gen_gfx_headers(verbose, gen_dir, headers_install, unifdef, gfx_include_uapi):
    error_count = 0
    for h in gfx_include_uapi:
        ...
        if not run_headers_install(
                ...
                gfx_uapi_include_prefix, h): error_count += 1
    return error_count
...
def main():
    ...
    parser.add_argument(
            '--gfx_include_uapi', required=True, nargs='*',
    ...
    if args.verbose:
        ...
        print('gfx_include_uapi [%s]' % args.gfx_include_uapi)
        print('headers_install [%s]' % args.headers_install)
        print('unifdef [%s]' % args.unifdef)

    return gen_gfx_headers(args.verbose, args.gen_dir,
            args.headers_install, args.unifdef, args.gfx_include_uapi)
