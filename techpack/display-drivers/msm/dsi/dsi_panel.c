...
int dsi_panel_switch(struct dsi_panel *panel)
{
	...
	mutex_lock(&panel->panel_lock);
	hmd_mode = panel->spec_pdata->hmd_mode;
	...
	if (refresh_rate == 120u && hmd_mode == 1) {
		rc = dsi_panel_set_hmd_mode(panel, 1);
		if (rc) {
			DSI_ERR("[%s] failed to set HMD mode ON, current mode=%d\n",
				panel->name, panel->spec_pdata->hmd_mode);
		}
	}
